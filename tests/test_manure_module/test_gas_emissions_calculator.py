...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
        0.0, 0.0, 0.0, 20.0, ValueError, "Total volatile solids must be positive. Total volatile solids provided: 0.0"),
...
def test_methane_emission_from_slurry_storage(
    ...
    if isinstance(expected, type) and issubclass(expected, Exception):
        with pytest.raises(expected, match=error_message):
            GasEmissionsCalculator.methane_emission_from_slurry_storage(total_volatile_solids,
                                                                        total_degradable_volatile_solids,
