...
def test_manure_manager_init(mocker: MockFixture) -> None:
    ...
    patch_for_manure_manager_config_handler.assert_called_once_with(
        mock_manure_manager_config
    )
    assert (
        manure_manager.manure_manager_config_handler
        == mock_manure_manager_config_handler
    )
    patch_for_manure_nutrient_manager.assert_called_once()
    patch_for_configure_manure_manager_components.assert_called_once_with(
        mock_animal_manager
    )
...
def test_configure_manure_manager_components(
    manure_separator: str, mocker: MockFixture
) -> None:
    ...
    mock_manure_manager_pen.manure_treatment = (
        manure_treatment
    ) = "test_manure_treatment"
    ...
    mock_manure_manager_config_handler.get_custom_bedding_config.return_value = (
        mock_custom_bedding_config
    )
    ...
    mock_manure_manager_config_handler.get_manure_handler_config.return_value = (
        mock_manure_handler_config
    )
    ...
    mock_manure_manager_config_handler.get_custom_bedding_config.assert_called_once_with(
        bedding_type
    )
    ...
    mock_manure_manager_config_handler.get_manure_handler_config.assert_called_once_with(
        manure_handler
    )
    ...
    if manure_separator == "none":
        ...
        mock_manure_manager_config_handler.get_custom_manure_separator_config.assert_called_with(
            manure_separator
        )
    ...
    mock_manure_manager_config_handler.get_custom_manure_treatment_config.assert_called_once_with(
        manure_treatment
    )
...
def test_is_compound_anaerobic_manure_treatment(
    ...
    # Act
    result = ManureManager._is_compound_anaerobic_manure_treatment(
        manure_treatment_name
    )
...
def test_handle_update_for_compound_anaerobic_manure_treatment(
    ...
    manure_manager.manure_separators_after_digestion = {
        pen_id: mock_manure_separator_after_digestion
    }
    ...
    mock_manure_treatment.anaerobic_digestion_daily_output = (
...
def test_pen_daily_update_for_separator_and_treatment(
    ...
    patch_for_handle_daily_update_for_compound_anaerobic_manure_treatment = (
        mocker.patch(
            "RUFAS.routines.manure.manure_manager.ManureManager."
    ...
    # Assert
    patch_for_is_compound_anaerobic_manure_treatment.assert_called_once_with(
        manure_treatment
    )
...
def test_pen_daily_update(mocker: MockFixture) -> None:
    ...
    patch_for_add_manure_nutrients.assert_called_once_with(
        mock_manure_manager_pen, mock_manure_treatment_daily_output
    )
...
def test_manure_manager_daily_update(mocker: MockFixture) -> None:
    ...
    assert patch_for_pen_daily_update.call_args_list == [
        mocker.call(simulation_day, pen) for pen in mock_all_pens
    ]
...
def test_get_manure_type(
    treatment_type: ManureTreatmentType, expected_manure_type: ManureType
) -> None:
    """
...
def test_get_manure_density_by_type(
    manure_type: ManureType, expected_density: float
) -> None:
    """
...
def test_add_manure_nutrients(mocker: MockFixture) -> None:
    ...
    # Arrange
    mocker.patch(
        "RUFAS.routines.manure.manure_manager.ManureManager.__init__", return_value=None
    )
    manure_manager = ManureManager(
    ...
    expected_calls = [
        call(
            nitrogen=max(
                mock_manure_treatment_daily_output.liquid_manure_nitrogen, 0.0
            ),
            phosphorus=max(
                mock_manure_treatment_daily_output.liquid_manure_phosphorus, 0.0
            ),
            potassium=max(
                mock_manure_treatment_daily_output.liquid_manure_potassium, 0.0
            ),
            dry_matter=max(
                mock_manure_treatment_daily_output.liquid_manure_total_solids, 0.0
            ),
            total_manure_mass=max(
                mock_manure_treatment_daily_output.liquid_manure_daily_volume
                * mock_manure_density,
                0.0,
            ),
...
def test_request_nutrients(mocker: MockFixture) -> None:
    ...
    mock_manure_nutrient_manager.request_nutrients.return_value = (
        mock_nutrient_request_results
    )
    ...
    # Assert
    mock_manure_nutrient_manager.request_nutrients.assert_called_once_with(
        mock_nutrient_request
    )
