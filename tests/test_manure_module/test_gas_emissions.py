...
def test_calc_nitrogen_loss_from_ammonia_emission(daily_nitrogen_input: float, is_bedding_tilled: bool,
                                                  expected: float, expected_error: type[Exception]) -> None:
    """
    Unit test for _calc_nitrogen_loss_from_ammonia_emission() method in gas_emissions.py.
    ...
    if expected_error:
        with pytest.raises(expected_error):
            GasEmissions._calc_nitrogen_loss_from_ammonia_emission(daily_nitrogen_input, is_bedding_tilled)
    else:
        actual = GasEmissions._calc_nitrogen_loss_from_ammonia_emission(daily_nitrogen_input, is_bedding_tilled)
        assert actual == pytest.approx(expected)
...
def test_calc_nitrogen_loss_to_leaching(daily_nitrogen_input: float, expected: float,
                                        expected_error: type[Exception]) -> None:
    """
    Unit test for _calc_nitrogen_loss_to_leaching() method in gas_emissions.py.
    ...
    if expected_error:
        ...
        actual = GasEmissions._calc_nitrogen_loss_to_leaching(daily_nitrogen_input)
        assert actual == pytest.approx(expected)
...
def test_calc_nitrogen_loss_from_nitrous_oxide_emission(daily_nitrogen_input: float, is_bedding_tilled: bool,
                                                        expected: float, expected_error: type[Exception]) -> None:
    """
    Unit test for _calc_nitrogen_loss_from_nitrous_oxide_emission() method in gas_emissions.py.
    ...
    if expected_error:
        with pytest.raises(expected_error):
            GasEmissions._calc_nitrogen_loss_from_nitrous_oxide_emission(daily_nitrogen_input, is_bedding_tilled)
    else:
        actual = GasEmissions._calc_nitrogen_loss_from_nitrous_oxide_emission(daily_nitrogen_input, is_bedding_tilled)
        assert actual == pytest.approx(expected)
...
def test_calc_total_nitrogen_loss_from_compost_bedded_pack_barn(daily_nitrogen_input: float,
    ...
    # Arrange
    mocker.patch(
        'RUFAS.routines.manure.gas_emissions.gas_emissions.GasEmissions._calc_nitrogen_loss_from_ammonia_emission',
    ...
    mocker.patch(
        'RUFAS.routines.manure.gas_emissions.gas_emissions.GasEmissions._calc_nitrogen_loss_from_nitrous_oxide_emission',
