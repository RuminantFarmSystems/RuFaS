...
def test_calc_E_CH4_slurry_storage(enclosed: bool, mocker: MockerFixture) -> None:
    """Tests calc_E_CH4_slurry_storage() in gas_emissions.py."""
    ...
    VS_frac = 0.5
    n_eff = 0.99
    VSd = GasEmissionConstants.Bo / GasEmissionConstants.POTENTIAL_METHANE_YIELD_OF_MANURE
    VSnd = 1 - VSd
    ...
    expected_E_CH4_open_air = 0.024 * TS * VS_frac * (VSd * b1 + VSnd * b2) * ex
    expected_E_CH4_enclosed = expected_E_CH4_open_air * (1 - n_eff)

    # Act
    actual = GasEmissions.calc_E_CH4_slurry_storage(TS, enclosed, tempC, VS_frac, n_eff)
...
def test_calc_E_CH4_housing(ambient_temp: float, mocker: MockerFixture) -> None:
    """Tests calc_E_CH4_housing() in gas_emissions.py."""
    ...
    # Act
    actual = GasEmissions.calc_E_CH4_housing(num_animals, barn_area, hours, t_min, t_max)
...
@pytest.mark.parametrize('ambient_temp', [temp for temp in range(-40, 40, 10)])
def test_calc_E_CO2_housing(ambient_temp: float, mocker: MockerFixture) -> None:
    """Tests calc_E_CO2_housing() in gas_emissions.py."""
    ...
    # Act
    actual = GasEmissions.calc_E_CO2_housing(num_animals, barn_area, hours, t_min, t_max)
...
def test_calc_E_NH3_emission(sign_of_RMQ: int, mocker: MockerFixture) -> None:
    """Tests calc_E_NH3_emission() in gas_emissions.py."""
    ...
    patch_for_calc_r_barn = mocker.patch(
            'RUFAS.routines.manure.gas_emissions.gas_emissions.GasEmissions._calc_r_barn',
            return_value=r,
    )
    ...
    M = urine / barn_area
    expected = num_animals * barn_area * ((urine_TAN / barn_area) * c * p) / (r * M * Q)

    # Act
    actual = GasEmissions.calc_E_NH3_emission(num_animals, barn_area, urine_TAN, urine, tempC, hsc)
...
def test_calc_r_barn() -> None:
    """Tests _calc_r_barn() in gas_emissions.py."""
    ...
    # Act
    actual = GasEmissions._calc_r_barn(tempC)
...
def test_calc_CH4_volume_using_Chen_equation() -> None:
    """Tests _calc_CH4_volume_using_Chen_equation() in gas_emissions.py."""
    ...
    # Act
    actual = GasEmissions.calc_CH4_volume_using_Chen_equation(VS_total, hydraulic_retention_time)
...
def test_calc_E_CH4_anaerobic_lagoon() -> None:
    """Tests calc_E_CH4_anaerobic_lagoon() in gas_emissions.py."""

    # Arrange
    VS = 10.0
    expected = (VS * GasEmissionConstants.Bo *
                GasEmissionConstants.MCF * GasEmissionConstants.MS * GasEmissionConstants.METHANE_FACTOR)

    # Act
    actual = GasEmissions.calc_E_CH4_anaerobic_lagoon(VS)
