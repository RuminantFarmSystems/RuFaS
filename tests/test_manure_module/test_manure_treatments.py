...
def test_get_current_day_temperature_and_rainfall(manure_treatment_type_name: str,
    ...
    mock_weather.rainfall[mock_time.year - 1][mock_time.day - 1] = rainfall_mm
...
def test_anaerobic_lagoon_update_methane_emission(mocker: MockFixture,
                                                  initial_methane_emission: float,
                                                  volatile_solids: float,
                                                  volatile_solids_factor: int,
                                                  expected_methane_emission: float,
                                                  expected_liquid_manure_volatile_solids: float) -> None:
    """
    ...
    patch_for_calc_methane_emission_for_slurry_storage = mocker.patch(
        ...
        '.GasEmissions.calc_methane_emission_from_slurry_storage',
    ...
    assert anaerobic_lagoon._accumulated_output.storage_methane == expected_methane_emission
...
def test_anaerobic_lagoon_update_ammonia_emission(mocker: MockFixture,
                                                  liquid_manure_ammoniacal_nitrogen: float,
                                                  urine_ammoniacal_nitrogen: float,
                                                  housing_ammonia: float,
                                                  mock_storage_ammonia_emission_value: float) -> None:
    """
    ...
    # Arrange
    mock_daily_output = mocker.MagicMock(spec=ManureTreatmentDailyOutput)
    mock_daily_output.liquid_manure_total_ammoniacal_nitrogen = liquid_manure_ammoniacal_nitrogen
...
@pytest.mark.parametrize(
    'lagoon_depth, lagoon_slope, volume_needed, expected_a, expected_b, expected_c, raises_error', [
        ...
        (1e-9, 2.0, 100.0, 3e-9, -8e-18, -100, False),  # Edge case with small lagoon_depth
    ])
def test_anaerobic_lagoon_calc_lagoon_width_coefficients(mocker: MockFixture, lagoon_depth: float,
                                                         lagoon_slope: float, volume_needed: float,
                                                         expected_a: float, expected_b: float,
                                                         expected_c: float, raises_error: bool) -> None:
    """
    ...
    if raises_error:
        ...
        actual_a, actual_b, actual_c = anaerobic_lagoon._calc_lagoon_width_coefficients()
        assert actual_a == approx(expected_a)
        assert actual_b == approx(expected_b)
        assert actual_c == approx(expected_c)


@pytest.mark.parametrize('a, b, c, expected_width', [
...
def test_anaerobic_lagoon_width(mocker: MockFixture, a: float, b: float, c: float, expected_width: float) -> None:
    ...
    mocker.patch.object(
        anaerobic_lagoon,
        '_calc_lagoon_width_coefficients',
        return_value=(a, b, c)
    )
...
@pytest.mark.parametrize('lagoon_width', [
    5.0,
    10.0,
    15.0,
    20.0,
...
@pytest.mark.parametrize('lagoon_width, lagoon_length', [
...
@pytest.mark.parametrize('lagoon_length, lagoon_width, lagoon_depth, lagoon_slope, expected_volume', [
    (30.0, 10.0, 5.0, 0.01, 1491.666667),  # Standard case with moderate slope
    (50.0, 20.0, 5.0, 0.02, 4968.333333),  # Larger lagoon dimensions and higher slope
...
def test_calc_modeled_lagoon_volume(mocker: MockFixture,
                                    lagoon_length: float,
                                    lagoon_width: float,
                                    lagoon_depth: float,
                                    lagoon_slope: float,
...
@pytest.mark.parametrize('current_day_rainfall, lagoon_surface_area, expected_precipitation_volume', [
...
def test_anaerobic_lagoon_precipitation_volume(mocker: MockFixture,
                                               current_day_rainfall: float,
                                               lagoon_surface_area: float,
                                               expected_precipitation_volume: float) -> None:
    """
...
@pytest.mark.parametrize('freeboard_input, lagoon_surface_area, expected_freeboard_volume', [
...
def test_anaerobic_lagoon_freeboard_volume(mocker: MockFixture,
                                           freeboard_input: float,
                                           lagoon_surface_area: float,
                                           expected_freeboard_volume: float) -> None:
    """
...
def test_bound_sludge_accumulation_volume(mocker: MockFixture,
