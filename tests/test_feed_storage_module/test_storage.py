...
def test_calculate_crude_protein_loss_to_effluent(
    storage: Storage, protein: float, effluent: float, expected: float
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (2.3, 0.0, 0.1, 1.0)(1.3, 3.4, 1.0, 1.3),
        (0.0, 2.1, 0.11, 0.0),
        (2.3, 1.2, 0.0, 0.0),
    ],
)
def test_calculate_non_protein_nitrogen_loss_to_effluent(
    storage: Storage, nitrogen: float, protein: float, effluent: float, expected: float
) -> None:
    """Test the calculate_non_protein_nitrogen_loss_to_effluent method in the Storage class."""
    actual = storage.calculate_non_protein_loss_to_effluent(nitrogen, protein, effluent)
