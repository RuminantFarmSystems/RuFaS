...
@pytest.mark.parametrize("snow_fall, rainfall, actual", [
    (0, 1, CurrentDayWeather(incoming_light=1, min_air_temperature=1, mean_air_temperature=1, max_air_temperature=1,
                             annual_mean_air_temperature=1, precipitation=1, irrigation=1, daylength=8.5)),
    (0, 2, CurrentDayWeather(incoming_light=1, min_air_temperature=1, mean_air_temperature=0, max_air_temperature=1,
                             annual_mean_air_temperature=1, precipitation=2, irrigation=1, daylength=8.5)),
    (3, 0, CurrentDayWeather(incoming_light=1, min_air_temperature=1, mean_air_temperature=-1, max_air_temperature=1,
                             annual_mean_air_temperature=1, precipitation=3, irrigation=1, daylength=8.5))
])
def test_current_weather_snowfall(snow_fall: int, rainfall: int, actual: CurrentDayWeather) -> None:
...
def test_determine_daylength(months: List[int], expected: List[int]):
    ...
    day_length = []
    for month in months:
        day_length.append(CurrentDayWeather.determine_daylength(month))
