...
def test_validate_manure_parameters(name: str, years: List[int], days: List[int], nitrogen: List[float],
                                    ...
                                    depths: List[float], remainder_fracs: List[float], expected: str) -> None:
    """Tests that invalid input is caught and raised with the correct error message in the init function."""
    with pytest.raises(ValueError) as e:
        ManureSchedule(name, years, days, nitrogen, phosphorus, manure_type, field_coverage, depths, remainder_fracs,
                       1, 1)
    assert str(e.value) == expected


@pytest.mark.parametrize("years,days,nitrogen,phosphorus,manure_type,coverage,depth,surface_frac,skip,repeat,expected",
                         [
                             ([1990, 1992], [100], [20], [20, 25], [ManureType.LIQUID], [0.8], [0], [1.0], 1, 1, [
                             ...
                             ([1990, 1990], [100, 200], [25, 10], [5, 5], [ManureType.LIQUID], [0.8, 0.6], [15, 0],
                              [0.3, 1.0], 0, 2, [
                             ...
                             ([1998], [115], [27], [22], [ManureType.LIQUID], [0.85], [0], [1.0], 0, 6, [
                             ...
                             ([1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999], [95, 94, 100, 95, 96, 89, 90, 93],
                              [18], [10], [ManureType.LIQUID], [0.9], [30], [0.7], 0, 0, [
...
def test_generate_manure_events(years: List[int], days: List[int], nitrogen: List[float], phosphorus: List[float],
                                manure_type: List[ManureType], coverage: List[float], depth: List[float],
                                surface_frac: List[float], skip: int, repeat: int, expected: List[ManureEvent]) -> None:
    """Tests that a full list of ManureEvents is correctly generated by the ManureSchedule."""
    man_sched = ManureSchedule("test", years, days, nitrogen, phosphorus, manure_type, coverage, depth, surface_frac,
                               skip, repeat)
