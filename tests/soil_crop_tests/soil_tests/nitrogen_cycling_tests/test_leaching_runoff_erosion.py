...
def test_erode_nitrogen(nitrates: float, ammonium: float, fresh: float, active: float, stable: float,
    ...
    runoff = 2.1
    data = SoilData(field_size=field_size, soil_layers=[layer], accumulated_runoff=runoff, eroded_sediment=0.92)
    ...
    inorganic_loss_calls = [
        call(nitrates, runoff, NITRATE_RUNOFF_COEFFICIENT, layer.bulk_density, layer.layer_thickness, field_size),
        call(ammonium, runoff, AMMONIUM_RUNOFF_COEFFICIENT, layer.bulk_density, layer.layer_thickness, field_size)
    ]
    ...
    with patch.object(incorp, "_calculate_nitrogen_removed_by_water", return_value=45) as calc_inorganic_loss, \
...
def test_leach_nitrogen() -> None:
    ...
    for index in [0, 2, 3]:
        ...
        new_calls = [
            call(40, 3.5, NITRATE_PERCOLATION_COEFFICIENT, layer.bulk_density, layer.layer_thickness, field_size),
            call(35, 3.5, AMMONIUM_PERCOLATION_COEFFICIENT, layer.bulk_density, layer.layer_thickness, field_size),
            call(
                15, 3.5, ACTIVE_ORGANIC_NITROGEN_PERCOLATION_COEFFICIENT, layer.bulk_density, layer.layer_thickness,
                field_size
            )
        ]
