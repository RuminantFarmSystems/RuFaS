...
@pytest.mark.parametrize(
    "active,stable", [(12, 5), (1.8, 15), (4, 4), (0, 6), (7, 0), (0, 0)]
)
def test_determine_intra_organic_mineralization(active: float, stable: float) -> None:
    ...
    observed = HumusMineralization._determine_intra_organic_mineralization(
        active, stable
    )
    expect = (10 ** (-5)) * (
        active * ((1 / FRACTION_OF_HUMIC_NITROGEN_IN_ACTIVE_POOL) - 1) - stable
    )
    if expect > 0:
...
def test_mineralize_organic_nitrogen(
    active_to_stable: float, active_to_nitrate: float, temp: float
) -> None:
    ...
    incorp._determine_intra_organic_mineralization = MagicMock(
        return_value=active_to_stable
    )
    incorp._determine_organic_to_nitrate_mineralization = MagicMock(
        return_value=active_to_nitrate
    )
    ...
    with patch.multiple(
        ...
        if temp <= 0:
            ...
            incorp._determine_intra_organic_mineralization.assert_called_once_with(
                15, 12
            )
            ...
            assert (
                ...
                == 15 - active_to_stable - active_to_nitrate
            )
            assert (
                incorp.data.soil_layers[0].stable_organic_nitrogen_content
                == 12 + active_to_stable
            )
