...
def test_distribute_residue_nutrients(
    ...
    assert soil_data.get_vectorized_layer_attribute("fresh_organic_nitrogen_content")[1:3] == pytest.approx(expected_n[1:-1])
    assert soil_data.get_vectorized_layer_attribute("labile_inorganic_phosphorus_content") == pytest.approx(expected_p[:-1])
