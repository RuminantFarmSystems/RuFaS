...
def test_extend_phosphate_uptakes_to_full_profile(layers: int) -> None:
    ...
    right length"""
    data = CropData(
        actual_phosphorus_uptakes=[1.0, 2.0, 3.0], inaccessible_soil_layers=layers
    )
    incorp = PhosphorusIncorporation(data)
...
def test_uptake_phosphorus(phosphates, depths):
    ...
    data = CropData(
        potential_phosphorus_uptake=17.5, root_depth=35.0, phosphorus_distro_param=0.32
    )
    incorp = PhosphorusIncorporation(data)
    ...
    NitrogenIncorporation.determine_layer_nutrient_uptake_potential = MagicMock(
        return_value=[3.25, 6.33, 7.10]
    )
    NitrogenIncorporation.determine_layer_nutrient_demands = MagicMock(
        return_value=[12, 15, 17]
    )
    NitrogenIncorporation.determine_layer_nutrient_uptake = MagicMock(
        return_value=[8.9, 9.9, 13.12]
    )
    NitrogenIncorporation.determine_layer_extracted_resource = MagicMock(
    ...
    NitrogenIncorporation.determine_layer_nutrient_uptake_potential.assert_called_once_with(
        [1, 2, 3], 17.5, 35.0, 0.32
    )
    assert data.layer_phosphorus_potentials == [3.25, 6.33, 7.10]

    NitrogenIncorporation.determine_layer_nutrient_demands.assert_called_once_with(
    ...
    NitrogenIncorporation.determine_layer_extracted_resource.assert_called_once_with(
        [8.9, 9.9, 13.12], [1, 2, 3]
    )
...
def test_incorporate_phosphorus(
    phosphates: list[float], depths: list[float], gate: bool
) -> None:
    ...
    soil.set_vectorized_layer_attribute(
        "labile_inorganic_phosphorus_content", phosphates
    )
    incorp = PhosphorusIncorporation(data)

    # mock intermediate functions
    incorp.shift_phosphorus_time = MagicMock(return_value=None)
    NitrogenIncorporation.determine_nutrient_shape_parameters = MagicMock(
        return_value=[1.2, 0.8]
    )
    NitrogenIncorporation.determine_optimal_nutrient_fraction = MagicMock(
        return_value=0.75
    )
    if gate:
        ...
        NitrogenIncorporation.determine_optimal_nutrient = MagicMock(return_value=268)
    NitrogenIncorporation.determine_potential_nutrient_uptake = MagicMock(
        return_value=123.1
    )
    ...
    NitrogenIncorporation.determine_nutrient_shape_parameters.assert_called_once_with(
        0.54, 0.99, 0.71, 0.68, 0.60
    )
    assert data.phosphorus_shapes == [1.2, 0.8]

    NitrogenIncorporation.determine_optimal_nutrient_fraction.assert_called_once_with(
        0.38, 0.71, 0.60, 1.2, 0.8
    )
    assert data.optimal_phosphorus_fraction == 0.75

    if gate:
        NitrogenIncorporation.determine_optimal_nutrient.assert_called_once_with(
            0.75, 122.8
        )
        ...
        assert data.optimal_phosphorus == 268
        NitrogenIncorporation.determine_potential_nutrient_uptake.assert_called_once_with(
            268, 0, 0.60, 999
        )
