...
def test_shift_phosphorus_time(mock_crop_data: CropData, old: float | None, new: float) -> None:
    """Ensure shift_phosphorus_time correctly copies current phosphorus value to previous_phosphorus."""
    mock_crop_data.phosphorus = new
    incorp = PhosphorusIncorporation(mock_crop_data, previous_phosphorus=old)
...
def test_phosphorus_determine_deepest_accessible_soil_layer(
    mock_crop_data: CropData, root_depth: float, depths: list[float], expect: list[float]
) -> None:
    ...
    incorp = PhosphorusIncorporation(mock_crop_data)
...
def test_access_layers(mock_crop_data: CropData, deepest: float, layers: list[float]) -> None:
...
def test_cd_phosphorus_from_soil_layers(
    mock_crop_data: CropData, uptakes: list[float], phosphates: list[float]
) -> None:
    """Check that layer_nitrates were correctly updated by extract_phosphorus_from_soil_layers."""
    nitrates_copy = phosphates.copy()
    incorp = PhosphorusIncorporation(mock_crop_data, actual_phosphorus_uptakes=uptakes)
...
def test_tally_total_phosphorus_uptake(mock_crop_data: CropData, uptakes: list[float]) -> None:
...
def test_extend_phosphate_uptakes_to_full_profile(mock_crop_data: CropData, layers: int) -> None:
    """Checks that the helper method extend_phosphate_uptakes_to_full_profile() does correctly updates the list to the
    right length."""
    mock_crop_data.inaccessible_soil_layers = layers
    incorp = PhosphorusIncorporation(mock_crop_data, actual_phosphorus_uptakes=[1.0, 2.0, 3.0])
...
def test_uptake_phosphorus(mock_crop_data: CropData, phosphates: list[float], depths: list[float]) -> None:
    ...
    incorp = PhosphorusIncorporation(mock_crop_data, potential_phosphorus_uptake=17.5, phosphorus_distro_param=0.32)
...
def test_incorporate_phosphorus(
    mock_crop_data: CropData, phosphates: list[float], depths: list[float], gate: bool
) -> None:
    ...
    incorp = PhosphorusIncorporation(mock_crop_data, previous_phosphorus=0)
    ...
    assert mock_crop_data.phosphorus == 99.3
