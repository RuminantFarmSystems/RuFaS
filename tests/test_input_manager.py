...
def test_dump_metadata_properties(mock_input_manager: InputManager) -> None:
    ...
    with patch.object(mock_input_manager, '_parse_metadata_properties', return_value=mock_records) as mock_parse, \
         patch('pandas.DataFrame.to_csv') as mock_to_csv, \
         patch('os.path.join', return_value=str(output_dir / "output.csv")) as mock_join, \
         patch('RUFAS.output_manager.OutputManager.generate_file_name', return_value="output.csv") as \
            mock_generate_file_name:
        ...
        mock_generate_file_name.assert_called_once_with(base_name=str(output_dir) + "InputManager_metadata_properties",
                                                        extension="csv")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        }, {'True': 2, 'False': 2}, 2, [{"mocked": "record"}, {"mocked": "record"}]),
        ({
            "level1": {
                "level2": {
                    "nestedProperty": {
        ...
        }, {'True': 2, 'False': 3}, 2, [{"mocked": "record"}])
    ])
def test_parse_metadata_properties(mock_input_manager: InputManager, nested_data: Dict[str, Any],
                                   expected_primitive_call_counts: Dict[str, int],
                                   expected_create_record_call_count: int, expected_results: List[Dict[str, str]]):
    ...
    with patch.object(mock_input_manager, '_check_property_type_primitive',
                      side_effect=side_effect_check_property_type_primitive) as mock_primitive, \
...
@pytest.mark.parametrize("property_dict, expected_result", [
...
def test_check_property_type_primitive(mock_input_manager: InputManager, property_dict: Dict[str, str],
                                       expected_result: bool):
...
@pytest.mark.parametrize("data_entry, name, expected_record", [
    ({
        "type": "string",
...
def test_create_record(mock_input_manager: InputManager, data_entry: Dict[str, str], name: str,
