...
def test_purchased_feed_emissions_estimator(mocker: MockerFixture) -> None:
    ...
    get_data.assert_called_once_with("config.FIPS_county_code")
    assert add_var.call_count == 2
    setup_emissions.assert_called_once()
    assert estimator.missing_feed_ids == []
...
def test_create_daily_purchased_feed_emissions_report(mocker: MockerFixture) -> None:
    ...
    mock_estimator.missing_feed_ids = ["2"]
    mock_estimator.feed_emissions = {"1": 0.15}
    ...
    assert mock_estimator.missing_feed_ids == ["2", "3"]
...
def test_setup_feed_emissions(mocker: MockerFixture, code: int, expected: dict[str, float]) -> None:
    ...
    mocker.patch("RUFAS.routines.animal.purchased_feed_emissions_estimator.im.get_data", return_value=mock_data)
    mocker.patch(
        "RUFAS.routines.animal.purchased_feed_emissions_estimator.PurchasedFeedEmissionsEstimator.__init__",
        return_value=None,
    )
    ...
    actual = mock_estimator._setup_feed_emissions()
