...
def test_calc_max_animal_spaces_per_pen(
    num_stalls: int, max_stocking_density: float, expected_max_animal_spaces: int
) -> None:
...
def test_create_additional_pens_for_potential_space_shortage(
    ...
    patch_for_calc_animal_space_shortage = mocker.patch.object(
        AnimalManager, "_calc_animal_space_shortage", return_value=space_shortage
    )
    num_stalls_per_pen = 5
    max_stocking_density_per_pen = 1.2
...
@pytest.mark.parametrize(
    "num_animals, max_spaces_in_pens, expected_allocation",
    [
...
def test_plan_animal_allocation(
    num_animals: int, max_spaces_in_pens: List[int], expected_allocation: List[int]
) -> None:
    """
...
def test_execute_allocation_plan_error_cases(
    allocation_plan: List[int], num_animals: int, num_pens: int, mocker: MockerFixture
) -> None:
    ...
    with pytest.raises(ValueError):
        AnimalManager.execute_allocation_plan(
            allocation_plan=allocation_plan, animals=mock_animals, animal_pens=mock_pens
        )
...
def test_allocate_animals_to_pens_helper(
    ...
    patch_for_calc_max_animal_spaces_per_pen = mocker.patch.object(
        AnimalManager, "_calc_max_animal_spaces_per_pen", return_value=max_animal_spaces
    )
    ...
    patch_for_plan_animal_allocation.assert_called_once_with(
        num_animals=num_animals, max_spaces_in_pens=[max_animal_spaces] * num_pens
    )
...
def test_allocate_animals_to_pens(mocker: MockerFixture) -> None:
    ...
    patch_for_allocate_animals_to_pens_helper = mocker.patch.object(
        AnimalManager, "_allocate_animals_to_pens_helper", return_value=None
    )
    ...
    patch_for_sort_animals_before_allocation = mocker.patch.object(
        AnimalManager, "_sort_animals_before_allocation", return_value=None
    )
