...
def pens_with_mock_animals() -> List[MagicMock]:
    ...
    mock_animals_in_pens = [
        {"id": 0, "animals_in_pen": {0: mock_animal_1, 1: mock_animal_2}},
        {"id": 1, "animals_in_pen": {0: mock_animal_3, 1: mock_animal_4}},
    ]
...
def mock_animals_small() -> List[MagicMock]:
    animal_attribute_dicts = [
        {"body_weight": 5.0, "p_animal": 7.0},
        {"body_weight": 1.0, "p_animal": 8.0},
        {"body_weight": 2.0, "p_animal": 1.0},
    ]
...
@pytest.fixture
def animal_manager(
    input_manager: InputManager, mock_im_pool: Dict[str, Dict[str, Any]]
) -> AnimalManager:
    init_pens_patch = patch(
        "RUFAS.routines.animal.animal_manager.AnimalManager.init_pens"
    )
    init_animals_patch = patch(
        "RUFAS.routines.animal.animal_manager.AnimalManager.init_animals"
    )
    init_nutrient_rqmts_patch = patch(
        "RUFAS.routines.animal.animal_manager.AnimalManager.init_nutrient_rqmts"
    )
    # init_allocate_all_pens_patch = patch('RUFAS.routines.animal.animal_manager.AnimalManager.allocate_all_pens')
    allocate_animals_to_pens_patch = patch(
        "RUFAS.routines.animal.animal_manager.AnimalManager.allocate_animals_to_pens"
    )
    ...
    animal_manager = AnimalManager(
        data, config, feed, weather, time, feed_emissions_manager
    )
...
@pytest.fixture
def animal_manager_with_mock_pens(
    animal_manager: AnimalManager, mock_pens: List[MagicMock]
) -> AnimalManager:
...
@pytest.fixture
def cowlist():
    cowlist = [MagicMock(), MagicMock(), MagicMock()]
    return cowlist
...
def test_init_animals(animal_manager: AnimalManager, mocker: MockerFixture):
    ...
    mocker.patch(
        "RUFAS.routines.animal.life_cycle.life_cycle.LifeCycleManager.initialize_herd",
        return_value=[None, None, None, None, None],
    )
    mocker.patch(
        "RUFAS.routines.animal.animal_manager.AnimalManager._print_animal_num_warnings"
    )
...
def test_print_animal_num_warnings(animal_manager: AnimalManager):
    ...
    with patch.object(om, "add_log") as add_log, patch.object(
        om, "add_warning"
    ) as add_warning:
        animal_keys = {
            "calf_num",
        ...
        animal_manager.simulate_animals = True
        animal_manager._print_animal_num_warnings(herd_data)
        add_log.assert_called_once_with(
            "simulate_animals_flag", "simulate_animals is true", expected_info_map
        )

        # test for warnings for every animal key and simulate_animals = False
        animal_manager.simulate_animals = False
...
def test_init_nutrient_rqmts(mocker: MockerFixture) -> None:
    ...
    animal.set_nutrient_rqmts.assert_has_calls(
        [call(None)] + ([call(27, None)] * 3), any_order=True
    )
...
def test_avg_pen_dist(animal_manager_with_mock_pens: AnimalManager) -> None:
    """Unit test for function avg_pen_dist in file routines/animal/animal_manager.py"""

    actual = animal_manager_with_mock_pens.avg_pen_dist()
    expected = (12 / 4, 24 / 4)
...
def pens_test_data_dict() -> List[dict[Any]]:
    ...
    Returns:
        ...
        AnimalManager test cases. The dictionaries include data needed to set up AnimalManager
    ...
    return [
        {
            "pen_data": {
                "pen0": {
                    ...
                    "post_removal_stocking_density": 0.50,
                    "num_stalls": 40,
                    "ration": {
                        "dummy_feed1": 205.0,
                        "dummy_feed2": 0.0,
                ...
                "pen1": {
                    ...
                    "cow_type_to_id_map": {
                        "Calf": [],
                        "HeiferI": [100439, 106977, 111123, 111262, 111527, 112516],
                    ...
                    "post_removal_stocking_density": 0.25,
                    "num_stalls": 36,
                ...
                "pen2": {
                    ...
                    "cow_type_to_id_map": {
                        ...
                        "Dry_Cow": [187817, 189760, 189801, 198331, 199999],
                        "Lac_Cow": [],
                    },
                    "pen_animal_combination": AnimalCombination.CLOSE_UP,
                    "post_removal_stocking_density": 0.10,
                    "num_stalls": 150,
                    "ration": {
                        "dummy_feed1": 10.0,
                        "dummy_feed2": 35.0,
                ...
                "pen3": {
                    ...
                    "post_removal_stocking_density": 0.20,
                    "num_stalls": 35,
                    "ration": {"dummy_feed1": 100.0, "status": "dummy_val"},
                },
            },
            "animals_to_be_removed": {115078, 169062, 112516, 189801, 198542},
        ...
        },
        {
            "pen_data": {
                ...
                "pen1": {
                    ...
                    "cow_type_to_id_map": {
                        "Calf": [],
                        "HeiferI": [105602, 106140, 111756, 111796, 113310, 116100],
                        "HeiferII": [
                            117695,
                            118217,
                            118719,
                    ...
                    "pen_animal_combination": AnimalCombination.GROWING,
                    "post_removal_stocking_density": 0.14,
                    "num_stalls": 150,
                    "ration": {
                        "objective": "dummy_val",
                        "dummy_feed2": 4.0,
                ...
                "pen2": {
                    ...
                    "pen_animal_combination": AnimalCombination.CLOSE_UP,
                    "post_removal_stocking_density": 0.35,
                    "num_stalls": 60,
                    "ration": {
                        "dummy_feed1": 44.4,
                        "dummy_feed2": 21.2,
        ...
        },
        {
            "pen_data": {
                ...
                "pen1": {
                    ...
                    "cow_type_to_id_map": {
                        "Calf": [],
                        "HeiferI": [112121, 117953, 138864, 144952],
                ...
                "pen2": {
                    ...
                    "cow_type_to_id_map": {
                        "Calf": [],
                        "HeiferI": [114067, 119819, 166593, 168618, 183594],
                    ...
                    "pen_animal_combination": AnimalCombination.GROWING,
                    "post_removal_stocking_density": 0.11,
                    "num_stalls": 100,
                    "ration": {
                        "dummy_feed1": 62.9,
                        "dummy_feed2": 123.0,
                ...
                "pen3": {
                    ...
                    "cow_type_to_id_map": {
                        ...
                        "Dry_Cow": [169688, 174053, 175698, 188017],
                        "Lac_Cow": [],
                    },
                    "pen_animal_combination": AnimalCombination.CLOSE_UP,
                    "post_removal_stocking_density": 0.5,
                    "num_stalls": 20,
                    "ration": {
                        "status": "dummy_value",
                        "objective": "dummy_value",
                        "dummy_feed1": 8.8,
                        "dummy_feed2": 72.6,
                        "dummy_feed3": 0.0,
                        "dummy_feed4": 50.0,
                ...
                "pen4": {
                    ...
                    "pen_animal_combination": AnimalCombination.LAC_COW,
                    "post_removal_stocking_density": 1.0,
                    "num_stalls": 10,
                    "ration": {
                        "dummy_feed1": 35.0,
                        "dummy_feed2": 2.0,
...
def setup_dummy_animal(animal_id: int, animal_type: str) -> AnimalBase:
    ...
    args_dict = {
        "breed": "dummy_breed",
        "birth_date": "dummy_birth_date",
        "days_born": "dummy_days_born",
        "milking": "dummy_milking_value",
        "id": animal_id,
    }
...
def setup_dummy_pen(pen_id: int, num_stalls: int, animal_list: List[AnimalBase]) -> Pen:
    ...
    dummy_pen_info_dict = {
        ...
        "bedding_type": "dummy_bedding_type",
        "pen_type": "dummy_pen_type",
        "manure_handling": "dummy_manure_handling",
        "manure_separator": "dummy_manure_separator",
        "manure_separator_after_digestion": "dummy_manure_separator_after_digestion",
        "manure_storage": "dummy_manure_storage",
    ...
    dummy_pen = Pen(
        dummy_pen_info_dict["id"],
        dummy_pen_info_dict["pen_name"],
        ...
        dummy_pen_info_dict["housing_type"],
        dummy_pen_info_dict["bedding_type"],
        dummy_pen_info_dict["pen_type"],
        dummy_pen_info_dict["manure_handling"],
        dummy_pen_info_dict["manure_separator"],
        dummy_pen_info_dict["manure_separator_after_digestion"],
        dummy_pen_info_dict["manure_storage"],
        dummy_pen_info_dict["animal_combination"],
...
def setup_dummy_animal_manager_with_pens(
    animal_manager: AnimalManager, info_dict: dict[Any], append_removals: bool = False
) -> dict[Any]:
    """
    ...
    of the AnimalManager object's variables. This is done by creating AnimalBase objects and placing them into
    Pen objects' 'animals_in_pen' variables, as well as configuring the Pen's ration variable before then
    appending individual Pen objects to the AnimalManager object's 'all_pens' variable.


    Args:
        animal_manager: An AnimalManager object
    ...
    Returns:
        A dictionary containing the edited AnimalManager object via the 'animal_manager_object'
    ...
    for pen_num, pen_dict in info_dict["pen_data"].items():
        ...
        for animal_type, animal_ids in pen_dict["cow_type_to_id_map"].items():
            for animal_id in animal_ids:
                ...
                animal_manager.animal_to_pen_id_map[dummy_animal.id] = pen_dict[
                    "pen_id"
                ]
                if (
                    append_removals
                    and dummy_animal.id in info_dict["animals_to_be_removed"]
                ):
                    animals_removed.append(dummy_animal)

        dummy_pen = setup_dummy_pen(
            pen_dict["pen_id"], pen_dict["num_stalls"], animal_list
        )
        ...
        animal_manager.pens_by_animal_combination[
            pen_dict["pen_animal_combination"]
        ].append(dummy_pen)

        pen_list.append(dummy_pen)

    animal_manager.all_pens = pen_list

    return {
        "animal_manager_object": animal_manager,
        "animals_removed_list": animals_removed,
    }


@pytest.mark.parametrize("info_dict", pens_test_data_dict())
def test_remove_animals_from_herd(
    info_dict: dict[Any], animal_manager: AnimalManager
) -> None:
    """Unit test for function remove_animals_from_herd in file routines/animal/animal_manager.py"""

    dummy_animal_manager = setup_dummy_animal_manager_with_pens(
        animal_manager, info_dict, True
    )["animal_manager_object"]
    animals_removed = setup_dummy_animal_manager_with_pens(
        animal_manager, info_dict, True
    ...
    assert (
        dummy_animal_manager.animal_to_pen_id_map
        == info_dict["animal_to_pen_id_map_after_removals"]
    )
    for idx, pen_dict in enumerate(info_dict["pen_data"].values()):
        assert (
            dummy_animal_manager.all_pens[idx].stocking_density
            == pen_dict["post_removal_stocking_density"]
        )
        assert (
            set(dummy_animal_manager.all_pens[idx].animals_in_pen)
            & set(animals_removed)
            == set()
        )
...
def test_track_former_pen_population(
    ...
    dummy_animal_manager = setup_dummy_animal_manager_with_pens(
        animal_manager, info_dict, False
    )["animal_manager_object"]
...
def test_daily_update_id_map(
    info_dict: dict[Any], animal_manager: AnimalManager, mocker: MockerFixture
):
    ...
    dummy_object_and_removals = setup_dummy_animal_manager_with_pens(
        animal_manager, info_dict, True
    )
    ...
    with patch("RUFAS.routines.animal.pen.Pen.set_up_new_animal") as set_up_new_animal:
        ...
        assert set_up_new_animal.call_count == len(info_dict["new_calf_dict"]) + len(
            info_dict["new_cow_dict"]
        )

    assert (
        dummy_animal_manager.animal_to_pen_id_map
        == info_dict["animal_to_pen_id_map_after_daily_update"]
    )
...
def test_group_pens_by_animal_combination(mocker: MockerFixture) -> None:
    ...
    expected = {"combo_0": [mock_pen_0], "combo_1": [mock_pen_1]}
    # Act
    actual = mock_animal_manager._group_pens_by_animal_combination(
...
def test_calc_max_animal_spaces_per_pen(
    num_stalls: float, max_stocking_density: float, expected: float
) -> None:
    ...
    if expected is ValueError:
        ...
        actual = AnimalManager._calc_max_animal_spaces_per_pen(
            num_stalls, max_stocking_density
        )
        assert actual == expected
...
def test_clear_pens(animal_manager_with_mock_pens: AnimalManager) -> None:
    """Unit test for function clear_pens in file routines/animal/animal_manager.py"""
    animal_manager_with_mock_pens.clear_pens()
...
def test_record_pen_history(mocker: MockerFixture) -> None:
    ...
    mock_animal_manager.gather_pen_history.assert_has_calls(
        [
            call(mock_animal_manager.calves),
...
def test_reset_milk_production_reduction(pens_with_mock_animals) -> None:
    """Unit test for function reset_milk_production_reduction in file routines/animal/animal_manager.py"""
...
def test_get_animals_snapshot(mocker: MockerFixture):
    """
    Unit test for function _get_animals_snapshot() in file animal_manager.py
    ...
    old_animal_grouping_scenario = AnimalManager.ANIMAL_GROUPING_SCENARIO
    ...
    mock_animal_grouping_scenario.find_animal_combination = mocker.MagicMock(
        side_effect=lambda animal: animal_combination_by_id[animal.id]
    )

    animal_manager.calves = mock_calves
    animal_manager.heiferIs = mock_heifer_Is
    animal_manager.heiferIIs = mock_heifer_IIs
    animal_manager.heiferIIIs = mock_heifer_IIIs
    animal_manager.cows = mock_cows
    ...
    # Act
    actual_snapshot = animal_manager._get_animals_snapshot()

    # Assert
    assert actual_snapshot == expected_snapshot
    assert (
        mock_animal_grouping_scenario.find_animal_combination.call_count
...
def test_handle_removed_animals_after_update(mocker: MockerFixture):
    """
    Unit test for the function _handle_removed_animals_after_update() in file animal_manager.py
    ...
    removed_animals = set(
        mock_calves_at_even_indices
        + mock_heifer_Is_at_even_indices
        + mock_heifer_IIs_at_even_indices
        + mock_heifer_IIIs_at_even_indices
        + mock_cows_at_even_indices
    )
    patch_remove_animal_from_pen_and_id_map = mocker.patch.object(
        animal_manager, "_remove_animal_from_pen_and_id_map"
    )

    # Act
    animal_manager._handle_removed_animals_after_update(
...
def test_handle_animals_with_unchanged_class_and_changed_combination(
    mocker: MockerFixture,
):
    """
    ...
    animals = (
        mock_calves + mock_heifer_Is + mock_heifer_IIs + mock_heifer_IIIs + mock_cows
    )
    ...
    patch_remove_animal = mocker.patch.object(
        animal_manager, "_remove_animal_from_pen_and_id_map"
    )
    patch_add_animal = mocker.patch.object(
        animal_manager, "_add_animal_to_pen_and_id_map"
    )

    # Act
    animal_manager._handle_animals_with_unchanged_class_and_changed_combination(
        animals_snapshot_before_update,
        animals_snapshot_after_update,
        mock_feed,
        mock_temp,
...
def test_handle_graduated_animals(mocker: MockerFixture):
    """
    Unit test for the function _handle_graduated_animals() in file animal_manager.py
    ...
    graduated_heifer_IIIs = random.sample(
        mock_heifer_IIs, num_animals_of_each_type // 3
    )
    ...
    animals_snapshot_after_update = {
        ...
        "heiferIs": set(mock_heifer_Is).union(set(graduated_heifer_Is))
        - set(graduated_heifer_IIs),
        "heiferIIs": set(mock_heifer_IIs).union(set(graduated_heifer_IIs))
        - set(graduated_heifer_IIIs),
        "heiferIIIs": set(mock_heifer_IIIs).union(set(graduated_heifer_IIIs))
        - set(graduated_cows),
    ...
    # Act
    animal_manager._handle_graduated_animals(
        animals_snapshot_before_update,
        animals_snapshot_after_update,
        mock_feed,
        mock_temp,
    ...
    # Assert
    all_graduated_animals = (
        graduated_heifer_Is
        + graduated_heifer_IIs
        + graduated_heifer_IIIs
        + graduated_cows
    )
...
def test_handle_newly_added_animals(mocker: MockerFixture):
    """
    Unit test for the function _handle_newly_added_animals() in file animal_manager.py
    ...
    mock_animals_by_type_property = mocker.patch.object(
        AnimalManager, "animals_by_type", new_callable=mocker.PropertyMock
    )
    mock_animals_by_type_property.return_value = mock_animals_by_type

    # Act
    animal_manager._handle_newly_added_animals(new_mock_animals, mock_feed, mock_temp)

    # Assert
    assert patch_add_animal.call_count == num_new_animals
    for animal in new_mock_animals:
        patch_add_animal.assert_any_call(animal, mock_feed, mock_temp)
        assert animal in animal_manager.animals_by_type[MockAnimal]


def test_remove_animal_from_pen_and_id_map(mocker: MockerFixture):
    """
    Unit test for the function _remove_animal_from_pen_and_id_map() in file animal_manager.py
    ...
    animal_manager.animal_to_pen_id_map = mock_animal_to_pen_id_map
    animal_manager.all_pens = mock_all_pens

    # Act
    animal_manager._remove_animal_from_pen_and_id_map(mock_animal)
...
def test_add_animal_to_pen_and_id_map(mocker: MockerFixture):
    ...
    mock_animal_grouping_scenario = mocker.MagicMock()
    mock_animal_grouping_scenario.find_animal_combination.return_value = (
        mock_animal_combination
    )

    mock_pens_by_animal_combination = {
        mock_animal_combination: [mock_pen_1, mock_pen_2]
    }
    ...
    # Act
    animal_manager._add_animal_to_pen_and_id_map(mock_animal, mock_feed, mock_temp)

    # Assert
    mock_pen_1.add_animal.assert_not_called()
    mock_pen_2.add_animal.assert_called_once_with(
        mock_animal, mock_animal_grouping_scenario, mock_feed, mock_temp, 0.0
    )
...
def test_daily_updates(is_end_ration_interval: bool, mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "RUFAS.routines.animal.animal_module_reporter.AnimalModuleReporter.report_daily_feed_emissions",
        return_value=None,
    )
    ...
    mock_animal_manager = AnimalManager(
        ...
        time=mock_time,
        feed_emissions_manager=mock_feed_emissions_manager,
    )
    ...
    mock_animal_manager.methane_mitigation_method = (
    ...
    patch_for_get_animals_snapshot = mocker.patch.object(
        AnimalManager,
        "_get_animals_snapshot",
        side_effect=[
            mock_animal_snapshot_before_update,
    ...
    mock_life_cycle_manager.daily_update.return_value = [
        mock_animals_added,
        mock_animals_removed,
        mock_calves_born,
        [],
    ]
    ...
    patch_for_handle_unchanged_animals = mocker.patch.object(
        AnimalManager,
        "_handle_animals_with_unchanged_class_and_changed_combination",
        return_value=None,
    )
    ...
    patch_for_determine_classes_in_pen.assert_has_calls(
        [mocker.call(mock_pen) for mock_pen in mock_all_pens]
    )
    ...
    assert (
        mock_animal_manager.life_cycle_manager.daily_milk_production == sum_daily_milk
    )
...
def test_collect_manure_excretions_output_data(mocker: MockerFixture) -> None:
    ...
    # act
    AnimalManager.collect_manure_excretions_output_data(
        animal_manager, pen, feed, manure_excretions_output_data
    )
...
@pytest.mark.parametrize(
    "num_animals, max_spaces, expected",
    [
        (100, 99, 1),
        (99, 100, -1),
        (100, 100, 0),
...
def test_calc_animal_space_shortage(
    num_animals: int, max_spaces: int, expected: int, mocker: MockerFixture
) -> None:
    ...
    mock_animal_manager = AnimalManager()
    mock_animal_manager._calc_max_animal_spaces_per_pen = MagicMock(
        return_value=max_spaces
    )
    ...
    actual = mock_animal_manager._calc_animal_space_shortage(
        num_animals, mock_animal_manager.all_pens
    )
    assert actual == expected
