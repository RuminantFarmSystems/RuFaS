...
@pytest.mark.parametrize(
    ...
    "estrus_cycle, days_born, stdi, avg_estrus_cycle, std_estrus_cycle, max_cycle_length,"
    "expected_estrus_day",
    [
        # Normal estrus cycle
        (100, 120, "Test Estrus", 20, 100, 2.0, 20, 5.0, math.inf, 120),
        # Negative estrus cycle
        (100, 120, "Negative Cycle", -10, 100, 2.0, 20, 5.0, math.inf, 110),
        # Very high estrus cycle within max limit
        (100, 120, "High Cycle", 50, 100, 2.0, 20, 5.0, 60, 150),
        # Zero estrus cycle
        (100, 120, "Zero Cycle", 0, 100, 2.0, 20, 5.0, math.inf, 100),
        # Estrus cycle exceeding max limit
        (100, 120, "Exceed Max", 50, 100, 2.0, 20, 5.0, 40, 139),
        # Different stdi values
        (100, 120, "Different STDI", 20, 100, 1.0, 20, 5.0, math.inf, 120),
        (100, 120, "Different STDI", 20, 100, 3.0, 20, 5.0, math.inf, 120),
        # Different average and standard deviation of estrus cycle
        (100, 120, "Different Avg and Std", 15, 100, 2.0, 15, 4.0, math.inf, 115),
        (100, 120, "Different Avg and Std", 25, 100, 2.0, 25, 6.0, math.inf, 125),
    ],
)
...
def test_compare_randomized_rate_less_than(
    ...
    # Arrange
    mocker.patch(
        "RUFAS.routines.animal.life_cycle.heiferII.random",
        return_value=mocked_random_value,
    )
...
def test_get_avg_estrus_cycle(
    mocker: MockerFixture, avg_estrus_cycle_config: int, expected_avg_estrus_cycle: int
) -> None:
    """
...
def test_get_std_estrus_cycle(
    mocker: MockerFixture,
    std_estrus_cycle_config: float,
    expected_std_estrus_cycle: float,
) -> None:
    """
...
def test_get_avg_estrus_cycle_after_pgf(
    mocker: MockerFixture,
    avg_estrus_cycle_after_pgf_config: int,
    expected_avg_estrus_cycle_after_pgf: int,
) -> None:
    """
...
def test_get_std_estrus_cycle_after_pgf(
    mocker: MockerFixture,
    std_estrus_cycle_after_pgf_config: float,
    expected_std_estrus_cycle_after_pgf: float,
) -> None:
    """
...
def test_get_general_estrus_detection_rate(
    mocker: MockerFixture,
    mocked_estrus_detection_rate: float,
    expected_estrus_detection_rate: float,
) -> None:
    """
    ...
    patch_for_get_repro_data = mocker.patch.object(
        HeiferII,
        "get_user_defined_repro_data",
...
def test_get_user_defined_synch_ed_estrus_detection_rate(
    mocker: MockerFixture,
    mocked_estrus_detection_rate: float,
    expected_estrus_detection_rate: float,
) -> None:
    ...
    patch_for_get_repro_sub_properties = mocker.patch.object(
        HeiferII,
        "get_user_defined_repro_sub_properties",
...
def test_get_default_synch_ed_estrus_detection_rate(
    mocker: MockerFixture,
    mocked_estrus_detection_rate: float,
    expected_estrus_detection_rate: float,
) -> None:
...
def test_get_user_defined_or_default_synch_ed_estrus_detection_rate(
    ...
    mocker.patch.object(
        ...
        return_value=user_defined_rate,
    )
    mocker.patch.object(
        HeiferII,
        "_get_default_synch_ed_estrus_detection_rate",
        return_value=default_rate,
    )
...
def test_get_general_conception_rate(
    mocker: MockerFixture,
    mocked_conception_rate: float,
    ...
    heiferII = HeiferII(mocker.MagicMock())
    patch_for_get_repro_data = mocker.patch.object(
        HeiferII, "get_user_defined_repro_data", return_value=mocked_conception_rate
    )
...
def test_get_user_defined_tai_conception_rate(
    mocker: MockerFixture,
    mocked_conception_rate: float,
    expected_conception_rate: float,
) -> None:
...
def test_get_default_TAI_conception_rate(
    mocker: MockerFixture,
    mocked_conception_rate: float,
    expected_conception_rate: float,
) -> None:
...
def test_get_user_defined_or_default_TAI_conception_rate(
    mocker: MockerFixture,
    repro_protocol: str,
    user_defined_rate: float,
    default_rate: float,
...
def test_detect_estrus(
    mocker: MockerFixture,
    initial_estrus_count: int,
    mocked_detection_outcome: bool,
    detection_rate: float,
    expected_result: bool,
    expected_estrus_count: int,
) -> None:
    """
    ...
    patch_for_compare_randomized_rate = mocker.patch.object(
        HeiferII,
        "_compare_randomized_rate_less_than",
...
def test_execute_ed_protocol(
    mocker: MockerFixture,
    is_pregnant: bool,
    initial_ed_days: int,
    days_born: int,
    breeding_start_day: int,
    estrus_day: int,
    sim_day: int,
    should_simulate_estrus: bool,
    should_handle_estrus_detection: bool,
) -> None:
    """
    ...
    if should_simulate_estrus:
        patch_for_simulate_estrus.assert_called_once_with(
            ...
            mock_avg_estrus_cycle,
            mock_std_estrus_cycle,
        )
    else:
        patch_for_simulate_estrus.assert_not_called()

    if should_handle_estrus_detection:
        patch_for_handle_generic_estrus_detection.assert_called_once_with(sim_day)
    else:
        patch_for_handle_generic_estrus_detection.assert_not_called()
...
def test_deliver_hormones(
    ...
    initial_PGF_count: int,
    expected_GnRH_count: int,
    expected_PGF_count: int,
    raises_error: bool,
) -> None:
    """
...
def test_execute_hormone_delivery_schedule(
    mocker: MockerFixture,
    days_born: int,
    sim_day: int,
...
def test_get_breeding_start_day(
    mocker: MockerFixture,
    breeding_start_day_config: int,
    expected_breeding_start_day: int,
) -> None:
    """
...
def test_get_user_defined_repro_data(
    mocker: MockerFixture, attribute: str, heifer_data: dict, expected_value: Any
) -> None:
    """
...
def test_get_user_defined_repro_protocol(
    mocker: MockerFixture, mocked_repro_protocol: str, expected_repro_protocol: str
) -> None:
    """
...
def test_get_user_defined_or_default_repro_sub_protocol(
    mocker: MockerFixture,
    current_program: str,
    user_defined_program: str,
    user_defined_sub_protocol: str,
    default_sub_protocol: str,
    expected_sub_protocol: str,
) -> None:
    # Arrange
    mocker.patch.object(HeiferII, "__init__", return_value=None)
    heifer = HeiferII(mocker.MagicMock())
...
def test_execute_tai_protocol(
    mocker: MockerFixture,
    days_born: int,
    sim_day: int,
    breeding_start_day: int,
    hormone_schedule: dict,
    hormone_schedule_exists: bool,
    ...
    patch_for_get_repro_sub_protocol = mocker.patch.object(
        HeiferII,
        "_get_user_defined_or_default_repro_sub_protocol",
...
def test_execute_synch_ed_protocol(
    mocker: MockerFixture,
    days_born: int,
    sim_day: int,
    breeding_start_day: int,
...
def test_set_up_hormone_schedule(
    ...
    patch_for_get_adjusted_schedule = mocker.patch.object(
        HormoneDeliverySchedule, "get_adjusted_schedule", return_value=adjusted_schedule
    )
...
def test_handle_synch_ed_hormone_delivery_and_set_estrus_day(
    ...
    days_born: int,
    sim_day: int,
    initial_hormone_schedule: dict,
    final_hormone_schedule: dict,
) -> None:
    """
    ...
    mock_avg_estrus_cycle_after_pgf = mocker.MagicMock()
    mock_std_estrus_cycle_after_pgf = mocker.MagicMock()
    mocker.patch.object(
        HeiferII,
        "get_avg_estrus_cycle_after_pgf",
        return_value=mock_avg_estrus_cycle_after_pgf,
    )
    mocker.patch.object(
        HeiferII,
    ...
    if len(initial_hormone_schedule) == 1 and len(final_hormone_schedule) == 0:
        patch_for_simulate_estrus.assert_called_once_with(
            ...
            sim_day,
            const.ESTRUS_DAY_SCHEDULED_NOTE,
            mock_avg_estrus_cycle_after_pgf,
            mock_std_estrus_cycle_after_pgf,
            max_cycle_length=14,
        )
    else:
        patch_for_simulate_estrus.assert_not_called()
...
def test_handle_synch_ed_estrus_detection(
    ...
    specific_estrus_detection_rate: float,
    external_specific_conception_rate: float,
) -> None:
    """
    ...
    mocker.patch.object(
        HeiferII,
        "get_user_defined_tai_conception_rate",
        return_value=external_specific_conception_rate,
    )
    patch_for_handle_estrus_not_detected = mocker.patch.object(
        heifer, "_handle_estrus_not_detected_in_synch_ed", return_value=None
    )
...
def test_handle_estrus_not_detected_in_synch_ed(
    ...
    TAI_conception_rate: float,
    expected_ai_day: int,
    internal_fallback_protocol: dict,
) -> None:
    """
...
def test_set_repro_program(
    ...
    days_born: int,
    sim_day: int,
    initial_repro_program: str,
    new_repro_program: str,
    is_valid: bool,
...
def test_perform_ai(
    ...
    conception_successful: bool,
    initial_semen_num: int,
    initial_AI_times: int,
    expected_semen_num: int,
    expected_AI_times: int,
) -> None:
    """
...
def test_handle_successful_conception(mocker: MockerFixture, days_born: int, sim_day: int) -> None:
    ...
    patch_for_log_event = mocker.patch.object(heifer, "log_event", return_value=None)
    patch_for_initialize_pregnancy_parameters = mocker.patch.object(
        heifer, "_initialize_pregnancy_parameters", return_value=None
    )
...
def test_calculate_calf_birth_weight(
    ...
    # Assert
    patch_for_truncnorm_rvs.assert_called_once_with(-stdi, stdi, avg_weight, std_weight)
    assert result == rvs_return
...
def test_preg_update(
    ...
    # Assert
    if days_born in [
        ai_day + preg_check_day_1,
        ai_day + preg_check_day_2,
        ai_day + preg_check_day_3,
    ]:
...
def test_handle_preg_check(
    mocker: MockerFixture,
    days_born: int,
    initial_preg_diagnoses: int,
    is_pregnant: bool,
    loss_rate: float,
    compare_randomized_rate_result: bool,
    has_on_not_preg: bool,
    sim_day: int,
    expected_preg_diagnoses: int,
) -> None:
    """
...
def test_terminate_pregnancy(
    ...
    sim_day: int,
    preg_loss_const: str,
    initial_days_in_preg: int,
    initial_body_weight: float,
    initial_conceptus_weight: float,
) -> None:
    """
