...
@pytest.mark.parametrize(
    "animal_category, protocol_name, expected",
    [
...
def test_get_schedule(animal_category: str, protocol_name: str, expected: dict | None) -> None:
    ...
    assert HormoneDeliverySchedule.get_schedule(animal_category, protocol_name) == expected  # type: ignore


@pytest.mark.parametrize(
    "animal_category, protocol_name, start_day, expected",
    [
...
def test_heifer_synch_ed_sub_protocols_when_estrus_not_detected(
    sub_protocol: str, expected_result: dict[str, Any]
) -> None:
    """
