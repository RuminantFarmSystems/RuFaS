...
@pytest.mark.parametrize(
    "file_contents, expected_exception",
    [
        (
        ...
        (["A.x"], TypeError),
        ({"name": "e2e_json_feed_filter", "filters": ["A.*"], "expected_results_last_updated": ""}, KeyError),
    ],
)
def test_validate_comparison_configuration_invalid_expected_results_file(
    mocker: MockerFixture, tmp_path: Path, file_contents: Any, expected_exception: type[Exception]
) -> None:
...
def test_validate_comparison_configuration_expected_results_outside_filters_directory(
    ...
    """Tests that an expected results file outside the task's filters directory fails the validation."""
    _, add_error = patch_output_manager(mocker)
    path_sets = make_validation_path_sets(tmp_path)
    ...
    with pytest.raises(ValueError, match="filters directory"):
...
@pytest.mark.parametrize(
    "mistake, expected_exception, expect_error_logged",
    [
        ("missing_expected_results_file", FileNotFoundError, False),
        ("expected_results_file_missing_keys", ValueError, True),
        ("actual_results_path_mismatch", ValueError, True),
    ],
)
def test_validate_update_configuration_failures(
    mocker: MockerFixture, tmp_path: Path, mistake: str, expected_exception: type[Exception], expect_error_logged: bool
) -> None:
    ...
    with pytest.raises(expected_exception):
        E2ETestResultsHandler.validate_update_configuration(VALIDATION_OUTPUT_PREFIX, tmp_path)
    assert add_error.call_count == (1 if expect_error_logged else 0)
