...
@pytest.mark.parametrize(
    ...
    [
        ("test1", 16.4, {"test1": 16.4}),
        ("test2", 924.6, {"test1": 13, "test2": 924.6})
    ]
)
...
def test_target_and_save(mocker: MockerFixture) -> None:
    ...
    v._target_and_save({"variables": {"a": "test.address.1", "b": "test.address.2"},
                        "constants": {"c": "value"}})
...
def test_check_target_and_save_block_message_contains_all_invalid_keys() -> None:
    ...
    assert all(
        any(f"Unsupported keys {k} provided." in e["message"] for e in cv._event_logs)
        for k in ("a", "b", "c")
    )