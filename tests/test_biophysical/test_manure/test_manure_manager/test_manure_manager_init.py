...
def test_init(
    ...
    mock_get_data = mocker.patch(
        "RUFAS.biophysical.manure.manure_manager.InputManager.get_data",
        return_value=manure_management_input_json
    )
...
def test_check_for_duplicate_processor_names(
        manure_manager: ManureManager,
        all_names: list[str],
        expected_duplicate_names: list[str],
        mocker: MockerFixture
) -> None:
...
def test_validate_and_parse_processor_connections(
    ...
    mock_check_for_unknown_processor_names = mocker.patch.object(
        manure_manager, "_check_for_unknown_processor_names")
...
def test_create_all_processors(
    ...
    mock_slurry_storage_underfloor_init = mocker.patch(
        "RUFAS.biophysical.manure.storage.slurry_storage_underfloor.SlurryStorageUnderfloor.__init__",
        return_value=None
    )
...
def test_populate_adjacency_matrix(
    ...
    manure_manager._all_separators = {
        name: MagicMock(auto_spec=Separator) for name in expected_all_separator_names
    }
    ...
    mock_populate_destination_proportions.assert_has_calls(
        [
            call(
                expected_processor_connections_by_name[origin_name]["destinations"],
                origin_name
