...
def test_calc_pen_requirements_mean(animal_requirements: AnimalRequirements) -> None:
    ...
    essential_amino_acid_values: list[EssentialAminoAcidRequirements] = [
        EssentialAminoAcidRequirements(
            histidine=1.0, isoleucine=1.5, leucine=2.0, lysine=2.5, methionine=0.8,
            phenylalanine=1.2, threonine=1.3, thryptophan=0.5, valine=1.4
        ),
        EssentialAminoAcidRequirements(
            histidine=1.1, isoleucine=1.6, leucine=2.1, lysine=2.6, methionine=0.9,
            phenylalanine=1.3, threonine=1.4, thryptophan=0.6, valine=1.5
        ),
        EssentialAminoAcidRequirements(
            histidine=1.2, isoleucine=1.7, leucine=2.2, lysine=2.7, methionine=1.0,
            phenylalanine=1.4, threonine=1.5, thryptophan=0.7, valine=1.6
        ),
    ]
    ...
    assert animal_requirements.avg_essential_amino_acid_requirement.histidine == \
        np.mean([eaa.histidine for eaa in essential_amino_acid_values])
    assert animal_requirements.avg_essential_amino_acid_requirement.isoleucine == \
        np.mean([eaa.isoleucine for eaa in essential_amino_acid_values])
    assert animal_requirements.avg_essential_amino_acid_requirement.leucine == \
        np.mean([eaa.leucine for eaa in essential_amino_acid_values])
    assert animal_requirements.avg_essential_amino_acid_requirement.lysine == \
        np.mean([eaa.lysine for eaa in essential_amino_acid_values])
    assert animal_requirements.avg_essential_amino_acid_requirement.methionine == \
        np.mean([eaa.methionine for eaa in essential_amino_acid_values])
    assert animal_requirements.avg_essential_amino_acid_requirement.phenylalanine == \
        np.mean([eaa.phenylalanine for eaa in essential_amino_acid_values])
    assert animal_requirements.avg_essential_amino_acid_requirement.threonine == \
        np.mean([eaa.threonine for eaa in essential_amino_acid_values])
    assert animal_requirements.avg_essential_amino_acid_requirement.thryptophan == \
        np.mean([eaa.thryptophan for eaa in essential_amino_acid_values])
    assert animal_requirements.avg_essential_amino_acid_requirement.valine == \
        np.mean([eaa.valine for eaa in essential_amino_acid_values])
...
@pytest.mark.parametrize("recalc, expected_method", [
    (True, "recalculate_requirements"),
    (False, "use_existing_requirements")
])
def test_set_requirements(
    ...
    mock_pen.set_milk_avgs.assert_called_once_with(
        animal_requirements.avg_milk,
        animal_requirements.avg_CP_milk,
        animal_requirements.avg_milk_production_reduction
    )
...
@pytest.mark.parametrize("animal_type, should_have_milk", [
...
def test_recalculate_requirements(
    ...
    assert updated_requirements["essential_amino_acid_requirement"] == [
        mock_animal.essential_amino_acid_requirement
    ]
    if should_have_milk:
...
@pytest.mark.parametrize("nutrient_standard, should_raise_error",
                         [
                             ("NRC", False),
                             ("NASEM", False),
                             ("Unsupported", True)
...
def test_calc_rqmts(
    ...
    mocker.patch.object(animal_requirements, "calculate_NRC_energy_maintenance_requirements", return_value=(10.0, 2.0,
                                                                                                            40.0))
    ...
    mocker.patch.object(animal_requirements, "calculate_NASEM_energy_maintenance_requirements", return_value=(10.0, 2.0,
                                                                                                              1.5))
    mocker.patch.object(animal_requirements, "calculate_NASEM_energy_growth_requirements", return_value=(5.0, 1.2,
                                                                                                         30.0))
    ...
    mock_amino_acid_calculator.calculate_essential_amino_acid_requirements.return_value = (
        EssentialAminoAcidRequirements(
            histidine=77.6472, isoleucine=136.5188, leucine=248.1471, lysine=218.7945, methionine=68.46508,
            phenylalanine=146.9849, threonine=142.1294, thryptophan=33.286381, valine=161.06672
        )
    )
    ...
    if should_raise_error:
        ...
        if nutrient_standard == "NASEM":
            for attr in EssentialAminoAcidRequirements.__annotations__.keys():
                assert getattr(result["essential_amino_acid_requirement"], attr) == \
                    pytest.approx(getattr(
                        mock_amino_acid_calculator.calculate_essential_amino_acid_requirements.return_value, attr
                    ))
...
def test_calculate_NASEM_energy_maintenance_requirements(
    ...
    expected_net_energy_maintenance = 0.10 * (body_weight
                                              - expected_gravid_uterine_weight
                                              - expected_uterine_weight) ** 0.75
