...
def test_generate_schema(mocker: MockerFixture) -> None:
    ...
    patch_object_schema_setup = mocker.patch(
        "RUFAS.schema_generator.SchemaGenerator.setup_object_schema", result={"new_schema": "yay"}
    )
    ...
    patch_object_schema_setup = mocker.patch(
        "RUFAS.schema_generator.SchemaGenerator.setup_object_schema", side_effect=ValueError("Oh no!")
    )
