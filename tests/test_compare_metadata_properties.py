...
@pytest.mark.parametrize("error, file_name", [
    (FileNotFoundError, "nonexistent.json"),
    (PermissionError, "restricted.json"),
    (OSError, "error.json")
])
...
def test_compare_metadata_properties():
    ...
    with patch('sys.argv', test_args):
        with patch('argparse.ArgumentParser.parse_args', return_value=MagicMock(file1="file1.json", file2="file2.json")):
            with patch('compare_metadata_properties.load_json', side_effect=[{"key": "value1"}, {"key": "value2"}]):
                with patch('deepdiff.DeepDiff') as mock_deepdiff:
                    mock_deepdiff.return_value = {"differences": "diff_output"}
