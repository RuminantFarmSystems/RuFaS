...
from RUFAS.data_structures.feed_storage_to_animal_connection import TotalInventory, IdealFeeds, RequestedFeed, \
    NutrientStandard
...
from RUFAS.data_structures.manure_to_crop_soil_connection import ManureEventNutrientRequestResults, \
    NutrientRequestResults, ManureEventNutrientRequest
...
def test_simulate(simulation_engine: SimulationEngine, mocker: MockerFixture, start_time: int, end_time: int) -> None:
    ...
    mock_report_end_of_simulation = mocker.patch("RUFAS.biophysical.animal.animal_module_reporter.AnimalModuleReporter"
                                                 ".report_end_of_simulation")
    mock_feed_query_available_feeds = mocker.patch.object(
        simulation_engine.feed_manager,
        "query_available_feeds",
        return_value=[{1: "1"}]
    )
...
@pytest.mark.parametrize(
    "is_time_to_recalculate_max_daily_feeds, is_time_to_reformulate_ration,"
    "is_ok_to_feed_animals",
    [
        (True, True, True),
        (True, True, False),
        (True, False, True),
...
def test_daily_simulation(
    ...
    simulation_engine.next_max_daily_feed_recalculation = simulation_engine.time.current_date \
        if is_time_to_recalculate_max_daily_feeds else simulation_engine.time.current_date + timedelta(days=1)
    simulation_engine.max_daily_feed_recalculation_interval = timedelta(days=round(365 / 4))
    simulation_engine.next_ration_reformulation = simulation_engine.time.current_date.date() \
        if is_time_to_reformulate_ration else (simulation_engine.time.current_date + timedelta(days=1)).date()
    ...
    mock_field_daily_update_routine = mocker.patch.object(
        ...
        return_value=(
            mock_harvested_crops := [
                HarvestedCropStorageType(crop_1, StorageType.BAG), HarvestedCropStorageType(crop_2, StorageType.BALEAGE)
            ]
        )
    )
    expected_harvested_crops_config_names = [[harvest_crop.harvested_crop.config_name]
                                             for harvest_crop in mock_harvested_crops]
    ...
    not_harvested_feeds_config_names = [
        config_name for config_name in simulation_engine.feed_manager.crop_to_rufas_id.keys()
    ...
    assert mock_feed_get_total_inventory.call_args_list == [
        call(mock_time.current_date.date(), mock_weather, mock_time)
    ] * 2
...
def test_formulate_ration(
    ...
    expected_next_ration_formulation_date = (datetime.today()
                                             + timedelta(days=ration_formulation_interval_length)).date()
...
def test_generate_daily_manure_applications(simulation_engine: SimulationEngine, mocker: MockerFixture) -> None:
    ...
    manure_event_request_1, manure_event_request_2 = (MagicMock(auto_spec=ManureEventNutrientRequest),
    ...
    manure_event_request_1.event, manure_event_request_2.event = ((mock_event_1 := MagicMock(auto_spec=ManureEvent)),
    ...
    manure_event_request_1.manure_supplement_method, manure_event_request_2.manure_supplement_method = (
        ManureSupplementMethod.NONE, ManureSupplementMethod.NONE)
    manure_event_request_1.nutrient_request, manure_event_request_2.nutrient_request = (
        (mock_nutrient_request_result := MagicMock(auto_spec=NutrientRequestResults)), None)
...
@pytest.mark.parametrize(
    "is_end_to_end_test_run", [True, False]
)
def test_initialize_simulation(
        is_end_to_end_test_run: bool, mocker: MockerFixture
) -> None:
    """
    ...
    assert (simulation_engine.next_max_daily_feed_recalculation == mock_time.current_date
            + timedelta(days=round(365 / 4)))
...
def test_annual_simulation(
        year_start_day: int, year_end_day: int, expected_day_count: int,
