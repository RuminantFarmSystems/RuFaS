...
\addbibresource{resources/manure.bib}  % Your .bib file name
\usepackage{titlesec}
\usepackage{etoolbox}
...
\begin{titlepage}
  ...
  {\Large Manure Module\par}
  \vspace{1cm}

  {\large \today\par}
  \vspace{1.5cm} % Add space before the image

  \includegraphics[width=0.25\textwidth]{resources/manure.png} % Adjust size and filename
...
\textbf{Structural setup of the Manure Module} 
...
\textbf{Overview of Required Inputs}
...
The assignment of manure generated in the Animal Module (which also reflects the quantity of bedding used) to its destination in the Manure Module occurs in the Animal Module. See the \hyperref[AnimalManureConn]{Animal to Manure Connection} section for more information. 

\end{tcolorbox}
\vspace{0.2cm}

\textbf{General Assumptions of the Manure Module}
        \begin{enumerate}
...
\section{Animal and Manure Module Connection} \label{AnimalManureConn}
...
Instance(s) of a ManureStream that represent(s) manure being loaded into the manure storage on a single day. ManureStream instances include the following variables (all in kg except for volume,  m\textsuperscript{3})
\vspace{0.3cm}

\textbf{Stream Formation}
\begin{itemize}
    \item Instances of PenManure objects that represent manure excreted by single pens of animals in the Animal module. See the Manure Excretion section of the Animal Module documentation for further details. 
...
\begin{itemize}
    \item A ManureStream class instance for each manure stream defined by the user, that represents the attributes of the manure in the specific manure stream. ManureStream instances include the following variables (all in kg except for volume, \(m^3)\):
    \begin{itemize}
        \item water 
        ...
        \item potassium
        \item ash
        \item non\_degradable\_volatile\_solids
        \item degradable\_volatile\_solids
        \item total\_solids
...
\section{Manure Handler} \label{sec:manure-handler}
\subsection{Introduction}
    \vspace{0.2cm}
...
\vspace{0.2cm}
    ...
    \begin{center}
        \(total\_cleaning\_water (m^3) = cleaning\_water (L) + fresh\_water\_volume\_used\_for\_milking (L) * LITERS\_TO\_CUBIC\_METERS LITERS\_TO\_CUBIC\_METERS:\ a\ constant\ set\ to\ 0.997\ kg/m^3\)
    \end{center}
        \vspace{0.5cm}
...
\begin{tcolorbox}[colback=white, colframe=black, boxrule=0.5pt, arc=3pt, width=\textwidth]
The soiled surface area (m2) value is calculated in the Animal module, using pen-level information (number of stalls, number of cows, etc.) and the soiled surface area value is passed to the manure module directly.
\end{tcolorbox}
...
After housing CH\textsubscript{4} and CO\textsubscript{2} emissions are calculated, the total quantity of VS loss through microbial degradation can be calculated.
 \vspace{0.2cm}

    \begin{center}
        \(total\_VS\_loss\ (kg) = housing\ CH_4\ (kg)  + housing\ CO_2\ (kg) \)
    \end{center}
        \vspace{0.5cm} 
...
\textbf{When:}
    \begin{itemize}
        ...
        \item housing CO\textsubscript{2} (kg): daily emissions of CO\textsubscript{2} from manure on barn floors, calculated in \textbf{[MN.HDL.5]}.
    \end{itemize}  


The fraction of volatile solids lost from the degradable (dVS) and non-degradable VS (ndVS) fractions is assumed to be equal to the proportion of these two fractions in manure entering the processor. 
 \vspace{0.2cm}
...
\textbf{Calculate housing NH3 emissions}
...
\vspace{0.5cm}
    ...
    \begin{itemize}
        ...
        \item \hypertarget{HENRY}{First}, we need to derive the value of the equilibrium coefficient Q for the NH3 gas in the air for a given concentration of TAN in stored slurry using Henry’s law of distribution. 
    \end{itemize}
    \vspace{0.15cm}
...
\textbf{When:}
    ...
    \begin{itemize}
        \item Second, we determine the effective total resistance value of NH3 transport from manure solution to the surface and from the manure surface to the atmosphere. 
         \vspace{0.2cm}
...
\textbf{When:}
    ...
    \end{itemize}
        ...
        \item Next, the rate of NH\textsubscript{3}-N loss in kg N/m2 from manure on the barn floor is calculated 
         \vspace{0.2cm}
...
\textbf{When:}
    ...
    \begin{center}
\(NH_3N\ emissions\ (kg) = NH3N\_rate * soiled\_surface\_area \)
    \end{center}
        \vspace{0.5cm} 
...
\textbf{When:}
    \begin{itemize}
        \item NH3N\_rate (kg N/m\textsuperscript{2}): Rate of NH\textsubscript{3}-N loss (kg/m\textsuperscript{2}) from manure, calculated in \textbf{[MN.AMM.5]}
        \item soiled\_surface\_area (m\textsuperscript{2}): the total surface area soiled with manure
...
\textbf{Manure composition update }
...
The equations below describe the general process for separating nutrients between liquid and solid fractions. The general equation is as follows:
         \vspace{0.2cm}
...
Nutrients that are separated according to this pattern are presented in the table below. The default separation efficiencies, as well as minimum and maximum allowable separation efficiencies, for the rotary screen and screw press separators are also provided \parencite{Hegg1981, Mukhtar1999, Varma2021}.
\newpage
...
\textbf{Calculate volatile solids fractions}
\vspace{ 0.25cm}

\textit{Separated solids fraction}
After the total quantity of volatile solids (VS) removed to the solids fraction is calculated, we then adjust the quantity of degradable (dVS) and non-degradable VS (ndVS) in each fraction according to their proportion in incoming manure. 
         \vspace{0.2cm}
...
\textbf{When:}
    \begin{itemize}
        \item degradable\_VS (kg): mass of degradable VS in manure loaded into the separator
        \item total\_VS (kg): mass of total VS in manure loaded into the separator

    \end{itemize}

Using the degradable VS fraction value, we can then determine the quantity of degradable and non-degradable VS in separated solids.
\vspace{0.3cm}

solidsdVS (kg) = (totalVS (kg) * separation efficiency) * degradableVSfrac \\
solidsndVS (kg) = (totalVS (kg) * separation efficiency) * (1 - degradableVSfrac) \\
\vspace{0.2cm}

    \begin{itemize}
        \item total\_VS (kg): mass of total VS in manure loaded into the separator
        \item separation efficiency: the separation efficiency for total VS, i.e., proportion of manure VS loaded into the separator that are partitioned into the solid fraction
        \item degradable\_VS\_frac: the proportion of total VS that are degradable VS
    \end{itemize}
    \vspace{0.2cm}
    
\textit{Separated liquid fraction} The quantities of dVS and ndVS in the separated liquid fraction are simply the total quantity loaded into the separator, minus the quantity partitioned into the solid fraction. 
\vspace{0.3cm}

liquid\_dVS (kg) = total\_dVS (kg) - solids\_dVS (kg) \\
liquid\_ndVS (kg) = total\_ndVS (kg) - solids\_ndVS (kg) \\
\vspace{0.2cm}

    \begin{itemize}
        \item total\_VS (kg): mass of total VS in manure loaded into the separator
        \item total\_ndVS (kg): mass of total ndVS in manure loaded into the separator
        \item solids\_dVS (kg): mass of dVS separated from raw manure into the solids fraction
        \item solids\_ndVS (kg): mass of ndVS separated from raw manure into the solids fraction
...
non\_degradable \_volatile\_solids & -  & 
\( \text{Entering ndVS (kg)} \times \text{VS removal efficiency} \times (1 - \text{degradable\_VS\_frac}) \) & 
\( \text{Entering ndVS (kg)} \times (1 - \text{VS removal efficiency}) \times (1 - \text{degradable\_VS\_frac}) \) \\

degradable \_volatile\_solids & - & 
\( \text{Entering dVS (kg)} \times \text{VS removal efficiency} \times \text{degradable\_VS\_frac} \) & 
\( \text{Entering dVS (kg)} \times (1 - \text{VS removal efficiency}) \times \text{degradable\_VS\_frac} \) \\
...
\vspace{0.2cm}
    ...
    \multicolumn{1}{c}{\textbf{Description}} \\
    \midrule
    AnaerobicDigester  & anaerobic\_digester.py it inherits behavior from the base class, Digester; however, at this time, there is no functionality included in the base Digester class. Therefore all methods are contained within the anaerobic\_digestion.py child class \\
...
\textbf{When:}
    ...
    \begin{itemize}
        \item First, we calculate the density of CO\textsubscript{2} based on the user-provided digestion setpoint temperature.
            \vspace{0.5cm}
...
\textbf{When:}
    ...
    \vspace{0.2cm}
        ...
        \item Second, we determine the quantity of CO\textsubscript{2} volume and mass generated, assuming digester biogas contains a 60:40 volumetric ratio of CH\textsubscript{4} to CO\textsubscript{2}. 
            \vspace{0.5cm}
...
\textbf{When:}
    ...
    \vspace{0.2cm}
        ...
        \item Third, we calculate the total destruction of total VS. 
            \vspace{0.5cm}
...
\(total\_volatile\_solids\_destruction = generated\_methane\_mass  + generated\_carbon\_dioxide\_mass \)
    \end{center}
        ...
        \item We then utilize the value of total\_volatile\_solids\_destruction (kg) to update VSd and VSnd using \_destroy\_volatile\_solids. As mentioned above, the total quantity of VS destroyed is partitioned between VSd and VSnd according to the proportion of the two fractions in manure entering the digester. E.g., if manure entering contained 80\% VSd and 20\% VSnd, 80\% of destroyed VS will be subtracted from VSd, and 20\% from VSnd.
        
        To do this, we calculate the ratio of VSd to VS (degradable\_volatile\_solids\_frac), apply this fraction to the total\_volatile\_solids\_destruction value to determine the updated VSd value, then determine the updated VSnd value.
    \end{itemize}
            \vspace{0.5cm}
...
\(degradable\_VS = degradable\_VS\ (kg)  - (total\_VS\_destruction\ (kg) * degradable\_VS\_frac)\)
    ...
    \begin{center}
\(non\_degradable\_VS = non\_degradable\_VS\ (kg) - total\_VS_destruction * (1.0 - degradable\_VS\_frac\ (kg) )\)
    \end{center}
        \vspace{0.5cm} 
...
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{@{} L{5cm} L{1cm} L{6cm} @{}}
...
total\_ammoniacal\_nitrogen  & kg & 
Entering ammoniacal\_nitrogen (kg) TAN\_INCREASE\_FACTOR, nitrogen (kg)  \\
...
non\_degradable\_volatile\_solids &  & 
\( \text{Entering non\_degradable\_volatile\_solids (kg)} - \text{total\_volatile\_solids\_destruction (kg)} \times (1.0 - \text{degradable\_volatile\_solids\_frac}) \) \\

degradable\_volatile\_solids &  & 
\( \text{Entering degradable\_volatile\_solids (kg)} - \text{total\_volatile\_solids\_destruction (kg)} \times \text{degradable\_volatile\_solids\_frac} \) \\

total\_solids  & kg & 
\( \text{Entering total\_solids (kg)} - \text{total\_volatile\_solids\_destruction (kg)} \) \\

volume  & m\textsuperscript{3} & 
\( \text{Entering volume (m}^3) - \frac{\text{total\_volatile\_solids\_destruction (kg)}}{\text{ManureConstants.SLURRY\_MANURE\_DENSITY}} \) \\
...
\midrule
Temperature & Barn temperature method & Ambient temperature method \\
...
\textbf{Python classes for slurry storage}
\vspace{0.2cm}
    \begin{table}[h!]
    \centering
    \renewcommand{\arraystretch}{1.3}
    ...
    \multicolumn{1}{c}{\textbf{Description}} \\
    \midrule
    SlurryStorageOutdoor(Storage)  & aslurry\_storage\_outdoor.py \\
    SlurryStorageUnderfloor(Storage)  & slurry\_storage\_outdoor.py; The content below is representative of both types of slurry storage, with specific differences between the two noted explicitly. \\
    \bottomrule
    \end{tabular}
    \end{table}  
...
\vspace{0.5cm}
    ...
    \begin{center}
        \(surface\_area (m^2) = \frac{(cow\_num * MANURE\_CONVERSION\_CONSTANT * storage\_time * FREEBOARD\_CONSTANT) }{ (DEPTH\_CONSTANT - PRECIPITATION\_CONSTANT)} \)
    \end{center}
        \vspace{0.5cm} 
...
\(Arrh\_exp\ g\ (CH_4\ kg^{-1}\ VS\ h^{-1}) = e ^{Ln(A) - \frac{ACTIVATION\_ENERGY}{(Gas constant * manure temperature)}}\)
    ...
    \textbf{When:}
    \begin{itemize}
        \item Ln(A): The natural log of the Arrhenius parameter (NATURAL\_LOG\_ARRHENIUS\_CONSTANT constant), set at 31.2 based on \cite{Petersen2024}. This is an empirically-derived value determined based on observed manure CH\textsubscript{4} emission values.
...
Daily emissions of CH\textsubscript{4} and CO\textsubscript{2} from slurry storage occur through microbial degradation of VS in slurry manure, among other processes \parencite{Petersen2024}. Therefore, gaseous emissions from slurry storage result in a decrease in the quantity of VS in stored slurry. 
VSd and VSnd remaining in manure are updated separately according to their loss via CH\textsubscript{4} \textbf{[MN.STO.4]}. Here, we assume a fixed 1:3 molar ratio of CH\textsubscript{4} to CO\textsubscript{2} emissions from stored slurry from \cite{Petersen2024} to calculate the total amount of VS\textsubscript{d} and VS\textsubscript{nd} lost through CH\textsubscript{4} and CO\textsubscript{2} emissions based on the quantity of CH\textsubscript{4} emitted from each VS fraction.

The molar mass of CO\textsubscript{2} is 2.75 times that of CH\textsubscript{4}, therefore, the total kg of VS loss per kg of CH\textsubscript{4} loss is 1 + (3 x 2.75) = 9.25 kg (or, 1 kg of VS loss from CH\textsubscript{4} + 8.25 kg of VS loss from CO\textsubscript{2}). We use this value to calculate total VS loss based on the estimated emission of CH\textsubscript{4} (kg) from each VS fraction in storage.
\vspace{0.5cm}

    \begin{center}
        \(VS_{d\ or\ nd}\ loss\ (kg)=\ CH_{4}\ emission\ from\ VS_{d\ or\ nd}\ (kg)*\ METHANE\_TO\_METHANE\_CARBON\_DIOXIDE\_RATIO\)
    \end{center}
        \vspace{0.5cm} 
    ...
    \begin{itemize}
        ...
        \item METHANE\_TO\_METHANE\_CARBON\_DIOXIDE\_RATIO: default ratio of CH\textsubscript{4} + CO\textsubscript{2} kg emitted per CH\textsubscript{4} kg emitted from slurry storage, set to 9.25
...
Emission of NH\textsubscript{3}-N from slurry storage is determined using equations from \cite{Rotz2006}, which are also utilized in the IFSM \parencite{Rotz2023}. Ammonia emissions are influenced by the quantity of TAN accumulated in manure storage, manure temperature, and manure storage surface area. First, we must derive the various parameters utilized in the calculation. This has already been outlined in the \hyperlink{HENRY}{Manure Handler} section.
\vspace{0.5cm}
...
\subsection{Output Variables}
    ...
    \textbf{\textit{Received and accumulated outputs}}
...
non\_degradable\_volatile\_solids &  & 
\( \text{stored non-degradable volatile solids (kg)} + \text{received non-degradable volatile solids (kg)} - \text{VSnd loss (kg)} \) \\

degradable\_volatile\_solids &  & 
\( \text{stored degradable volatile solids (kg)} + \text{received degradable volatile solids (kg)} - \text{VSd loss (kg)} \) \\

total\_solids  & kg & 
\( \text{stored total solids (kg)} + \text{received total solids (kg)} - \text{VSd loss (kg)} - \text{VSnd loss (kg)} \) \\

volume  & m\textsuperscript{3} & 
\( \text{stored volume (m}^3) + \text{received volume (m}^3) - \frac{\text{VSd loss (kg)} + \text{VSnd loss (kg)}}{\text{SLURRY\_MANURE\_DENSITY}} \) \\
...
\begin{itemize}
    ...
    \item storage\_ammonia (kg): Daily emission of NH3 from accumulated manure an anaerobic lagoon.
...
Temperature of stored manure is assumed to be equal throughout the entire mass of manure. In manure stored outdoors, manure temperature is assumed to be equal to air temperature, but is bounded to 0 to 35\(^\circ C\).
\vspace{0.25cm}
...
Exposed surface area (m2) of the manure in storage is important in determining NH\textsubscript{3}-N emissions, as well as in determining precipitation volume added to storage if the storage is not covered or indoors. Wherever possible, this value should be provided by the user if modeling a real farm. If farm-specific information is unavailable or the farm being modeled is theoretical, the surface area should be estimated using tools like \hyperlink{https://www.nrcs.usda.gov/resources/tech-tools/usda-animal-waste-management-version-241}{Animal Waste Management}. However, the RuFaS team recognizes that minimizing required inputs is desirable, though a fixed storage surface area is undesirable due to the variability in storage structure size and surface area. With this, an equation was developed that estimates storage surface area based on the following assumptions:
    \begin{itemize}
        ...
        \item Herd composition, and thus manure excretion, is fixed, and the number of animals in each life stage class is proportional to the number of mature cows. 
    \end{itemize}
A constant value was derived to calculate estimated manure excretion based on the number of mature cows housed on the farm (a user input). The average number of animals in each class was determined according to default RuFaS animal lifecycle inputs, and the total mass and volume of manure excreted by the herd was calculated. This resulted in an estimated daily herd-wide manure excretion of 168.6 kg or 0.118  m\textsuperscript{3} of manure per mature cow housed on the farm. The resulting equation is used to calculate storage surface area (m2).
        \vspace{0.5cm}
...
Detailed descriptions are outlined in the \hyperref[cover]{Slurry Storage} section of this module. \\
Precipitation volume for anaerobic lagoons that are uncovered or have a crust is calculated as follows:\\
        \vspace{0.5cm}
...
\(Arrh\_exp\ (g\ CH_{4}\ kg^{-1}\ VS\ h^{-1}) = e ^{Ln(A) - \frac{ACTIVATION\_ENERGY}{(Gas constant * manure temperature)}}\)
    ...
    \textbf{When:}
    \begin{itemize}
        \item Ln(A): The natural log of the Arrhenius parameter (NATURAL\_LOG\_ARRHENIUS\_CONSTANT), set at 31.2 based on \cite{Petersen2024}. This is an empirically-derived value determined based on observed manure CH\textsubscript{4} emission values.
        \item ACTIVATION\_ENERGY: the apparent activation energy of methanogenesis in cattle slurry (J/mol), set at 81,000 J/mol, based on \cite{Elsgaard2016}. 
        \item Gas constant: ideal gas constant, set at 8.314 J K/mol.
        \item Manure temperature (K): temperature of manure in storage. 
...
\(CH_{4}\ emission\ from\ VS_{d\ or\ nd}\ (kg\ d^{-1}) = 24 * Arrh\_exp * VS_{d\ or\ nd} * rate\_factor\)
    ...
    \begin{itemize}
        \item 24: conversion factor from hours to day. 
        \item Arrh\_exp: Arrhenius parameter for CH\textsubscript{4} emission rate (g CH\textsubscript{4} kg\textsuperscript{-1} VS h\textsuperscript{-1}), calculated in \textbf{[MN.MET.2]}. 
        \item S\textsubscript{d} or \textsubscript{nd}: The mass (kg) of VS\textsubscript{d} or VS\textsubscript{nd} in manure in storage.
        \item rate\_factor: The unitless rate-correcting factor, set to 1 for VS\textsubscript{d} and 0.01 for VS\textsubscript{nd}. 
    \end{itemize}
    \vspace{0.5cm}
...
\vspace{0.5cm}
    ...
    \begin{center}
\(Daily\ storage\ CH_{4}\ (kg) : storage\ CH_{4} * METHANE\_DESTRUCTION\_EFFICIENCY\)
    \end{center}
        \vspace{0.5cm} 
...
VS\textsubscript{d} and VS\textsubscript{nd} remaining in manure are updated separately according to their loss via CH\textsubscript{4} (MN.STO.3). Here, we assume a fixed 1:3 molar ratio of CH\textsubscript{4} to CO\textsubscript{2} emissions from stored slurry from \cite{Petersen2024} to calculate the total amount of VSd and VSnd lost through CH\textsubscript{4} and CO\textsubscript{2} emissions based on the quantity of CH\textsubscript{4} emitted from each VS fraction.

The molar mass of CO\textsubscript{2} is 2.75 times that of CH\textsubscript{4}, therefore, the total kg of VS loss per kg of CH\textsubscript{4} loss is 1 + (3 x 2.75) = 9.25 kg (or, 1 kg of VS loss from CH\textsubscript{4} + 8.25 kg of VS loss from CO\textsubscript{2}). We use this value to calculate total VS loss based on the estimated emission of CH\textsubscript{4} (kg) from each VS fraction.
\vspace{0.5cm}
    ...
    \begin{itemize}
        ...
        \item METHANE\_TO\_METHANE\_CARBON\_DIOXIDE\_RATIO: default ratio of CH\textsubscript{4} + CO\textsubscript{2} kg emitted per CH\textsubscript{4} kg emitted from an anaerobic lagoon, set to 9.25
...
Emission of NH\textsubscript{3}-N from anaerobic lagoons is determined using equations from \cite{Rotz2006}, which are also utilized in the IFSM \parencite{Rotz2023}. Ammonia emissions are influenced by the quantity of total ammoniacal N (TAN) accumulated in manure storage, manure temperature, and manure storage surface area. First, we must derive the various parameters utilized in the calculation.
    ...
    \begin{itemize}
        \item First, we need to derive the value of the equilibrium coefficient Q for the NH3 gas in the air for a given concentration of TAN in stored manure using Henry’s law. Note that the concentration of NH3 in the free atmosphere is assumed to be zero. Since Q is a function of the Henry’s law coefficient K\textsubscript{h} and a dissociation of ammonium coefficient K\textsubscript{a}, we will calculate those first. 
        \vspace{0.2cm}
...
\textbf{When:}
    \begin{itemize}
        \item TAN (kg N/m\textsuperscript{2}): Mass of ammoniacal N in stored manure
        \item c: time conversion constant (86400 s per d)
        \item y: manure density, set to 990 kg/m\textsuperscript{3} 
...
N2O emissions (kg N2O-N) are based on the daily quantity of manure N loaded into the lagoon, and whether the lagoon is covered or uncovered. This method is based on \cite{IPCC2019}; however, it should be noted that the original IPCC method is based on daily manure N excretion by animals, whereas the current method is based on manure N loading into storage, which may reflect upstream N losses from NH3 emissions in housing, solid liquid separation, etc. The calculation is as follows:
    ...
    \textbf{\textit{Stored manure}}
    \vspace{0.1cm}
    
    Below is a summary of updates to ManureStream variables representing the stored manure. Note that the formulas below may be a summarization of multiple steps detailed above, and are intended to provide an overview of what mass losses/gains are reflected in the value of each variable.
    \vspace{0.2cm}

    Equations in the calculation column of Table 20 are in the format of: updated stored manure value = yesterday’s stored manure value + today’s manure value +/- XYZ. The updated stored manure values reflect the total quantity of manure/nutrients in storage on a single day after accounting for all gains/losses that occurred on that day. Received manure simply refers to the manure being loaded into the manure storage each day. 
    \vspace{0.5cm}
    ...
    \textbf{Received manure}
    Received manure outputs represent the quantity of manure mass and nutrients added to the anaerobic lagoon on a single day. No nutrient losses from gas or other emissions/losses are reflected in these output values. 
    \vspace{0.2cm}
...
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{@{} L{5cm} L{1cm} L{9cm} @{}}
\caption{Manure storage variables and calculation formulas} \\
\toprule
\multicolumn{1}{c}{\textbf{Variable}} & 
\multicolumn{1}{c}{\textbf{Units}} & 
\multicolumn{1}{c}{\textbf{Description}} \\
...
non\_degradable\_volatile\_solids &  & 
\( \text{stored non-degradable VS (kg)} + \text{received non-degradable VS (kg)} - \text{VSnd loss (kg)} \) \\
...
volume & m\textsuperscript{3} & 
\( \text{stored volume (m}^3) + \text{received volume (m}^3) - \frac{\text{VSd loss (kg)} + \text{VSnd loss (kg)}}{\text{LIQUID\_MANURE\_DENSITY}} \) \\
...
Bedded packs fall into one of two management methods: a traditional bedded pack, or a compost bedded pack. See \href{https://thedairylandinitiative.vetmed.wisc.edu/home/housing-module/adult-cow-housing/bedded-pack/}{UW-Madison Dairyland Initiative Report} for further detail.
    \vspace{0.2cm}
    
    \begin{tcolorbox}[colback=white, colframe=black, boxrule=0.5pt, arc=3pt, width=\textwidth]
        Though the names are often used interchangeably, note here that the primary difference between the two types is the presence or absence of active mixing, and the length of time between complete removal of the accumulated bedding/manure mixture from the pen.
    \end{tcolorbox}
    \vspace{0.2cm}
...
CompostBeddedPackBarn(Storage) & bedded\_pack.py  \\
    ...
    \end{tabular}
    \end{table} 
    {\footnotesize\textit{Note:} that Bedded Pack inherits some behavior (functionality) from the base Storage class.}
...
\subsection{Methodology}
    ...
    Description: Calculates the daily mass of methane emitted from the bedded pack based on daily manure VS added to the bedded pack (through animal excretion and bedding addition) and an emission factor based on mixing activity and simulation average temperature \parencite{Hanson2024}.  
    \vspace{0.2cm}

    \begin{tcolorbox}[colback=white, colframe=black, boxrule=0.5pt, arc=3pt, width=\textwidth]
Here and in other equations, ‘daily’ denotes the value associated with received manure added to the bedded pack on a specified simulation day. 
    \end{tcolorbox}
    \vspace{0.2cm}
    ...
    \begin{itemize}
        \item B\textsubscript{o} = achievable methane generation constant (kg CH\textsubscript{4} per kg manure VS)
        ...
        \item tVS (kg) = daily mass (kg) of manure total volatile solids in the bedded pack manure, received from ManureStream(s)
...
Description: In addition to microbial processes that occur in anaerobic conditions, which generate primarily CH\textsubscript{4} and CO\textsubscript{2}, carbon in the manure/bedding mixture is also degraded through aerobic microbial processes. This process is a function of substrate availability/degradability, temperature, moisture, aeration, and microbial population. This series of calculations is based on the IFSM composting simulation method \parencite{bonifacio2017a, bonifacio2017b}. Simplifications/assumptions that have been made which diverge from the original method are explicitly noted below. 
    ...
    Manure nitrogen being deposited and accumulating in the bedded pack results in NH3 emissions. Here we utilize an emission factor based on mixing activity \parencite{hanson2024} to estimate total kg of ammonia loss based on the daily manure N deposition by animals. 
        \vspace{0.5cm}        
    ...
    In addition to NH\textsubscript{3}-N emissions, manure nitrogen deposition in the bedded pack also results in N2O emissions. Similar to NH3, we estimate daily N2O-N loss using an emissions factor from Hanson et al. (2024) based on the type of management used. 
            \vspace{0.5cm}        
...
\textbf{Received and accumulated outputs}
\vspace{0.2cm}
...
\begin{itemize}
    \item ManureStream values, as received from the previous processor(s) in the manure management chain
    \item Precipitation water (kg), calculated in \textbf{[MN.STO.2]} (if applicable), is added to the water value in ManureStream
\end{itemize}
\vspace{0.5cm}
...
non\_degradable\_volatile\_solids  &  & \( \text{Stored non-degradable VS} - \text{VS loss} \times (1.0 - \text{degradable VS fraction}) \) \\
degradable\_volatile\_solids &  & \( \text{Stored degradable VS} - \text{VS loss} \times \text{degradable VS fraction} \) \\
...
\subsection{Introduction} 
    ...
    \begin{itemize}
        ...
        \item Gas emissions in this processor are based on annual emissions factors that are extrapolated to a daily timestep, meaning that calculations are made based on daily excretion of manure VS or N, rather than accumulated quantities of the nutrients. With this, for N and NH3 emissions for example, manure is assumed to lose all estimated NH\textsubscript{3}-N on the day it is excreted. Manure N accumulation in lot manure is tracked, but accumulated manure N does not influence NH3 emissions.
    \end{itemize}
...
\vspace{0.2cm}
 ...
 \\
    ...
    {\footnotesize\textit{Note:} that Open Lot inherits some behavior (functionality) from the base Storage class.}
\vspace{0.5cm}
...
\subsection{Methodology}
    ...
    Calculates the daily mass of methane emitted from the open lot based on daily manure volatile solids (VS) added to lot manure (through animal excretion and bedding addition) and the emission factor calculated in \textbf{[MN.MET.5]}.  
     \vspace{0.5cm}    

     \begin{tcolorbox}[colback=white, colframe=black, boxrule=0.5pt, arc=3pt, width=\textwidth]
        Here and in other equations, ‘daily’ denotes the value associated with received manure added to the open lot on a specified simulation day.
    \end{tcolorbox}

    
    \begin{center}
        \(methane\ (kg) = Bo * MCF * tVS* METHANE\_FACTOR\)
    \end{center}
        \vspace{0.5cm} 
    ...
    \begin{itemize}
        \item Bo = achievable methane generation constant (kg CH\textsubscript{4} per kg manure VS)
        \item Constant Value: 0.24 m\textsuperscript{3} CH\textsubscript{4}/kg VS
        \item MCF = methane conversion factor calculated in \textbf{[MN.MET.5]}, based on daily ambient temperature
        \item tVS (kg) = daily mass (kg) of manure total volatile solids in the open lot pen, received from ManureStream(s)
...
In addition to microbial processes that occur in anaerobic conditions, which generate primarily CH\textsubscript{4} and CO\textsubscript{2}, open lot manure C is also degraded through aerobic microbial processes. This process is a function of substrate availability/degradability, temperature, moisture, aeration, and microbial population. This series of calculations is based on the IFSM composting simulation method, which is described in detail in \cite{bonifacio2017a} and \cite{bonifacio2017b}. Simplifications/assumptions that have been made which diverge from the original method are explicitly noted below. 
    ...
    \textbf{Calculate N loss to nitrous oxide}
    \_calculate\_cbpb\_nitrous\_oxide\_emission
    In addition to NH\textsubscript{3}-N emissions, manure nitrogen deposition in the bedded pack also results in N2O emissions. Similar to NH3, we estimate daily N2O-N loss using an emissions factor from \cite{Hanson2024}) based on the type of management used. 
            \vspace{0.5cm}        
...
(i.e., deposited on the open lot surface). In open lot processors, the following nutrient sources are represented in received manure values:
...
Daily precipitation volume/mass is not currently represented in received open lot manure (i.e., precipitation falling onto the lot each day is not represented in the received manure outputs representing nutrient/mass/water additions to the lot each day).
...
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{@{} L{5cm} L{1cm} L{9cm} @{}}
\caption{Manure composition variables and calculations after volatile solids loss} \\
\toprule
\multicolumn{1}{c}{\textbf{Variable}} &
\multicolumn{1}{c}{\textbf{Units}} &
\multicolumn{1}{c}{\textbf{Calculation}} \\
...
non\_degradable\_volatile\_solids & kg & 
\( \text{stored non-degradable volatile solids (kg)} - \text{total volatile solids loss (kg)} \times (1.0 - \text{degradable volatile solids fraction}) \) \\

degradable\_volatile\_solids &  & 
\( \text{stored degradable volatile solids (kg)} - \text{total volatile solids loss (kg)} \times \text{degradable volatile solids fraction} \) \\
...
\subsection{Methodology}
    ...
    Calculates the daily mass of methane emitted from the compost based on manure volatile solids (VS) added to compost (from animal excretion and bedding addition) and an emission factor based on composting method and simulation average temperature described in Table 1 \parencite{Hanson2024}. 

    \begin{tcolorbox}[colback=white, colframe=black, boxrule=0.5pt, arc=3pt, width=\textwidth]
        Note here and in other equations, ‘daily’ denotes the value associated with received manure added to the compost on a specified simulation day. 
    \end{tcolorbox}
...
Passive windrow & 1.0 & 2.0 & 2.5 \\
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Constant Value: 0.24 m\textsuperscript{3} CH\textsubscript{4}/kg VS
        \end{itemize}
        \item MCF = methane conversion factor (Table 27), based on simulation average ambient temperature
        \item tVS (kg) = daily mass (kg) of manure total volatile solids added to compost, received from ManureStream(s)
...
Description: Manure nitrogen being added to and accumulating in the compost results in NH3 emissions. Here we utilize daily manure N addition to compost and an emission factor based on composting method  \parencite{Hanson2024} to estimate total kg of NH\textsubscript{3}-N loss.
...
\renewcommand{\arraystretch}{1.3}
\begin{longtable}{@{} L{5cm} L{1cm} L{9cm} @{}}
\caption{Manure pool calculations after solids loss} \\
\toprule
...
\begin{longtable}{@{} L{4.5cm} L{6cm} L{3cm} L{8cm} @{}}
...
\multirow{10}{*}{\textbf{Anaerobic Lagoon}} 
 ...
 & FREEBOARD\_CONSTANT & 1.2 & 20\% volume allowance above max volume if surface area is not user-defined. \\
 ...
 & METHANE\_TO\_METHANE \_CARBON\_DIOXIDE\_RATIO & 9.25 kg & Mass ratio of CO\textsubscript{2} + CH\textsubscript{4} to CH\textsubscript{4} from storage \parencite{Petersen2024}. \\
 & NATURAL\_LOG\_ARRHENIUS \_CONSTANT & 31.2 g CH\textsubscript{4}/kg VS/h & Log of Arrhenius parameter for methane emissions from stored manure. \\
 & STORAGE\_RESISTANCE & 23.1 & Default resistance to ammonia volatilization (s/m). \\
\cmidrule(lr){1-4}

\textbf{Animal Manure Connection} & SLURRY\_MANURE\_DENSITY & 990 kg/m\textsuperscript{3} & Default density of manure as excreted. \\
\cmidrule(lr){1-4}
\multirow{5}{*}{\textbf{Bedded Pack}} 
 & AMMONIA\_EMISSION\_COEFFICIENT \_WITH\_UNTILLED\_BEDDING & 0.25 & Ammonia emission coefficient (no mixing). \\
 & AMMONIA\_EMISSION\_COEFFICIENT \_WITH\_TILLED\_BEDDING & 0.5 & Ammonia emission coefficient (with mixing). \\
 & DEFAULT\_DAYS\_SINCE\_LAST\_MIXING & 1 d & Days since last mixing, for C decomposition. \\
 & NITROUS\_OXIDE\_EMISSION \_COEFFICIENT \_WITH\_UNTILLED\_BEDDING & 0.01 & N\textsubscript{2}O-N emitted per kg of manure N/day (no mixing). \\
 & NITROUS\_OXIDE\_EMISSION \_COEFFICIENT \_WITH\_TILLED\_BEDDING & 0.07 & N\textsubscript{2}O-N emitted per kg of manure N/day (with mixing). \\
\cmidrule(lr){1-4}
\multirow{4}{*}{\textbf{Manure Handler}} 
...
\cmidrule(lr){1-4}
\multirow{5}{*}{\textbf{Slurry Storage}} 
 & CARBON\_DIOXIDE\_MOLAR\_MASS & 44.01 g/mol & Molar mass of CO\textsubscript{2}. \\
 & CARBON\_DIOXIDE\_TO\_METHANE\_RATIO & 4 to 6 & Volumetric ratio of CO\textsubscript{2} to CH\textsubscript{4} during digestion. \\
...
\cmidrule(lr){1-4}
\textbf{Bedded Pack, Open Lots} & DEFAULT\_LAYER\_TEMPERATURE & 30 \textdegree{}C & Default layer temperature for decomposition. \\
\cmidrule(lr){1-4}
\multirow{8}{*}{\shortstack[l]{\textbf{Bedded Pack, Open Lots,} \\ \textbf{Composting}}} 
 & DECOMPOSITION\_TEMPERATURE & 60 \textdegree{}C & Temperature for peak microbial activity. \\
 & DEFAULT\_CARBON\_FRACTION \_AVAILABLE\_IN\_VSD & 0.5 & Carbon content of degradable VS. \\
 & DEFAULT\_CARBON\_FRACTION \_AVAILABLE\_IN\_VSND & 0.35 & Carbon content of non-degradable VS. \\
 & DEFAULT\_MOISTURE\_EFFECT \_MICROBIAL\_DECOMP & 0.65 & Moisture effect on microbial decomposition. \\
 ...
 & FIRST\_ORDER\_DECAYING \_COEFFICIENT & 0.01 & First-order decay coefficient. \\
 & LEACHING\_COEFFICIENT & -- & N leached per kg manure N/day. \\
\cmidrule(lr){1-4}
\textbf{Open Lots, Composting} & DEFAULT\_DAYS\_SINCE\_LAST\_MIXING & 1 d & Days since last harrow event. \\
\cmidrule(lr){1-4}
\multirow{2}{*}{\shortstack[l]{\textbf{Slurry Storage, Bedded Pack,} \\ \textbf{Open Lots, Composting}}} 
 & ACHIEVABLE\_METHANE\_EMISSION & 0.24 m\textsuperscript{3} CH\textsubscript{4}/kg VS & Achievable methane generation. \\
 & -- & -- & -- \\
\cmidrule(lr){1-4}
\multirow{2}{*}{\shortstack[l]{\textbf{Separators, Bedded Pack,} \\ \textbf{Open Lots, Composting}}} 
...
\newpage
\section{References}
\label{sec:references-manure-module}
