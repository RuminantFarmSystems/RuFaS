...
\section{Solid Liquid Separators} \label{sec:Solid Liquid Separators}
...
\section{Anaerobic Digestion} \label{sec:Anaerobic Digestion}
...
\section{Slurry Storage} \label{sec:Slurry Storage}
...
Daily temperature of manure in slurry storage outdoors is determined using a sine/cosine least squares fit to user-provided weather data, with a fixed amplitude dampening and lag (phase shift) factor. For more information, see the \textit{Calculate Stored Manure Temperature} in the Anaerobic Lagoon section.
\vspace{0.5cm}
...
\newpage
\section{Anaerobic Lagoon} \label{sec:Anaerobic Lagoon}
...
\textbf{When:}
    ...
    \begin{itemize}
        ...
        \item MANURE\_DAMPING\_FACTOR: a fixed damping factor applied to the air temperature amplitude, set to 0.5.
    \end{itemize}
    \vspace{0.5cm}
...
Detailed descriptions are outlined in the \hyperref[sec:Slurry Storage]{Slurry Storage} section of this module. Precipitation volume for anaerobic lagoons that are uncovered or have a crust is calculated as follows:\\
        \vspace{0.5cm}
...
MANURE\_DAMPING\_FACTOR & 0.5 & Fixed damping factor applied to the air temperature amplitude. \\
