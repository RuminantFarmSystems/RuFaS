...
\etocsettocstyle{\subsection*{Contents}}{} % title of the module ToC
\etocsetnexttocdepth{subsubsection}        % choose depth: section/subsection/subsubsection
...
\subsection{Herd Initialization} 
...
\subsubsection{Nutrient Requirements of Dairy Cattle (NRC)} 
    ...
    \textbf{ Energy Requirements} For details about the variables included in each calculation, please refer to the \colorbox{red}{energy requirements key.}
    \vspace{0.2cm}
...
\vspace{0.5cm}
    ...
    \textbf{ Protein Requirements} Reporting of protein requirements is divided into 4 components: maintenance, growth, pregnancy, and lactation (all in metabolizable protein). For details about the variables included in each calculation, please refer to the \colorbox{red}{protein requirements key}.
    \vspace{0.2cm}
...
NPg & Net protein requirement for growth  & g \\
    ...
    \begin{tcolorbox}[colback=white, colframe=black, title=Important Note]
        Only calcium (Ca) and phosphorus (P) requirements are considered currently. For details about the variables included in each calculation, please refer to the \colorbox{red}{mineral requirements key}.
    \end{tcolorbox}
...
\paragraph{NASEM: Calculating Nutrient Requirements of Dairy Cattle: Eighth Revised Edition} 
    ...
    The maintenance requirement is calculated based on metabolic body weight (MBW, body weight\(^{0.75}\)) adjusted for the tissues associated with pregnancy. For details about the variables included in each calculation, please refer to the \colorbox{red}{energy requirements key}.
    \vspace{0.2cm}
...
\textbf{ Mineral Requirements} Only calcium and phosphorus requirements are included in the code at present. For details about the variables included in each calculation, please refer to the \colorbox{red}{mineral requirements key}.
    \vspace{0.5cm}
...
\textbf{ Dry Matter Intake Estimation} Dry matter intake estimation is different for lactating cows and dry cows. The sum of the dry matter intake of each feed is assumed to be less than the dry matter intake estimation For details about the variables included in each calculation, please refer to the \colorbox{red}{DMI estimation key}.
    \vspace{0.5cm}
...
The ration formulation will proceed as described in the \colorbox{red}{Automated Methodology} above, but when formulation fails, different outcomes will occur.
...
For each animal object, the daily updates are called according to the order described in the \colorbox{red}{Day in the Life for Animal} on a RuFaS Farm. This is:
...
\vspace{-0.25cm}
    ...
    \begin{enumerate}
        ...
        At the beginning of the simulation, the herd is assigned distributions for the lactation curve parameters l, m, and n for each parity of cows. It calculates Wood’s lactation curve parameter mean values by parity, adjusted based on the location, production, management practices, +/- reported milk production of the farm being simulated. 
         ...
         These are comprehensively outlines in the \colorbox{red}{Lactation Curve Parameters Determination} Section.
        \vspace{0.1cm}
...
\(Calculated\ milk\ production = l*t^m *e^{[-nt]}\)
    ...
    \vspace{0.5cm}
        ...
        \begin{tcolorbox}[colback=white, colframe=black, boxrule=0.5pt, arc=3pt, width=\textwidth]
            For more information, refer to the \colorbox{red}{Reproduction} section of this document.
        \end{tcolorbox}
        \vspace{0.2cm}
    ...
    \vspace{0.2cm}
        ...
        \begin{tcolorbox}[colback=white, colframe=black, boxrule=0.5pt, arc=3pt, width=\textwidth]
            For more details on milk changes and reductions, go to the \colorbox{red}{Milk Production and Reduction} portion of this section. . 
        \end{tcolorbox}
...
\subsection{Lactation Curve: Parameters and Definitions} 
    ...
    This module is part of the Animal Module and integrates with the \colorbox{red}{Milk Production} calculations. The outputs influence various downstream models, including nutrient requirements, manure excretion, and greenhouse gas emission calculations.
...
\vspace{0.5cm}
    ...
    Equations below are not in the LactationCurve class, but they are useful to understanding the bigger picture of how lactation curve parameter estimates are generated in RuFaS. Please see the \colorbox{red}{Milk Production} section for more details on these equations \parencite{Wood1967}.
    \vspace{0.5cm}
...
\subsection{Manure Excretion}
    ...
    \begin{enumerate}
        \item  \colorbox{red}{General Animal Inputs}
        \item \colorbox{red}{Cow Specific Inputs} 
        \item \colorbox{red}{Phosphorus Specific Inputs}
    \end{enumerate}
