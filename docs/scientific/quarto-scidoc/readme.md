...
to use for the project (`bibliography`), defining which template files to use for the website (`format:html:css`) and for
the PDF (`format:pdf:include-in-header`).
...
* _book: the rendered output files created by `quarto`
