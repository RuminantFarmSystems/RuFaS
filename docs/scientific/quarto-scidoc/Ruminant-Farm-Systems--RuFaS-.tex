...
Our documentation include quite a few tables with long, detailed
definitions and extended variable names. It is important that these are
all legible and consistently formated for readability. This requires
precise alignment of gridlines-- which can get tedious. Additionally,
any updates to variables or defaults may make this process more time
consuming in the future.

Instead, we have four excel files that correspond to each existing
module. In each file, sheets are named by their label id (see next
section for more on id logic). That way, you can auto-generate a .txt
file with your table properly sized and formated using the
make\_grid\_table.py (quarto-scidoc\textgreater scripts). The .txt file
output will show up in
quarto-scidoc\textgreater output\textgreater tables.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{After you create your excel sheet in the module file of the data folder (in this example it is animal.xslx).}


\NormalTok{python scripts\textbackslash{}make\_grid\_table.py data\textbackslash{}animal.xlsx {-}{-}sheet "EXAMPLE" {-}{-}widths "32,55,10,6,24" {-}{-}caption "This is an example caption." {-}{-}label "tbl{-}example{-}inputs" {-}{-}output output\textbackslash{}tables\textbackslash{}example\_table\_file.txt}

\NormalTok{Remember, you will need to update }
\NormalTok{{-}{-}sheet (the sheet name should match the label)}
\NormalTok{{-}{-}caption (to relevant description)}
\NormalTok{{-}{-}label (follow label structure in next section)}
\NormalTok{{-}{-}output (file name only)}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.0621}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1862}}@{}}
\caption{This is an example
caption.}\label{tbl-example-inputs}\tabularnewline
\toprule\noalign{}
...
HeiferIII's and Dry Cows; LAC\_COWS = Lactating Cows &
...
\item
\end{itemize}
\end{minipage} & CALF, GROWING, CLOSE\_UP, LAC\_COWS \\
\end{longtable}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{The audit label and crosslinking is tracked by \#eq{-}an{-}nrc{-}2026. It is important to follow the custom label structure even for the invisible reference to make tracking easier. }

\NormalTok{The actual custom labeling is written as \textbackslash{}text}

\NormalTok{::: \{\#eq{-}an{-}nrc{-}2026\} }
\NormalTok{$$}
\NormalTok{CBW = MW * 0.06275 \textbackslash{}qquad \textbackslash{}text\{(AN.NRC.2026)\}}
\NormalTok{$$}
\NormalTok{:::}

\NormalTok{Below is how the equation can be linked in other parts of the text: }

\NormalTok{See }\CommentTok{[}\OtherTok{AN.NRC.2026}\CommentTok{](\#eq{-}an{-}nrc{-}2026)\textless{}!{-}{-} @eq{-}an{-}nrc{-}2026 {-}{-}\textgreater{}}\NormalTok{.}
\end{Highlighting}
\end{Shaded}

This is how your equation and the linking should appear.

\protect\phantomsection\label{eq-an-nrc-2026}
\[
CBW = MW * 0.06275 \qquad \text{(AN.NRC.2026)}
\]

See \hyperref[eq-an-nrc-2026]{AN.NRC.2026}.
...
{\centering \includegraphics[width=0.35\linewidth,height=\textheight,keepaspectratio]{resources/animal.png}
...
\tightlist
\item
  Lactating and Dry Cows ::: \{\#eq-an-nrc-1\} \[
  CBW = MW * 0.06275 \qquad \text{(AN.NRC.1)}
  \] :::
\end{itemize}

\protect\phantomsection\label{eq-an-nrc-2}
\[
...
\protect\phantomsection\label{eq-an-nrc-3}
\[
...
\tightlist
\item
  Heifers : The maintenance requirement is calculated based on metabolic
  body weight (body weight(\^{}\{0.75\})), body condition score, and the
  previous month's temperatures.
\end{itemize}

\protect\phantomsection\label{eq-an-nrc-1}
\[
CBW = MW * 0.06275 \qquad \text{(AN.NRC.1)}
\]
...
\protect\phantomsection\label{eq-an-nrc-6}
\[
BCS9 = (BCS5 - 1) * 2 + 1 \qquad \text{(AN.NRC.6)}
\]

\protect\phantomsection\label{eq-an-nrc-7}
\[
NEmaint = (BW - CW)^{0.75}\cdot (0.086\cdot (0.8 + (BCS9 - 5)\cdot 0.05) + 0.0007 (20 - PrevTemp)) \qquad \text{(AN.NRC.7)}
\]

\emph{Activity} Activity requirement is proportional to body weight and
daily walking distance. A grazing system and hilly topography will cost
additional energy.

\begin{itemize}
\tightlist
\item
  Lactating and Dry Cows ::: \{\#eq-an-nrc-8\} \[
