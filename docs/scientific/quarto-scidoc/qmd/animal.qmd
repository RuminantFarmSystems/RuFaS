...
for the Wood’s curve parameters l, m, and n for each parity of cows (see more information in the [Lactation Curve](#sec-an-lac-curve) section). At the beginning of each new lactation, each cow is assigned a set of parameters for that lactation based on the herd’s distributions for her current parity. 
...
### Lactation Curve: Parameters and Definitions{#sec-an-lac-curve}
...
* Death day probability: `death_day_prob`
   * Values to construct the CDF to determine future death day, given that death is going to occur 
   * Values in the array correspond to the days in `cull_day_count`
