...
Maintenance protein requirements are estimated as a function of animal bodyweight
excluding conceptus weight if pregnant, dry matter intake, and the amount of 
rumen bacterial protein supply $(\text{MPbact})$. 
...
Similar to energy, protein requirements for pregnancy are estimated as a function
of the days in pregnancy $(\text{DOP})$ and the calf birthweight $(\text{CBW})$. 
...
Estimations are different for lactating cows and dry cows. For details about the variables included in each calculation, please refer to @tbl-an-dmi-var. The sum of DMI of each feed is assumed to be less than the estimated DMI:
...
$$
\text{DMIest} = (\text{BW}^{0.75} \times \frac{(0.2435 \times \text{NEadj}) - (0.0466 \times \text{NEadj}^{2}) - 0.0869}{\text{NEadj}}) - \text{PREGadj}
$$
:::
...
$$
\text{NEmaint} = 0.10 \times (\text{MBW} - \text{GrUterW} - \text{UterW})^{0.75}
$$
:::
...
$$
\text{UterW} = (\text{CBW} \times 0.2288 - 0.204) \times e^{(-0.2 \times \text{DyLact})} + 0.204
$$
:::
...
\text{NEa1} =
\begin{cases}
0, & \text{if } \frac{\text{Dt\_PastIn}}{\text{DtDMIn}} < 0.005 \\[8pt]
\frac{\text{Dt\_PastIn}}{\text{DtDMIn}}, & \text{otherwise}
\end{cases}
$$
:::
...
\text{NEa1} =
\begin{cases}
0.0075 \times \text{BW}^{0.75} \times \frac{(600 - 12 \times \text{DtPastSupplIn})}{600},& \text{if } \frac{\text{Dt\_PastIn}}{\text{DtDMIn}} \geq 0.005 \\[8pt]
0, & \text{otherwise}
\end{cases}
$$
:::
