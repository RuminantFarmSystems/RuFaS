...
- `ration_nutrient_amount_for\_pen_[AnimalCombination].[nutrient]`: is reporting the amount of each of the following nutrients that is provided by the ration per animal in the pen (all reported in kilograms unless otherwise indicated).
...
* *Option 1:* The first and most commonly used option for grouping animals is to assign one `AnimalCombination` to each pen such that there is at least one pen for the 
four AnimalCombinations: `CALF`, `GROWING`, `CLOSE_UP`, and `LAC_COW`. 

* *Option 2:* In some cases, and especially in very small farms, a user might wish to represent a scenario where all growing animals, pregnant heifers, and non-lactating cows are housed together. In this case, there is an option to have a pen that houses both `GROWING` and `CLOSE_UP` animals such that the there is at least one pen with each of the three  AnimalCombination  inputs: `CALF`, `GROWING_AND_CLOSE_UP`, and `LAC_COW`. 
...
RuFaS needs a set of ingredients to work from. The RuFaS feed library should be consulted for the full list of ingredients that 
has been adapted from the NASEM and NRC feed libraries for use with these nutrient requirement models. Users interested in feeding a ration that they define, need to provide a list of ingredients 
...
If a user would like RuFaS to use the list of ingredients to formulate a least-cost ration, providing the proper costs for each 
ingredient listed is essential, but the % dry matter intake in the user defined ration percentages does not need to be included. Feed costs need to be entered in 
...
::: {.callout-note}
At this time, only one set of feeds and ration recipe can be provided for each animal class. 
:::
...
##### Energy Requirements
For details about the variables included in each calculation, please refer to @tbl-an-energy-nrc for a full list.

**Maintenance** <br>
The maintenance requirement is calculated based on metabolic body weight ($\text{BW}^{0.75}$).

* **Lactating and Dry Cows**

First the conceptus weight ($\text{CW}$) is estimated as a function of the expected calf
birth weight ($\text{CBW}$) which is estimated as fraction of the cow's mature bodyweight ($\text{MW}$): 
...
$$
\text{CW} = (18 + (\text{DOP} - 190) \times 0.665) \times \left(\frac{\text{CBW}}{45}\right),  \text{ if DOP} \geq 190.0 
$$
$$
\text{CW} = 0, \text{ if DOP} < 190.0 
$$
:::


Then maintenance energy requirements are estimated as a function of the metabolic bodyweight without the conceptus weight. 
...
The maintenance requirement is calculated based on metabolic body weight $(\text{BW}^{0.75})$, body condition score, and the previous month's temperatures.

First the body condition score must be translated to a 9 point scale $(\text{BSC9})$. The conceptus weight $(\text{CW})$ is calculated the same as for lactating and dry cows.
...
$$
\text{NEmaint} = (\text{BW} - \text{CW})^{0.75}\times (0.086\times (0.8 + (\text{BCS9} - 1)\times 0.05) + 0.0007 \times (20 - \text{PrevTemp})) 
$$
:::

**Activity** <br>
Activity requirement is proportional to body weight and daily walking distance in kilometers. A grazing system and hilly topography will cost additional energy. 
...
$$
\text{NEa1} = 0.0012\times \text{BW}, \qquad \text{if Housing = Grazing}
$$
$$
\text{NEa1} = 0.0,  \qquad \text{if Housing } \neq \text{Grazing}
$$
...
$$
\text{NEa2} = (0.00045 \times \text{Distance} \times \text{BW}) + \text{NEa1}
$$
:::
...
* **Lactating and Dry Cows**
Cows will continue to grow toward their mature body weight until the end of their second lactation. Their growth rate is estimated as a linear increase to reach the expected proportion of their mature bodyweight $\text{MW}$ at the beginning of their next parity. 


First the shrunk bodweight $(\text{SBW})$, the mature shrunk bodyweight $(\text{MSBW})$ and the equivalent shrunk bodyweight are calculated $(\text{EQSBW})$ to remove the effect of gut fill on bodyweight, the conceptus, and relate the animal's weight to the reference animal of 478 kg. 
...
Next the average daily gain $(\text{ADG})$, is calculated as the difference between the target fraction of $(\text{MSBW})$ at the start of the current parity and the start of the next parity divided by the calving interval ($(\text{CI})$). Specifically, cows are expected to be 82%, 92% and 100% of their $(\text{MSBW})$ at the start of their 1st, 2nd, and 3rd parities. 
...
Finally, the expected $(\text{ADG})$ and $(\text{EQSBW})$ are adjusted for the weight associated with internal organs to get the equivalent empty bodyweight gain $(\text{EQEBG})$ and the equivalent empty body weight $(\text{EQEBW})$ which is then used to estimate the net energy for gain.
...
Heifer growth is estimated according to a similar principal as lactating cow growth. Growth targets for heifers are set based on their expected weights at the start of breeding (55% of $\text{MSBW}$) and age at first calving (82% of $(\text{BSC9})$ which is equivalent to the start of parity 1). 
...
\text{ADG} =
\begin{cases}
\dfrac{0.55 \times \text{MSBW} - \text{SBW}}{(\text{Age1stBred} - \text{Age}) \times 30.4}, & \text{before breeding} \\[10pt]
\dfrac{0.82 \times \text{MSBW} - \text{SBW}}{(\text{Age1st} - \text{Age}) \times 30.4},& \text{after breeding}
\end{cases}
$$
:::
...
The energy requirements for pregnancy are based on the days in pregnancy $(\text{DOP})$ and calf birth weight $(\text{CBW})$ and only applies when the $\text{DOP}$ advanced beyond 190 days. 

:::{#eq-an-nrc-25}
[[**AN.NRC.25**]]{.aside .content-visible when-format="html"}
$$
\text{MEpreg} = (2 \times 0.00159 \times \text{DOP} - 0.0352) \times \frac{\text{CBW}}{45} \times 0.14, \qquad\text{if DOP > 190}
$$
:::
...
Energy requirements for lactation are based on the total fat, true protein and
lactose produced. First the energy required per kg of milk $(\text{Milk}_{\text{Energy}})$ is estimated based on the milk fat, protein, and lactose composition. 

:::{#eq-an-nrc-27}
[[**AN.NRC.27**]]{.aside .content-visible when-format="html"}
$$
\text{Milk}_{\text{Energy}} = (0.0929 \times \text{Fat\_Milk}) + (\frac{0.0563}{0.93} \times \text{TP\_Milk}) + (0.0395 \times \text{Lactose\_Milk})
$$
:::

Then the total net energy for lactation is estimated by multiplying the $\text{Milk}_{\text{Energy}}$ by the total milk produced $(\text{MilkYeild, kg})$. 

:::{#eq-an-nrc-28}
[[**AN.NRC.28**]]{.aside .content-visible when-format="html"}
$$
\text{NElact} = \text{Milk}_{\text{Energy}} \times \text{MilkYeild}
$$
:::
...
**Total Protein** <br>
Reporting of protein requirements is divided into four components: maintenance $(\text{MPm})$, growth $(\text{MPg})$, pregnancy $(\text{MPpreg})$, and lactation $(\text{MPlact})$ (all in metabolizable protein). For details about the variables included in each calculation, please refer to the protein requirements key.
...
Maintenance protein requirements are estimated as a function of animal bodyweight excluding conceptus weight if pregnant, dry matter intake, and the amount of  rumen bacterial protein supply $(\text{MPbact})$. 

:::{#eq-an-nrc-29}
[[**AN.NRC.29**]]{.aside .content-visible when-format="html"}
$$
\begin{aligned}
\text{MPm} &= (0.3 \times (\text{BW} - \text{CW})^{0.6}) + (4.1 \times (\text{BW} - \text{CW})^{0.5}) + \\[8pt]
& \qquad (\text{DMI} \times 1000 \times 0.03 - 0.5 \times (\frac{\text{MPbact}}{0.8} - \text{MPbact})) + (0.4 \times 11.8 \times \frac{\text{DMI}}{0.67})
\end{aligned}
$$
:::
...
Protein requirements for growth are a function of average daily gain $(\text{ADG})$.
...
\begin{cases}
0, & \text{if ADG = 0} \\[8pt]
\text{ADG} \times (268 - 29.4 \times \frac{\text{NEg}}{\text{ADG}}),& \text{otherwise}
\end{cases}
$$
:::
...
\text{EffMP\_NPg} =
\begin{cases}
\dfrac{83.4 - (0.114 \times \text{EQSBW})}{100},& \text{if EQSBW} \leq 478 \\[8pt]
0.28908,& \text{otherwise}
\end{cases}
$$
:::
...
\text{EffMP\_NPg} =
\begin{cases}
\frac{83.4 - (0.114 \times \text{EQSBW})}{100}, & \text{if EQSBW} \leq 478 \\[8pt]
0.28908, & \text{otherwise}
\end{cases}
$$
:::
...
Similar to energy, protein requirements for pregnancy are estimated as a function of the days in pregnancy $(\text{DOP})$ and the calf birthweight $(\text{CBW})$. 

:::{#eq-an-nrc-37}
[[**AN.NRC.37**]]{.aside .content-visible when-format="html"}
$$
\text{MPpreg} = (0.69 \times \text{DOP} - 69.2) \times \frac{\text{CBW}}{45 \times 0.33}, \qquad \text{if DOP > 190}
$$
:::

**Lactation** <br>

Protein requirements for lactation are a function of the protein secreted in milk.

:::{#eq-an-nrc-38}
[[**AN.NRC.38**]]{.aside .content-visible when-format="html"}
$$
\text{MPlact} = \text{MilkYield} \times \frac{\text{TP}_{\text{Milk}}}{100} \times \frac{1000}{0.67}
$$
:::
...
Only calcium (Ca) and phosphorus (P) requirements are considered currently. For details about the variables included in each calculation, please refer to the @tbl-an-min-req-var.
...
\text{Ca}_{\text{main}} =
\begin{cases}
(0.031 \times \text{BW}) + (0.08 \times \dfrac{\text{BW}}{100}), & \text{for lactating cows} \\[8pt]
(0.0154 \times \text{BW}) + (0.08 \times \dfrac{\text{BW}}{100}), & \text{for dry cows}
\end{cases}
$$
:::
...
\text{P}_{\text{main}} =
\begin{cases}
(1 \times \text{DMI}) + (0.002 \times \text{BW}), & \text{for lactating cows} \\[8pt]
(0.8 \times \text{DMI}) + (0.002 \times \text{BW}), & \text{for dry cows}
\end{cases}
$$
:::
...
$$
\text{Ca}_{\text{main}} = (0.0154 \times \text{BW}) + (0.08 \times \frac{\text{BW}}{100})
$$
:::
...
$$
\text{P}_{\text{main}} = (0.8 \times \text{DMI}) + (0.002 \times \text{BW})
$$
:::
...
$$
\text{Ca}_{\text{growth}} = 9.83 \times \text{MW}^{0.22} \times \text{BW}^{-0.22} \times \frac{\text{ADG}}{0.96}
$$
:::
...
$$
\text{P}_{\text{growth}} = (1.2 + 4.635 \times \text{MW}^{0.22} \times \text{BW}^{-0.22}) \times \frac{\text{ADG}}{0.96}
$$
:::
...
\text{Ca}_{\text{preg}} =
\begin{cases}
0.02456 \times e^{0.05581 - (0.00007 \times \text{DOP}^2)} - 0.02456 \times e^{0.05581 - (0.00007 \times \text{-DOP} - 1)}, & \text{if DOP > 190} \\[8pt]
0, & \text{otherwise}
\end{cases}
$$
:::
...
$$
\text{P}_{\text{preg}} = \begin{cases} 
0.02743 \times e^{(0.05527 - 0.000075 \times \text{DOP}^2)} - 0.02743 \times e^{(0.05527 - 0.000075 \times (\text{DOP} - 1)^2)},& \text{if DOP > 190} \\[8pt]
0, & \text{otherwise}
\end{cases}
$$
:::
...
$$
\text{Ca}_{\text{lact}} = 1.22 \times \text{MilkYield}
$$
:::
...
$$
\text{P}_{\text{lact}} = 0.9 \times \text{MilkYield}
$$
:::
...
$$
\text{Ca}_{\text{req}} = \text{Ca}_{\text{main}} + \text{Ca}_{\text{growth}} + \text{Ca}_{\text{preg}} + \text{Ca}_{\text{lact}}
$$
:::
...
$$
\text{P}_{\text{req}} = \text{P}_{\text{main}} + \text{P}_{\text{growth}} + \text{P}_{\text{preg}} + \text{P}_{\text{lact}}
$$
:::
...
Methods for estimating DMI are different for lactating cows and dry cows. For details about the variables included in each calculation, please refer to @tbl-an-dmi-var. The sum of DMI of each feed is assumed to be less or equal to than the estimated DMI:

$$
\sum_{i} \text{Feed}_{i} \le \text{DMIest}
$$

**Lactating Cows** <br>

For lactating cows, the estimated DMI $(\text{DIMest})$ is calculated as a function of the fat corrected milk $(\text{FCM})$, bodyweight $(\text{BW})$, and week of lactation $(\text{WOL})$. 

:::{#eq-an-nrc-51}
[[**AN.NRC.51**]]{.aside .content-visible when-format="html"}
$$
\text{FCM} = (0.4 \times \text{Milk}) + (15 \times \text{Fat\_Milk} \times \frac{\text{Milk}}{100})
$$
:::
...
$$
\text{DMIest} = (0.372 \times \text{FCM} + 0.0968 \times \text{BW}^{0.75}) \times (1 - e^{(-0.192 \times \text{WOL} + 3.67)})
$$
:::

**Dry Cows** <br>

For dry cows, $\text{DIMest}$ is a function of day of pregnancy $(\text{DOP})$ and $\text{BW}$.

:::{#eq-an-nrc-53}
[[**AN.NRC.53**]]{.aside .content-visible when-format="html"}
$$
\text{DMIest} = (\frac{1.97 - 0.75 \times e^{0.16 \times (\text{DOP} - 280)}}{100}) \times \text{BW}
$$
:::

**Heifers** <br>

* If a heifer is *LESS* than 365 days old her $\text{DIMest}$ is calculated as a function of her metabolic bodyweight $(\text{BW}^{0.75})$ with an adjustment for the energy concentration of the diet:

:::{#eq-an-nrc-54}
[[**AN.NRC.54**]]{.aside .content-visible when-format="html"}
$$
\text{DMIest} = (\text{BW}^{0.75} \times \frac{(0.2435 \times \text{NEadj}) - (0.0466 \times \text{NEadj}^{2}) - 0.0869}{\text{NEadj}})
$$
:::

* If the heifer is *MORE* than 365 days old, a separate intercept is used and adjustment is also made for the pregnacy status if she is greater than 210 $\text{DOP}$:

:::{#eq-an-nrc-55}
[[**AN.NRC.55**]]{.aside .content-visible when-format="html"}
$$
\text{DMIest} = (\text{BW}^{0.75} \times \frac{(0.2435 \times \text{NEadj}) - (0.0466 \times \text{NEadj}^{2}) - 0.1128}{\text{NEadj}}) - \text{PREGadj}
$$
:::
...
##### Energy Requirements
The maintenance requirement is calculated based on metabolic body weight (MBW, body weight $^{0.75}$) adjusted for the tissues associated with pregnancy. For details about the variables included in each calculation, please refer to @tbl-an-energy-nasem.

**Maintenance** <br>

* Lactating and Dry Cows

For Lactating cows, the body weight is adjusted both for the weight of the gravid uterine tissues $(\text{GrUterW})$ and the uterine tissues $(\text{UterW})$ for normal activity in confinement conditions; otherwise include adjustments for activity requirements under grazing conditions. 

:::{#eq-an-nsm-1}
[[**AN.NSM.1**]]{.aside .content-visible when-format="html"}
$$
\text{NEmaint} = 0.10 \times (\text{BW} - \text{GrUterW} - \text{UterW})^{0.75}
$$
:::
...
$$
\text{GrUterW} = (\text{CBW} \times 1.825) \times e^{(-0.0243 - (0.0000245 \times \text{DOP}) \times (280 - \text{DOP}))}
$$
:::
...
$$
\text{UterW} = (\text{CBW} \times 0.2288 - 0.204) \times e^{(-0.2 \times \text{DIM})} + 0.204
$$
:::
...
$$
\text{NEmain} = 0.10 \times (\text{BW} - \text{GrUterW})^{0.75}
$$
:::

**Activity** <br>
Activity energy requirements are the sum of energy required for grazing activity, walking to the parlor, and for gaining elevation. 

If the DMI from pasture $(\text{DMI}_{Pasture})$ is greater that 0.5% of the total DMI $(\text{DMI}_{Total})$, then the energy requirements for grazing activities are a function of $\text{BW}$ and the amount of supplemental feed that is provided $(\text{DMI}_{Supp})$.
...
\text{NEa1} =
\begin{cases}
0.0075 \times \text{BW}^{0.75} \times \frac{(600 - 12 \times \text{DMI}_{Supp})}{600},& \text{if } \frac{\text{DMI}_{Pasture}}{\text{DMI}_{Total}} \geq 0.005 \\[8pt]
0 & \text{otherwise}
\end{cases}
$$
:::
...
$$
\text{NEa2} = 0.00035 \times \frac{\text{DistParlor}}{1000} \times \text{TripParlor} \times \text{BW}
$$
:::

The energy required for an animal to gain elevation is calculated as a function of $\text{BW}$ and the gain in elevation:

:::{#eq-an-nsm-9}
[[**AN.NSM.9**]]{.aside .content-visible when-format="html"}
$$
\text{NEa3} = 0.0067 \times \frac{\text{EnvTopoParlor}}{1000} \times \text{BW}
$$
:::
...
$$
\text{NEa} = \text{NEa1} + \text{NEa2} + \text{NEa3}
$$
:::

**Growth** <br>
In @nrc_2021, body frame gain (fat + protein) corresponds to the true growth and it is part of the calculation which is further partitioned into body reserves or condition gain (or loss), and pregnancy-associated gain (considered a pregnancy requirement).

First the empty bodyweight grain $(\text{EBG})$ is estimated from the $\text{ADG}$ which is calculated as described in [AN.NRC.17](#eq-an-nrc-17) and [AN.NRC.21](#eq-an-nrc-21). 
...
Then amount of fat $(\text{FatADG})$ and lean tissue $(\text{ProtADG})$ daily gain is estimated from the $(\text{EBG})$.

:::{#eq-an-nsm-13}
[[**AN.NSM.13**]]{.aside .content-visible when-format="html"}
$$
\text{FatADG} = (0.067 + 0.375 \times \frac{\text{BW}}{\text{MW}}) \times \frac{\text{EBG}}{\text{ADG}}
$$
:::
...
$$
\text{ProtADG} = (0.201 + 0.081 \times \frac{\text{BW}}{\text{MW}}) \times \frac{\text{EBG}}{\text{ADG}}
$$
:::

Finally the amount of energy associated with the energy in the $(\text{FatADG})$ and $(\text{ProtADG})$ as retained energy $(\text{REFADG})$ and the amount of metabolizable $(\text{MEFrameADG})$ and net $(\text{NElFrameADG})$ energy needed for frame grown is estimated.

:::{#eq-an-nsm-15}
[[**AN.NSM.15**]]{.aside .content-visible when-format="html"}
$$
\text{REFADG} = (9.4 \times \text{FatADG}) + (5.55 \times \text{ProtADG})
$$
:::
...
$$
\text{MEFrameADG} = \frac{\text{REFADG}}{0.4}
$$
:::
...
$$
\text{NElFrameADG} = \frac{\text{REFADG}}{0.61}
$$
:::

**Pregnancy** <br>
The daily rate of gravid uterine tissue deposition ($\text{GrUterWGain}$, kg/d) is estimated from the current gravid uterine weight $(\text{GrUterW})$ estimated in equations [AN.NSM.2](#eq-an-nsm-2) and [AN.NSM.3](#eq-an-nsm-3) as defined above. Then the energy required to support this weight gain is estimated. 
...
$$
\text{GrUterWGain} = (0.0243 - (0.0000245 \times \text{DayGest})) \times \text{GrUterW}
$$
:::
...
\text{MEpreg} =
\begin{cases}
(2 \times 0.00159 \times \text{DOP} - 0.0352) \times \frac{\text{CBW}}{45} \times 0.14, & \text{if DOP > 190} \\[8pt]
0, & \text{otherwise}
\end{cases}
$$
:::
...
$$
\text{MilkEn} = (0.0929 \times \text{Fat}_{Milk}) + (0.0547 \times \text{Prot}_{Milk}) + (0.0395 \times \text{Lact}_{Milk})
$$
:::
...
$$
\text{MilkEn} = (0.0929 \times \text{Fat}_{Milk}) + (0.0585 \times \text{TP}_{Milk}) + (0.0395 \times \text{Lact}_{Milk})
$$
:::

##### Protein Requirements
These are divided into 4 components: maintenance, growth, pregnancy, and lactation (all in Metabolizable Protein or MP). Requirements for EEA are also included in @nrc_2021 but have not yet been implemented.

The requirements for each physiological function are first quantified in terms of the net TP (true protein), instead of CP (crude protein).

To convert both net TP and net amino acid values to a metabolizable basis, net values are divided by the calculated efficiencies of use for each physiological function. For simplicity, fixed efficiences were applied. For details about the variables included in each calculation, please refer to @tbl-an-nasem-prot-req-var.

**Maintenance** <br>
The protein requirements for non-productive functions include scurf,  endogenous urinary loss, and metabolic fecal protein.
...
$$
\text{NPscurf} = 0.20 \times \text{BW}^{0.60} \times 0.85
$$
:::
...
$$
\text{NetAAscurf} = \text{NPscurf} \times \frac{\text{AACorrScurf}}{100}
$$
:::
...
$$
\text{NPEndUrin} = 53 \times 6.25 \times \text{BW} \times 0.001
$$
:::
...
$$
\text{NetAAEndUrin} = 0.010 \times 6.25 \times \text{BW} \times \frac{\text{AACorrWhEmpBody}}{100}
$$
:::
...
$$
\text{CPMFP} = (11.62 + 0.134 \times \text{NDF}) \times \text{DMI}
$$
:::

The CP lost as MFP is assumed to be 73% TP:

:::{#eq-an-nsm-28}
[[**AN.NSM.28**]]{.aside .content-visible when-format="html"}
$$
\text{NPMFP} = \text{CPMFP} \times 0.73
$$
:::
...
$$
\text{NetAAMFP} = \text{NPMFP} \times \frac{\text{AACorrMFP}}{100}
$$
:::

**Growth**

Two distinct growth processes are represented: frame growth and BW changes related to the deposition (and mobilization) of body reserves.

The following are the protein requirements for growth associated with increased frame size, where $\text{FrameWG}$ is the sum of $\text{FatADG}$ and $\text{ProtADG}$ as described in [AN.NSM.13](#eq-an-nsm-13) and [AN.NSM.14](#eq-an-nsm-14):

:::{#eq-an-nsm-30}
[[**AN.NSM.30**]]{.aside .content-visible when-format="html"}
$$
\text{NPGrowth}_{\text{Frame}} = \text{FrameWG} \times 0.11 \times 0.86
$$
:::

If the change in BW is not frame growth but rather a change in body reserves during lactation, then the following equation is used because the protein content is assumed to be 8.0 percent protein where the $\text{TissueWG}$ is drawn from equations described in [AN.BWT.18](#eq-an-bwt-18), [AN.BWT.19](#eq-an-bwt-19), and [AN.BWT.20](#eq-an-bwt-20).

:::{#eq-an-nsm-31}
[[**AN.NSM.31**]]{.aside .content-visible when-format="html"}
$$
\text{NPGrowth}_{\text{Tissue}} = \text{TissueWG} \times 0.08 \times 0.86
$$
:::
...
$$
\text{NetAAGrowth} = \text{NPGrowth} \times \frac{\text{AACorrWEBW}}{100}
$$
:::

**Pregnancy**

The protein requirements for pregnancy are based on the gravid uterine weight gain estimated in [AN.NSM.2](#eq-an-nsm.2), assuming 125 g of protein per kg of weight gain:

:::{#eq-an-nsm-33}
[[**AN.NSM.33**]]{.aside .content-visible when-format="html"}
$$
\text{NPGest} = \text{GainGrUter} \times 125
$$
:::
...
$$
\text{NetAAGest} = \text{NPGest} \times \frac{\text{AACorrWEBW}}{100}
$$
:::
...
$$
\text{NetAAMilk} = \text{NPMilk} \times \frac{\text{AAcalcMilk}}{100}
$$
:::

**Total Requirements**

The total MP requirements are the sum of the NP requirements divided by the relevant efficiency of conversion of MP to NP for each physiological process:
...
$$
\begin{aligned}
\text{MPReq\_Lac} &= \frac{\text{NPScurf} + \text{NPMFP} + \text{NPMilk} + \text{NPGrowth}}{\text{EffMP}} \\[8pt]
& \qquad + \frac{\text{NPGest}}{0.33} + \text{NPEndUrin}
\end{aligned}
$$
:::
...
$$
\begin{aligned}
\text{MPAAReq\_Lac} &= \frac{\text{NetAAScurf} + \text{NetAAMFP} + \text{NetAAMilk} + \text{NetAAGrowth}}{\text{EffAA}} \\[8pt]
& \qquad + \frac{\text{NetAAGest}}{0.33} + \text{NetAAEndUrin}
\end{aligned}
$$
:::
...
$$
\text{MPReq\_NonLac} = \frac{\text{NPScurf} + \text{NPMFP}}{\text{EffMP}} + \frac{\text{NPGest}}{0.33} + \frac{\text{NPGrowth}}{0.4} + \text{NPEndUrin}
$$
:::
...
$$
\begin{aligned}
\text{MPAAReq\_NonLac} &= \frac{\text{NetAAScurf} + \text{NetAAMFP}}{\text{NetAAMFP}} + \frac{\text{NetAAGest}}{0.33} + \frac{\text{NetAAGrowth}}{0.4} \\[8pt]
& \qquad + \text{NetAAEndUrin}
\end{aligned}
$$
:::
...
##### Mineral Requirements
Only calcium and phosphorus requirements are included in the code at present. For details about the variables included in each calculation, please refer to the @tbl-an-min-req-var.

**Maintenance** <br>

* Calcium maintenance requirements are based on DMI:

:::{#eq-an-nsm-40}
[[**AN.NSM.40**]]{.aside .content-visible when-format="html"}
$$
\text{Ca}_{\text{main}} = 0.90 \times \text{DMI}
$$
:::

* Phosphorus maintenance requirements are based on DMI and BW:

:::{#eq-an-nsm-41}
[[**AN.NSM.41**]]{.aside .content-visible when-format="html"}
$$
\text{P}_{\text{main}} = 1.0 \times \text{DMI} + 0.0006 \times \text{BW}
$$
:::

**Growth** <br>

* Calcium requirements for growth are a function of the mature bodyweight, current bodyweight and ADG:

:::{#eq-an-nsm-42}
[[**AN.NSM.42**]]{.aside .content-visible when-format="html"}
$$
\text{Ca}_{\text{growth}} = 9.83 \times \text{MW}^{0.22} \times \text{BW}^{-0.22} \times \text{ADG}
$$
:::

* Phosphorus growth requirements are also a function of mature bodyweight, current bodyweight and ADG:

:::{#eq-an-nsm-43}
[[**AN.NSM.43**]]{.aside .content-visible when-format="html"}
$$
\text{P}_{\text{growth}} = 1.2 + (4.635 \times \text{MW}^{0.22} \times \text{BW}^{-0.22}) \times \text{ADG}
$$
:::

**Pregnancy** <br>

Calcium and Phosphorus requirements for pregnancy are only considered after 190 days in pregnancy and are both estimated as a function of bodyweight and days in pregnanchy.
...
\text{Ca}_{\text{preg}} =
\begin{cases}
(0.02456 \times e^{(0.05581 - 0.00007 \times \text{DOP}) \times \text{DOP}}) \\[8pt]
\qquad - (0.02456 \times e^{0.05581 - (0.00007 \times (\text{DOP} - 1)) \times (\text{DOP} - 1)} \times \dfrac{\text{BW}}{715}), & \text{if DOP > 190} \\[8pt] 
0, & \text{otherwise}
\end{cases}
$$
:::
...
\text{P}_{\text{preg}} =
\begin{cases}
(0.02743 \times e^{(0.05527 - 0.000075 \times \text{DOP}) \times \text{DOP}}) \\[8pt]
\qquad - (0.02743 \times e^{(0.05527 - 0.000075 \times (\text{DOP} - 1)) \times (\text{DOP} - 1)} \times \frac{\text{BW}}{715}), & \text{if DOP > 190} \\[8pt]
0, & \text{otherwise}
\end{cases}
$$
:::

The denominator, 715 kg, is the average cow weight in the study by @house1993; therefore, the pregnancy (gestation) requirement is scaled to that BW. For details see Page 107 in the Minerals chapter of [@nrc_2021]. 

**Lactation** <br>

Calcium and Phosphorus requirements for lactation are a function of milk production and milk TP content:
...
$$
\text{Ca}_{\text{lact}} = \text{Milk} \times (0.295 + 0.239 \times \text{MilkTP}) 
$$
:::
...
$$
\text{P}_{\text{lact}} = \text{Milk} \times (0.496 + 0.13 \times \text{TPMilk})
$$
:::
...
$$
\text{P}_{\text{lact}} = \text{P}_{\text{main}} + \text{P}_{\text{growth}} + \text{P}_{\text{preg}} + \text{P}_{\text{lact}}
$$
:::
...
##### DMI Estimation
Dry matter intake estimation is different for lactating cows and dry cows. For details about the variables included in each calculation, please refer to @tbl-an-nasem-dmi-var. The sum of the dry matter intake of each feed is assumed to be less than the dry matter intake estimation:
...
$$
\begin{aligned}
\text{DMIest} &= ((3.7 + \text{Parity} \times 5.7) + (0.305 \times \text{MilkE}) + (0.022 \times \text{BW}) \\[8pt]
& \qquad + (-0.689 - 1.87 \times \text{Parity}) \times \text{BCS}) \\[8pt]
& \qquad \times (1- (0.212 + \text{Parity} \times 0.136) \times e^{(-0.053 \times \text{DIM})})
\end{aligned}
$$
:::
...
$$
\begin{aligned}
\text{DMIest} &= (0.0226 \times \text{MatBW} \times(1 - e^{(-1.47 \times \frac{\text{BW}}{\text{MatBW}})}) \\[10pt]
& \qquad - (0.082 \times (\text{NDF} - (23.1 + 56 \times \frac{\text{BW}}{\text{MatBW}} - 30.6 \times (\frac{\text{BW}}{\text{MatBW}})^2)))
\end{aligned}
$$
:::
...
The methods for estimating the amino acids requirements for lactating cows based on the @nrc_2021, in RuFaS are described below.
...
$$
\text{NetAAscurf}_{i} = \text{NPscurf} \times \frac{\text{AACorrScurf}_{i}}{100}
$$

* Endogenous Urinary Protein

$$
\text{NetAAEndUrine}_{i} = 0.010 \times 6.25 \times \text{BW} \times \frac{\text{AACorrWholeEmptyBody}_{i}}{100}
$$

* Metabolic Fecal Protein

$$
\text{NetAAMFP}_{i} = \text{NPMFP} \times \frac{\text{AACorrMFP}_{i}}{100}
$$

**Growth** 

$$
\text{NetAAGrowth}_{i} = \text{NPGrowth} \times \frac{\text{AACorrWholeEmptyBody}_{i}}{100}
$$

**Pregnancy** 

$$
\text{NetAAGest}_{i} = \text{NPGest} \times \frac{\text{AACorrWholeEmptyBody}_{i}}{100}
$$

**Lactation** 

$$
\text{NetAAMilk}_{i} = \text{NPMilk} \times \frac{\text{AAcalcMilk}_{i}}{100}
$$

**Total Requirements**

Here are the equations for calculating the total $\text{AA}_{\text{i}}$ requirements for lactating cows, late-gestation cows, and heifers. 
...
$$
\begin{aligned}
\text{TotalAArequirements}_{i} & = \frac{\text{NetAAscurf}_{i}+\text{NetAAMFP}_{i}+\text{NetAAGrowth}_{i}+\text{NetAAMilk}_{i}}{\text{TargetEfficiencyAA}_{i}} \\[10pt]
& \qquad +\frac{\text{NetAAGest}_{i}}{\text{TargetEfficiencyGest}}+\text{NetAAEndUrine}_{i}
\end{aligned}
$$
...
$$
\begin{aligned}
\text{TotalAArequirements}_{i} &= \frac{\text{NetAAscurf}_{i}+\text{NetAAMFP}_{i}}{\text{TargetEfficiencyAA}_{i}} + \frac{\text{NetAAGrowth}_{i}}{\text{TargetEfficiencyGrowth}} \\[10pt]
& + \frac{\text{NetAAGest}_{i}}{\text{TargetEfficiencyGest}} + \text{NetAAEndUrine}_{i}
\end{aligned}
$$
...
::: {.callout-note}
Variables marked with * are from @nrc_2021.
:::
...
Energy supply from the feed includes NEm, NEl, and NEg. NEm provides energy for maintenance and activity requirements, NEl provides energy for 
lactation and pregnancy, and NEg provides energy for growth. Feed actual energy contents are corrected from standard values provided by @nrc_2001. 
Minerals do not provide any energy.

```{python}
#| label: tbl-an-energy-nrc
#| tbl-cap: Key variables used in calculation of net energy in NRC (2001).
import_table(
  "../resources/table_data/animal/tbl-an-energy-nrc.csv",
  colalign = ["left", "left", "center"]
)
```
...
:::{#eq-an-sup-1-1}
[[**AN.SUP.1.1**]]{.aside .content-visible when-format="html"}
$$
\text{TDNconc} = \frac{\text{TotalTDN}}{\text{DMI}} \times 100
$$
:::

:::{#eq-an-sup-2-1}
[[**AN.SUP.2.1**]]{.aside .content-visible when-format="html"}
$$
\text{DMI\_to\_maint} = 1, if \text{TotalTDN} < 0.035 \times \text{BW}^{0.75}, \frac{\text{TotalTDN}}{0.035 \times \text{SBW}^{0.75}}
$$
:::
...
:::{#eq-an-sup-3-1}
[[**AN.SUP.3.1**]]{.aside .content-visible when-format="html"}
$$
\text{Discount} = 1, \quad if \quad \text{TDNconc} < 60 \frac{\text{TDNconc} - (0.18 \times \text{TDNconc} - 10.3) \times (\text{DMI}_{\text{to,maint}} - 1)}{\text{TDNconc}}
$$
:::

:::{#eq-an-sup-4-1}
[[**AN.SUP.4.1**]]{.aside .content-visible when-format="html"}
$$
\text{TDNact}_{\text{i}} = \text{TDN}_{\text{i}} \times \text{Discount}
$$
:::
...
:::{#eq-an-sup-4}
[[**AN.SUP.4**]]{.aside .content-visible when-format="html"}
$$
\text{DEact}_{\text{i}} = \text{DE}_{\text{i}} \times \text{Discount}
$$
:::
...
:::{#eq-an-sup-5-1}
[[**AN.SUP.5.1**]]{.aside .content-visible when-format="html"}
$$
\text{MEacti} =
\begin{cases}
(1.01 \times \text{DEacti}) + (0.0046 \times 0.0046 \times \text{EEi}) - 3,& \text{if feed type is not fat and fat content is} \geq3\% \\[8pt]
(1.01 \times \text{DEacti}) - 0.45,& \text{if feed type is not fat and fat content is} \leq3\% \\[8pt]
(1.01 \times \text{DEacti}),& \text{if feed type is fat}
\end{cases}
$$
:::
...
:::{#eq-an-sup-7-1}
[[**AN.SUP.7.1**]]{.aside .content-visible when-format="html"}
$$
\text{NEmacti} =
\begin{cases}
(1.37 \times \text{MEacti}) - (0.138 \times \text{MEacti}^2) + (0.0105 \times \text{MEacti}^3) - 1.12 ,& \text{if feed type is not fat} \\[8pt]
(0.8 \times \text{MEacti}) ,& \text{if feed type is fat} 
\end{cases}
$$
:::
...
:::{#eq-an-sup-6-1}
[[**AN.SUP.6.1**]]{.aside .content-visible when-format="html"}
$$
\text{NElacti}=
\begin{cases}
(0.703 \times \text{MEacti}) - 0.019 + (0.097 \times \text{MEacti}) + (0.1997 \times \text{EEi}) - 3 ,& \text{if feed type is not fat and fat content is} \geq3\% \\[8pt]
(0.703 \times \text{MEacti}) - 0.19 ,& \text{if feed type is not fat and fat content is} \leq3\% \\[8pt]
(0.8 \times \text{0DEacti}) ,& \text{if feed type is fat}
\end{cases}
$$
:::
...
:::{#eq-an-sup-8-1}
[[**AN.SUP.8.1**]]{.aside .content-visible when-format="html"}
$$
\text{NEgacti}
=
\begin{cases}
(1.42 \times \text{MEacti}) - (0.174 \times \text{MEacti}^2) + (0.0122 \times \text{MEacti}^3) - 1.65 ,& \text{if feed type is not fat} \\[8pt]
(0.55 \times \text{MEacti}) ,& \text{if feed type is fat} 
\end{cases}
$$
:::
...
Energy supply in the updated 2021 Nutrient Requirement model [@nrc_2021] is estimated based only on the NEl and follows a simplified estimation method when compared 
to the 2001 model [@nrc_2001].
...
:::{#eq-an-sup-1-2}
[[**AN.SUP.1.2**]]{.aside .content-visible when-format="html"}
$$
\begin{aligned}
\text{DE}_i &= (0.042 \times \text{NDF}_{\text{i}} \times \text{dNDF}_{\text{i}}) + (0.0423 \times \text{Starch}_{\text{i}} \times \text{dStarch}_{\text{i}}) \\[8pt]
& \qquad + (0.0940 \times \text{FA}_i \times \text{dFA}_{\text{i}}) + (0.0565 \times (\text{RDP}_{\text{i}} + (\text{RUP}_{\text{i}} \times \text{dRUP}_{\text{i}})- \text{NPN}_{\text{supp, i}})) \\[8pt]
& \qquad  + (0.0089 \times \text{NPN}_{\text{supp, i}}) + (0.040 \times \text{ROM}_{\text{i}} \times \text{dROM}_{\text{i}}) - 0.318
\end{aligned}
$$
:::
    
Parameter definitions and units are provided in @tbl-an-energy-nasem. 

```{python}
#| label: tbl-an-energy-nasem
#| tbl-cap: Key variables used in calculation of net energy in NASEM (2021).
import_table(
  "../resources/table_data/animal/tbl-an-energy-nasem.csv",
  colalign = ["left", "left", "center"]
)
```

The nutrient compositions of each feed ($\text{NDF}_\text{i}$, $\text{Starch}_\text{i}$, $\text{EE}_\text{i}$, $\text{RUP}_\text{i}$) are provided in the feed library. 
The digestibilities of each feed are estimated to account for the effects of intake and diet composition:
...
$$
\text{dNDF}_\text{i} = (\text{dNDF}_\text{base,i} - (0.0059 \times \text{Starch}_\text{Diet}) - 0.26)  - 1.1 \times (\frac{\text{DMI}}{\text{BodyWeight}} - 0.035)
$$

And the $\text{dNDF}_\text{base,i}$ is calculated based on the lignin and NDF content of the feed.

:::{#eq-an-sup-2-2}
[[**AN.SUP.2.2**]]{.aside .content-visible when-format="html"}    
$$
\text{dNDF}_\text{i} = \frac{0.75 \times (\text{NDF}_{\text i} - \text{Lignin}_{\text i}) \times (1-(\frac{\text{Lignin}_\text{i}}{\text{NDF}_{\text i}})^{0.667})}{\text{NDF}_\text{i}} 
$$
:::    
...
:::{#eq-an-sup-3-2}
[[**AN.SUP.3.2**]]{.aside .content-visible when-format="html"}    
$$
\text{dStarch}_\text{i} = \text{dStarch}_\text{base,i} - 1.0 \times (\frac{\text{DMI}}{\text{BW}} - 0.035) 
$$
:::  

And the base starch digestibility, $\text{dStarch}_\text{base,i}$, is considered to be an innate property of the feed and is provided in the feed library.

* Fatty Acid Digestibility (dFA)

Fatty acids are assumed to have a digestibility of 0.73 $(\text{dFA}_\text{i} = 0.73)$ unless a digesibility specific to the fat supplement is provided by the manufacturer.
...
:::{#eq-an-sup-4-2}
[[**AN.SUP.4.2**]]{.aside .content-visible when-format="html"}    
$$
\begin{aligned}
& \text{RDP}_\text{i} = \frac{1 - \text{RUP}_\text{CP,i}}{\text{CP}_\text{i}} \\[8pt]
& \text{RDP}_\text{i} = \frac{\text{RUP}_\text{CP,i}}{\text{CP}_\text{i}} 
\end{aligned}
$$
::: 
...
:::{#eq-an-sup-5-2}
[[**AN.SUP.5.2**]]{.aside .content-visible when-format="html"}    
$$
\text{ROM}_\text{i} = 1 - \frac{\text{FA}_\text{i}}{1.06} - \text{Ash}_\text{i} - \text{NDF}_\text{i} - \text{Starch}_\text{i} - (\text{CP}_\text{i} - 0.64 \times \text{NPN}_\text{supp,i})
$$
::: 

The digestibility of ROM is assumed to be 0.96 $(\text{dROM}_\text{i} = 0.96)$ and $\text{NPN}_\text{supp,i}$ is the supplemented non-protein nitrogen fraction as a proportion of the total CP content. 

**Digestible Energy of the Diet**

:::{#eq-an-sup-6-2}
[[**AN.SUP.6.2**]]{.aside .content-visible when-format="html"}    
$$
\text{DE}_\text{Diet} = \sum^{n}_{i=1} \text{DE}_\text{i} \times \text{PropFeed}_\text{i}
$$
::: 
...
:::{#eq-an-sup-7-2}
[[**AN.SUP.7.2**]]{.aside .content-visible when-format="html"}    
$$
\text{ME}_\text{Diet} = \text{DE}_\text{Diet} - \text{GaseEnergy} - \text{UrineEnergy}
$$
::: 
...
:::{#eq-an-sup-8-2}
[[**AN.SUP.8.2**]]{.aside .content-visible when-format="html"}    
$$
\text{GasEnergy} = 13.28 \times \text{EntericMethane}
$$
::: 
...
:::{#eq-an-sup-9-2}
[[**AN.SUP.9.2**]]{.aside .content-visible when-format="html"}    
$$
\text{UrineEnergy} = 0.0146 \times \text{UrinaryNitrogen}
$$
::: 
...
:::{#eq-an-sup-9}
[[**AN.SUP.9**]]{.aside .content-visible when-format="html"}
$$
\text{Kpi} =
\begin{cases}
2.904 + (1.375 \times \frac{\text{DMI}}{\text{BW}} \times 100) - (0.02 \times \text{PercentConc}) ,& \text{if feed is concentrate} \\[8pt]
3.362 + (0.479 \times \frac{\text{DMI}}{\text{BW}} \times 100) - (0.17 \times \text{NDF}_\text{i}) - (0.007 \times \text{PercentConc}) ,& \text{if feed is forage} \\[8pt]
3.054 + (0.614 \times \frac{\text{DMI}}{\text{BW}} \times 100) ,& \text{if feed is wet forage}
\end{cases}
$$
:::

:::{#eq-an-sup-10}
[[**AN.SUP.10**]]{.aside .content-visible when-format="html"}
$$
\text{RDPi} =
\begin{cases}
(\frac{\text{Kdi}}{\text{Kdi + Kpi}} \times \frac{\text{NBi}}{100} \times \text{CPi}) + (\frac{\text{NAi}}{100} \times \text{CPi}) ,& \text{if Kpi + Kdi} > 0 \\[6pt]
0 ,& \text{if Kpi + Kdi} \leq 0 \\
\end{cases}
$$
:::

:::{#eq-an-sup-11}
[[**AN.SUP.11**]]{.aside .content-visible when-format="html"}    
$$
\text{MPbact} = 0.64 \times min(1000 \times 0.13 \times \text{TDNact}_\text{diet}, 1000 \times 0.85 \times \text{RDP}_\text{diet})
$$
:::
...
:::{#eq-an-sup-12}
[[**AN.SUP.12**]]{.aside .content-visible when-format="html"}    
$$
\text{RUP}_\text{i} = \text{CP}_\text{i} - \text{RDP}_\text{i}
$$
:::

:::{#eq-an-sup-13}
[[**AN.SUP.13**]]{.aside .content-visible when-format="html"}    
$$
\text{RUP}_\text{i} = \sum_i\text{Feed}_\text{i} \times \text{RUP}_\text{i} - \text{dRDP}_\text{i}
$$
:::
...
$$
\text{EndP} = 0.4 \times 11.8 \times \text{DMI}
$$

##### Mineral Supply

We will use calcium as an example, but phosphorus calculations will be similar.

:::{#eq-an-sup-15}
[[**AN.SUP.15**]]{.aside .content-visible when-format="html"}    
$$
\text{Ca}_\text{supply} = \sum_i\text{Feed}_\text{i} \times \text{Ca}_\text{i} \times \frac{\text{dCa}_\text{i}}{100}
$$
:::
...
```{python}
#| label: tbl-an-auto-ration-feed-input-purch
#| tbl-cap: Key inputs found in the feed input file for `purchased_feeds`.
import_table(
  "../resources/table_data/animal/tbl-an-auto-ration-feed-input-purch.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```

```{python}
#| label: tbl-an-auto-ration-feed-input-perc
#| tbl-cap: Key inputs found in the feed input file for 'User_defined_ration_percentages`.
import_table(
  "../resources/table_data/animal/tbl-an-auto-ration-feed-input-perc.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```
...
calculated prior to first formulation. This process then proceeds to utilize a series of constraints methods (@tbl-an-ration-input-constr) that compares the
animal requirements against the supply of the optimizer’s ration attempt, all while abiding feed ingredient-specific inclusion rates, as defined in @nrc_2001 and @nrc_2021. 
...
* Milk production will be reduced.
  * If average milk production for the pen drops below the values defined in the Milk Production Section (below), then the simulation halts with an error.
...
::: {.callout-note}
A graphical overview of this process can be found in @fig-ani-nutr-req-feeding.
:::
...
* Record failed constraints and formulation attempt to OutputManager.
* Milk production will be reduced.
  * IF average milk production for the pen drops below the values defined in [Milk Production and Reduction](#sec-an-milk-prod-red), then the simulation halts with an error.
...
to the herd num specified by the user. Details of this method are described in the [Culling](#sec-an-cull) section of Animal-level processes and management. 
...
falls within acceptable range. 
  ...
  * Logic - While the number of (Heifer IIIs + cows) is greater than `herd_num` x `selling_threshold`: 
  ...
  * This cycle iterates until the number of (Heifer IIIs + cows) is equal to `herd_num` x `selling_threshold`
...
current herd size, purchase thresholds, and the availability of heifers in the replacement market.
  ...
  * While the number of (Heifer IIIs + cows + bought\_heifer\_num) is less than `herd_num` x `buying_threshold`: 
  ...
  * This cycle iterates until the number of (Heifer IIIs + cows + `bought_heifer_num`) is equal to `herd_num` x `buying_threshold` 
...
$$     
\text{lactation\_tissue\_change} = -\frac{\text{P1}}{\text{P2}} \times \
  e^{\bigg(1 - \frac{\text{DIM}}{\text{P2}}\bigg)} + \
  \frac{\text{P1}}{\text{P2}^{2}} \times \text{DIM} \times e^{\bigg(1 - \frac{\text{DIM}}{\text{P2}}\bigg)}
$$
:::
...
During the dry period, the net tissue change is assumed to be restored. 

:::{#eq-an-bwt-19}
[[**AN.BWT.19**]]{.aside .content-visible when-format="html"}
$$
\text{dry\_tissue\_change\_numerator} = \text{P1} \times \frac{\text{DIM}}{\text{P2}} \times e^{(1 - \frac{\text{DIMdry}}{\text{P2}})} 
$$
:::
and 
...
$$
\text{bodyweight\_tissue\_change} = \frac{\text{dry\_tissue\_change\_numerator}}{\text{gestation length} - \text{DIPdry}}
$$
:::
...
```{python}
#| label: tbl-an-repro_inputs-heifer-mgmt
#| tbl-cap: User inputs in `management_decisions` that determine the heifer reproductive breeding program and impact reproductive efficiency and age at first calving.
import_table(
  "../resources/table_data/animal/tbl-an-repro-inputs-heifer-mgmt.csv",
  colalign = ["left", "left", "center", "center", "center"]
)

```
```{python}
#| label: tbl-an-repro_inputs-heifer-farm
#| tbl-cap: User inputs in `farm_level_heifers` that determine the heifer reproductive breeding program and impact reproductive efficiency and age at first calving.
import_table(
  "../resources/table_data/animal/tbl-an-repro-inputs-heifer-farm.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```

```{python}
#| label: tbl-an-repro_inputs-cow-mgmt
...
import_table(
  "../resources/table_data/animal/tbl-an-repro-inputs-cow-mgmt.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```
```{python}
#| label: tbl-an-repro_inputs-cow-farm
#| tbl-cap: User inputs that determine reproductive efficiency of cows and influence culling, calving interval, and calf output.
import_table(
  "../resources/table_data/animal/tbl-an-repro-inputs-cow-farm.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```
```{python}
#| label: tbl-an-repro_inputs-cow-cow
#| tbl-cap: User inputs that determine reproductive efficiency of cows and influence culling, calving interval, and calf output.
import_table(
  "../resources/table_data/animal/tbl-an-repro-inputs-cow-cow.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```
...
#| tbl-cap: Inputs that can be used to determine if there is a decrease in the conception rate with parity and rebreeding attempts (`farm_level` section).
import_table(
...
for the Wood’s curve parameters l, m, and n for each parity of cows (see more information in the [Lactation Curve](#sec-an-lac-curve) section. At the beginning of each new lactation, 
...
These are comprehensively outlined in the [Lactation Curve Parameters Determination](@sec-an-lac-curve) Section.
...
The Lactation Curve Parameter Determination module calculates and adjusts Wood’s lactation curve parameters (l, m, n) based on farm-specific attributes such as
geographic location, management practices, and production levels. These parameters shape the milk production curve for cows across different parities 
(1st, 2nd, 3rd + lactations).
    
This module is part of the Animal Module and integrates with the Milk Production calculations. The outputs influence various downstream models, 
including nutrient requirements, manure excretion, and greenhouse gas emission calculations.
...
The module retrieves input data from simulation configuration files and the Input Manager. These inputs are organized into groups based on their functional roles. 
Each input is detailed by its parameter name, definition (including units where applicable), and any default values or constraints. 
There are three categories on inputs that should be considered in this section: 

* Wood's Lactation Curve Base Parameters [@Li_2022]
* Adjustment Factors
* Farm Specific 305 d Milk Production

```{python}
#| label: tbl-an-lac-woods
#| tbl-cap: Wood's Lactation Curve base parameters, provide baseline values and variability.
import_table(
  "../resources/table_data/animal/tbl-an-lac-woods.csv",
  colalign = ["left", "left", "center", "center"]
)
```

```{python}
#| label: tbl-an-lac-adjust
#| tbl-cap: Adjustment factors based on geographical, temporal, and parity considerations.
import_table(
  "../resources/table_data/animal/tbl-an-lac-adjust.csv",
  colalign = ["left", "left", "center", "center"]
)
```

```{python}
#| label: tbl-an-lac-milk-prod
#| tbl-cap: Farm specific 305-day milk production, used to tailor the lactation curve to the farm's production targets and herd composition.
import_table(
  "../resources/table_data/animal/tbl-an-lac-milk-prod.csv",
  colalign = ["left", "left", "center", "center"]
)
```
...
```{python}
#| label: tbl-an-lac-outputs
#| tbl-cap: Outputs for lactation curve.
import_table(
  "../resources/table_data/animal/tbl-an-lac-outputs.csv",
  colalign = ["left", "left", "center"]
)
```
...
**Determining Base Lactation Curve Parameters** (`set_lactation_parameters`) 

The Wood’s lactation curve parameters are initialized with base values 
retrieved from `lactation_inputs["parameter_mean_values"]` in `LACTATION_CURVE_ADJUSTMENT_INPUTS.json`.

**Adjusting parameters** (`_calculate_adjusted_wood_parameters`) 

Wood's lactation curve parameters are adjusted based on temporal, geographical, 
and management factors [@Li_2022]. Each parameter (l, m, n) is adjusted using a series of modifications:

::: {#eq-an-mlk-1}
[[**AN.MLK.1**]]{.aside .content-visible when-format="html"}
$$
\text{l}_\text{adjusted} = \text{l} + \sum \text{adjustments} 
$$
:::

::: {#eq-an-mlk-2}
[[**AN.MLK.2**]]{.aside .content-visible when-format="html"}
$$
\text{m}_\text{adjusted} = \text{m} + \text{scaling factor}_\text{m} \times \sum \text{adjustments} 
$$
:::

::: {#eq-an-mlk-3}
[[**AN.MLK.3**]]{.aside .content-visible when-format="html"}
$$
\text{n}_\text{adjusted} = \text{n} + \text{scaling factor}_\text{n} \times \sum \text{adjustments} 
$$
:::

Where:
$$ 
\text{scaling factor}_\text{m} = 10^{-2}
$$

$$
\text{scaling factor}_\text{m} = 10^{-4}
$$ 
...
**Fitting Parameters to Target Milk Yield** (`_adjust_lactation_curve_to_milk_yield`) 

If `annual_milk_yield` is provided, the l parameter is optimized to match the expected 305-day milk yield:

* Estimate herd average 305-day milk yield:

::: {#eq-an-mlk-4}
[[**AN.MLK.4**]]{.aside .content-visible when-format="html"}
$$
\text{Milk Yield}_{305} = \frac{\text{Annual Milk Yield}}{\text{Number of Milking Cows}} \times \frac{305}{\text{Days per year}}
$$
:::
...
::: {#eq-an-mlk-5}
[[**AN.MLK.5**]]{.aside .content-visible when-format="html"}
$$
M305_{Herd} = (M305_{P1} \times P1\%) + (M305_{\text{P2}} \times P2\%) + (M305_{P3} times P3^+\%)
$$
:::

::: {#eq-an-mlk-6}
[[**AN.MLK.6**]]{.aside .content-visible when-format="html"}
$$
M305_{P2} = M305_{P1} \times 118\%
$$
:::

::: {#eq-an-mlk-7}
[[**AN.MLK.7**]]{.aside .content-visible when-format="html"}
$$
M305_{P3^+} = M305_{P1} \times 125\%
$$
:::

::: {.callout-note}
118% and 125% are assumptions made based on the work by @Li_2022.
:::
...
::: {#eq-an-mlk-8}
[[**AN.MLK.8**]]{.aside .content-visible when-format="html"}
$$
\text{Error} = \text{Predicted MilkYield305} - \text{Target MilkYield305}
$$
:::

Using Scipy's minimize function, the l parameter is iteratively adjusted to minimize this error.
        
* Final Lactation Curve Parameter Mapping. Once adjustments are made, parameters are stored in:
...
Equations below are not in the `LactationCurve` class, but they are useful to understanding the bigger picture of how lactation curve parameter estimates 
are generated in RuFaS. Please see the Milk Production section for more details on these equations [@Wood1967].

**Wood's Lactation Curve**

[See [AN.MLK-9](#eq-an-mlk-9)]{.aside .content-visible when-format="html"}
$$
Y_t = l * t^{m} * e^{-nt}
$$
...
::: {#eq-an-mlk-10}
[[**AN.MLK.10**]]{.aside .content-visible when-format="html"}
$$
...
```{python}
#| label: tbl-an-cull-inputs
#| tbl-cap: Inputs used to determine probability and timing of cow death and sales events.
import_table(
  "../resources/table_data/animal/tbl-an-cull-inputs.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```
::: {.callout-note}
Variables marked with * are commonly left at default values. Adjusting them changes the timing of cow exit from the herd but does not affect the total number of exits.
:::
...
* `Report_life_cycle_manager_data` - is reported on a daily basis for the numbers of calves, HeiferII, HeiferIII, and Cow sold (calves born deceased tracked as newborns sold). It also reports HeiferIII purchased and documents other cow culls (i.e. sold and died). Culling data is also reported here documenting the average age of cows and HeiferII culled and the reason (e.g. death, poor production, lameness, injury, mastitis, disease, udder, or unkown/other).
...
* Death day probability: `death_day_prob`
   * Values to construct the CDF to determine future death day, given that death is going to occur (default values are given in @tbl-an-cdf-cull-death)
   * Values in the array correspond to the days in `cull_day_count` (another user input; default values are the “days” in @tbl-an-cdf-cull-death)

**Parameters and Equations**

The values for the CDF determining probability of death by day of lactation are located in the animal input json file (`death_day_prob`).

```{python}
#| label: tbl-an-cdf-cull-death
#| tbl-cap: Cumulative Distribution Function (CDF) values associated with the conditional probability of death over time. The 'cull_day_count' array defines the segments of the CDF.
import_table(
  "../resources/table_data/animal/tbl-an-cull-cdf-death.csv",
...
::: {#eq-an-anm-1}
[[**AN.ANM.1**]]{.aside .content-visible when-format="html"}
$$
X = x_{\text{i-1}} + a_{\text{i}} \times (R - c_{\text{i-1}})
$$
:::
...
* $c_{\text{i-1}},c_{\text{i}}$ = the upper and lower values around R in the death day prob CDF
* $x_{\text{i-1}},x_{\text{i}}$ = days, corresponds to $c_{\text{i-1}},c_{\text{i}}$

Empirical CDF: 
$a_{\text{i}} = \frac{x_{\text{i-1}},x_{\text{i}}}{c_{\text{i-1}},c_{\text{i}}}$ 

* `Animal.determine_future_death_date`
  * Called by `Animal.daily_routines()` at the beginning of each new lactation (on the day a new calf is born).
  ...
  * Logic: Draw random value between 0-1 and compare it to probability of death in that lactation group (`parity_death_probability`).
    * If drawn value \(\leq \) `parity_death_probability`, then go on to select date that death will occur - draws another random value between 0-1.
...
Probability of disease sales associated with each lactation (1, 2, 3, 4+) is specified at the beginning of the simulation as a user input. At the beginning of each lactation for each cow, it is determined whether she will be sold for disease during that lactation with a random draw compared with the appropriate probability. Then, for individuals assigned to be sold during that lactation, another random draw determines the reason for that sale from one of six possibilities (@tbl-an-cull-reason). Finally, an empirical CDF is used to determine the day of sale based on the probability of sale occurring during each interval of the lactation for that particular sale reason. 
...
* PFor each of the six reasons:
   * `Probability` - Conditional probability that a sold animal is removed due to each group of issues. This probability is used to determine the reason for sale after it has been decided that the animal will be sold during the current lactation. The sum of probabilities for the six culling reasons equals 1. See @tbl-an-cull-reason below for default values used to construct the empirical PDF.
  * `Cull_day_prob`: Values to construct the CDF to determine future sale day, given that disease-related sale is going to occur. Values in the array correspond to the days in `cull_day_count` (another user input; default values are the “days”; default values in @tbl-an-cull-cdf) 


**Parameters and Equations**

```{python}
#| label: tbl-an-cull-reason
#| tbl-cap: Cumulative Distribution Function (CDF) values associated with the conditional probability that a solud animal is removed due to each group of issues. Corresponds to the “probability” value for each cull reason in the animal input json file..
import_table(
  "../resources/table_data/animal/tbl-an-cull-reason.csv",
  colalign = ["left", "center", "center", "center", "center"]
)
```

```{python}
#| label: tbl-an-cull-cdf
#| tbl-cap: Cumulative Distribution Function (CDF) values associated with the likelihood over time of sales for each reason.
import_table(
  "../resources/table_data/animal/tbl-an-cull-cdf.csv",
  colalign = ["left", "center", "center", "center", "center"]
)
```
...
::: {#eq-an-anm-2}
[[**AN.ANM.2**]]{.aside .content-visible when-format="html"}
$$
...
* R = random float number between 0-1  
* $c_{\text{i-1}},c_{\text{i}}$ = the upper and lower values around R in the sale day prob CDF
* $x_{\text{i-1}},x_{\text{i}}$ = days, corresponds to $c_{\text{i-1}},c_{\text{i}}$
* Empirical CDF: $a_{\text{i}} = \frac{x_{\text{i-1}},x_{\text{i}}}{c_{\text{i-1}},c_{\text{i}}}$ 
...
* `Animal.determine_future_cull_date()`
   ...
   * Logic: Draw random value between 0-1 and compare it to probability of death in that lactation group (`parity_death_probability`)
      * If drawn value \(\leq \) p`arity_death_probability`, then go on to select date that death will occur - draws another random value between 0-1
      * Compares random draw to the CDF defined by `death_day_prob` array to determine number of days into the future at which death will occur - adds that number of days to current days born to determine future death date 
      * If drawn value \( > \) `parity_death_probability`, then future death date is assigned as `sys.maxsize` (a really really big number – effectively, she will never reach her assigned future death date)
* `Animal.animal_life_stage_update()` 
   * Called by `HerdManager.daily_routines()` → `Animal.daily_routines()`
...
* `Animal.life_stage_update()`
   ...
   * Includes the following logic: 
      * If a cow is `do_not_breed` and daily milk produced is less than user-defined `cull_milk_production`: 
...
```{python}
#| label: tbl-an-manexc-outputs-excre
#| tbl-cap: Expected manure excretion outputs from the manure excretion calculator.
import_table(
  "../resources/table_data/animal/tbl-an-manexc-outputs-excre.csv",
  colalign = ["left", "left", "center"]
)
```

```{python}
#| label: tbl-an-manexc-outputs-nutr
#| tbl-cap: Expected manure nutrient composition outputs from the manure excretion calculator.
import_table(
  "../resources/table_data/animal/tbl-an-manexc-outputs-nutr.csv",
  colalign = ["left", "left", "center"]
)
```
...
$$
\text{manure\_nitrogen} = \frac{112.55 \times \text{dry\_matter\_intake} \times \frac{\text{crude\_protein}}{100}}{1000} 
$$
:::
...
$$
\text{manure\_nitrogen} = 15.1 + 8.83 \times \frac{\text{dry\_matter\_intake} \times 1000 \times 
\frac{\frac{\text{crude\_protein\_concentration}}{6.25}}{100}}{1000}
$$
:::
...
$$
\text{fecal\_nitrogen} = 0.345 + 0.317 \times \frac{\text{dry\_matter\_intake} \times 1000 \times 
\frac{\frac{\text{crude\_protein\_concentration}}{6.25}}{100}}{1000}
$$
:::
...
$$
\begin{aligned}
\text{fecal\_water} &= (1.987 \times \text{dry\_matter\_intake}) + (0.348 \times \text{acid\_detergent\_fiber\_concentrations})\\
& \qquad - (0.412 \times \text{crude\_protein\_concentration}) - (0.074 \times \text{dry\_matter\_concentration}) \\
& \qquad - (0.0057 \times \text{days\_in\_milk})
\end{aligned} 
$$
:::

*Total solids/Fecal dry matter (kg/d)*

Calculate the amount of fecal dry matter excreted (kg/d). Fecal dry matter is assumed to be equivalent to total solids:

::: {#eq-an-exc-19}
[[**AN.EXC.19**]]{.aside .content-visible when-format="html"}
$$
\begin{aligned}
\text{fecal\_solids} &= 0.576 + (0.370 \times \text{dry\_matter\_intake})\\
& \qquad - (0.075 \times \text{crude\_protein\_concentration}) \\
& \qquad + (0.059 \times \text{acid\_detergent\_fiber\_concentrations})
\end{aligned}
$$
:::
...
$$
\begin{aligned}
\text{urine} &= -7.742 + (0.388 \times \text{dry\_matter\_intake}) \\
& \qquad + (0.726 \times \text{crude\_protein\_concentration}) + (2.066 \times \text{milk\_protein})
\end{aligned}
$$
:::
...
$$
\begin{aligned}
\text{degradable\_volatile\_solids} &= -1.017 + (0.364 \times \text{organic\_matter\_intake}) \\
& \qquad + (0.029 \times \text{neutral\_detergent\_fiber\_concentration}) \\
& \qquad - (0.023 \times \text{crude\_protein\_concentration})
\end{aligned}
$$
:::
...
$$
\begin{aligned}
\text{total\_volatile\_solids} &= -1.201 + (0.402 \times \text{organic\_matter\_intake}) \\
& \qquad + (0.036 \times \text{neutral\_detergent\_fiber\_concentration}) \\
& \qquad - (0.024 \times \text{crude\_protein\_concentration})
\end{aligned}
$$
:::
...
$$
\text{manure\_nitrogen} = 20.3 + 0.654 \times \frac{\text{dry\_matter\_intake} \times 1000 \times \frac{\frac{\text{crude\_protein\_concentration}}{6.25}}{100}}{1000}
$$
:::
...
$$
\text{fecal\_nitrogen} = \frac{(10.1 \times \text{dry\_matter\_intake} - 18.5)}{1000}
$$
:::
...
#### Dry Cow Manure Excretion (`_calculate_dry_cow_manure`)
...
$$
\begin{aligned}
\text{total\_manure\_excreted} &= (0.00711 \times \text{body\_weight}) + (0.324 \times \text{crude\_protein\_concentration}) \\
&\qquad + (0.259 \times \text{neutral\_detergent\_fiber\_concentration})
\end{aligned}
$$
:::
...
\begin{aligned}
\text{degradable\_volatile\_solids} &= -1.017 + (0.364 \times \text{organic\_matter\_intake}) \\
&\qquad + (0.029 \times \text{neutral\_detergent\_fiber\_concentration}) - (0.023 \times \text{crude\_protein\_concentration})
\end{aligned}
$$
...
\begin{aligned}
\text{total\_volatile\_solids} &= -1.201 + (0.402 \times \text{organic\_matter\_intake}) \\
&\qquad + (0.036 \times \text{neutral\_detergent\_fiber\_concentration}) - (0.024 \times \text{crude\_protein\_concentration})
\end{aligned}
$$
...
#### Manure Phosporus Excretion (`_calculate_phosphorus_excretion_manure`)
...
::: {.callout-note}
All references may be found at the end of this document or are based on expert communication with V. Sempali in 2023 
and 2024.
::: 
...
$$
\begin{aligned}
\text{soluble\_residue} &= 100 - \text{ash\_concentration} - \text{neutral\_detergent\_fiber\_concentration} \\
&\qquad - \text{crude\_protein\_concentration} - \text{ethyl\_ester\_concentration}
\end{aligned}
$$
:::
...
$$
\begin{aligned}
\text{gross\_energy\_concentration} &= 0.263 \times \left(\text{crude\_protein\_concentration} + 0.522\right) \\
&\qquad \times \left(\text{ethyl\_ester\_concentration} + 0.198\right) \times \left(\text{neutral\_detergent\_fiber\_concentration} + 0.160\right) \\
&\qquad \times \text{soluble\_residue}
\end{aligned}
$$
:::
...
[[**AN.MET.1**]]{.aside .content-visible when-format="html"}
$$
\text{methane\_emission} = \frac{0.013 \times \text{body\_weight}^{0.75} \times 4.184 \text{J/Mcal}}{0.05565}
$$
:::
...
$$
\begin{aligned}
\text{methane\_emission} &= -126 + (11.3 \times \text{dry\_matter\_intake}) \\
&\qquad + (2.30 \times \text{neutral\_detergent\_fiber\_concentration})   \\
&\qquad + (28.8 \times \text{milk\_fat}) + (0.148 \times \text{body\_weight})
\end{aligned}
$$
:::
...
\begin{aligned}
  \text{methane\_emission} = \frac{ 45.98 \times \
    e^{- (\text{starch\_to\_acid\_detergent\_fiber\_concentration\_ratio} + 0.0045)} \
...
This module predicts the daily enteric methane emissions for individual animals. Outputs are described in @tbl-an-output-methane.
...
$$
\begin{aligned}
\text{methane\_yield\_reduction} &= -30.8 - (0.226 \times (\text{methane\_mitigation\_additive\_amount} - 70.5)) \\
&\qquad + (0.906 \times (\text{neutral\_detergent\_fiber\_concentration} - 32.9 )) \\
&\qquad + (3.871 \times (\text{ethyl\_ester\_concentration} - 4.2 )) \\
&\qquad - (0.337 \times (\text{starch\_concentration} - 21.1 ))
\end{aligned}
$$
:::

**Monensin Methane Yield Reduction** 
...
$$
\begin{aligned}
\text{methane\_yield\_reduction} &= 6.35 - (0.277 \times \text{methane\_mitigation\_additive\_amount} ) \\
&\qquad - (0.182 \times \text{starch\_concentration})
\end{aligned}
$$
:::
...
If no valid mitigation method is selected, the reduction defaults to 0%.

**Adjust the Initial Predicted Enteric Methane (`EntericMethaneCalculator.calculate_cow_methane`)** 
...
$$
\begin{aligned}
\text{adjusted\_methane\_emission} &= \text{methane\_yield} \times \left( 1 + \frac{\text{methane\_yield\_reduction}}{100} \right) \\
&\qquad \times \text{dry\_matter\_intake}
\end{aligned}
$$
:::
