...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-sim-outline.csv",
)
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-grp-inputs.csv",
)
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-def-grp.csv",
)
...
\text {CW} = (18 + (\text{DOP} - 190)\cdot 0.665)\left(\frac{\text{CBW}}{45}\right), \text{if DOP} > 190.0 \\
$$
...
$$
\begin{aligned}
NEmaint = (BW - CW)^{0.75}\cdot (0.086\cdot (0.8 + (BCS9 - 5)\cdot 0.05) + 0.0007 (20 - PrevTemp)) \\
\hfill \text{[AN.NRC.7]}
\end{aligned}
$$
:::
...
#| label: tbl-ani-gen-manexc-inputs
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-gen-manexc-inputs.csv",
)
