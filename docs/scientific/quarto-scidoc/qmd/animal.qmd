...
::: {.callout-note}
For the time being, only one set of feeds and ration recipe can be provided for each animal class. 
:::
...
* Heifers: The maintenance requirement is calculated based on metabolic body weight $(\text{body weight}^{0.75})$, body condition score, and the previous month's temperatures.
...
\text{bodyweight\_tissue\_change} = \frac{\text{P1}}{\text{P2}} \times \
  e^{(1 - \frac{\text{DIM}}{\text{P2}})} + \
  \text{DIM} \times e^{(1 - \frac{\text{DIM}}{\text{P2}})}
$$
:::
...
import_table(
  "../resources/table_data/animal/tbl-an-cull-cdf-death.csv",
    colalign = ["center", "right", "center", "center", "center"], headersalign = ["same"]
)
```
...
#| tbl-cap: Default values used to construct the Cumulative Distribution Function (CDF) values associated with the conditional probability that a sold animal is removed due to each group of issues.
import_table(
...
@Niu2018 equation is available for lactating cows. Finally, a separate equation published by @Pattanaik2003 is used for calves. 

<br> Simulation of enteric methane mitigation via mitigation supplements 3-NOP or monensin are also available for lactating cows The module integrates with the `DigestiveSystem` class and supports methane mitigation strategies using feed additives.
...
$$
\begin{aligned}
\text{gross\_energy\_concentration} &= 0.263 \times \text{crude\_protein\_concentration} + \\
&\qquad 0.522 \times \text{ethyl\_ester\_concentration} + \\
&\qquad 0.198 \times \text{neutral\_detergent\_fiber\_concentration} +  \\
&\qquad 0.160 \times \text{soluble\_residue}
\end{aligned}
$$
:::
...
$$
\text{methane\_emission} = \frac{0.065 \times \text{gross\_energy\_concentration} \times \text{dry\_matter\_intake}}{0.05565}
$$
:::
