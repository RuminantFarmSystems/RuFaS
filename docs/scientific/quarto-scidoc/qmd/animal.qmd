...
CW = (18 + (DOP - 190) \times 0.665)\left(\frac{CBW}{45}\right), \text{if DOP} > 190.0 
$$
...
CW = (18 + (\text{DOP} - 190)\times 0.665)\left(\frac{\text{CBW}}{45}\right), \text{if DOP} > 190.0 
$$
<p style="text-align: right;">[AN.NRC.2]</p>
...
::: {#eq-an-nrc-7}
$$
NEmaint = (BW - CW)^{0.75}\times (0.086\times (0.8 + (BCS9 - 5)\times 0.05) + 0.0007 \times (20 - PrevTemp)) 
$$
...
```{python}
#| label: tbl-ani-cow-input-manexc
...
md_tab = make_markdown_table(
    "resources/table_data/tbl-ani-cow-input-manexc.csv",
)
...
#| label: tbl-ani-cow-phos-input-manexc
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-cow-phos-input-manexc.csv",
)
...
$$
<p style="text-align: right;">[AN.EXC.8ALT]</p>
...
::: {#eq-an-exc-19}
$$
\text{fecal\_solids} = 0.576 + \left(0.370 \times \text{dry\_matter\_intake}\right) - \left(0.075 \times \text{crude\_protein\_concentration}\right)+ \left(0.059 \times \text{acid\_detergent\_fiber\_concentrations}\right)
$$
...
::: {#eq-an-exc-20}
$$
\text{urine} = -7.742 + \left(0.388 \times \text{dry\_matter\_intake}\right) + \left(0.726 \times \text{crude\_protein\_concentration}\right) + \left(2.066 + \text{milk\_protein}\right)
$$
...
::: {#eq-an-exc-23}
$$
\text{degradable\_volatile\_solids} = -1.017 + \left(0.364 \times \text{organic\_matter\_intake}\right) + \left(0.029 \times \text{neutral\_detergent\_fiber\_concentration}\right) - \left(0.023 \times \text{crude\_protein\_concentration}\right)
$$
...
::: {#eq-an-exc-24}
$$
\text{total\_volatile\_solids} = -1.201 + \left(0.402 \times \text{organic\_matter\_intake}\right) + \left(0.036 \times \text{neutral\_detergent\_fiber\_concentration}\right) - \left(0.024 \times \text{crude\_protein\_concentration}\right)
$$
...
::: {#eq-an-exc-29}
$$
\text{total\_manure\_excreted} = \left(0.00711 \times \text{body\_weight}\right) + \left(0.324 \times \text{crude\_protein\_concentration}\right) + \left(0.259 \times \text{neutral\_detergent\_fiber\_concentration}\right) 
$$
...
$$
See [AN.EXC.11ALT](#eq-an-exc-11ALT).
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-gen-enteric-met.csv",
)
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-cow-enteric-met.csv",
)
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-model-methane.csv",
)
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-output-methane.csv",
)
...
::: {#eq-an-met-1}
$$
\text{soluble\_residue} = 100 - \text{ash\_concentration} - \text{neutral\_detergent\_fiber\_concentration} - \text{crude\_protein\_concentration} - \text{ethyl\_ester\_concentration} 
$$
...
::: {#eq-an-met-2}
$$
\text{gross\_energy\_concentration} = 0.263 \times \left(\text{crude\_protein\_concentration} + 0.522\right) \times \left(\text{ethyl\_ester\_concentration} + 0.198\right) \times \left(\text{neutral\_detergent\_fiber\_concentration} + 0.160\right)  \times \text{soluble\_residue}
$$
...
::: {#eq-an-met-6}
$$
\text{methane\_emission} = -126 + \left(11.3 \times \text{dry\_matter\_intake} \right) + \left(2.30 \times \text{neutral\_detergent\_fiber\_concentration}\right) + \left(28.8 \times \text{milk\_fat}\right) + \left(0.148 \times \text{body\_weight}\right)
$$
...
::: {#eq-an-met-7}
$$
\text{methane\_emission} = \frac{45.98 \times e^{- \left(\text{starch\_to\_acid\_detergent\_fiber\_concentration\_ratio} + 0.0045\right)} \times \text{metabolizable\_energy\_intake} \times 4.184}{0.05565}
$$
...
#| label: tbl-ani-enter-met-diet-mitigate
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-enter-met-diet-mitigate.csv",
)
...
#| label: tbl-ani-enter-met-additive-type-dose
...
md_tab = make_markdown_table(
    "../resources/table_data/tbl-ani-enter-met-additive-type-dose.csv",
)
...
This module predicts the daily enteric methane emissions for individual animals:
See [previous table](#tbl-ani-output-methane).
```{python}
...
::: {#eq-an-met-9}
$$
\text{methane\_yield\_reduction} = 6.35 -  \left(0.277 \times \text{methane\_mitigation\_additive\_amount} \right) - \left(0.182 \times \text{starch\_concentration}\right)
$$
...
::: {#eq-an-met-11}
$$
\text{adjusted\_methane\_emission} = \text{methane\_yield} \times \left(1 + \frac{\text{methane\_yield\_reduction}}{100}\right) \times \text{dry\_matter\_intake}
$$
