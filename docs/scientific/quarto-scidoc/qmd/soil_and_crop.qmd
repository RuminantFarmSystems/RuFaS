...
```{python}
#| label: tbl-sc-field-inputs
#| tbl-cap: Required user inputs for the Field section.
import_table(
  "../resources/table_data/soil_and_crop/tbl-sc-field-inputs.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```

```{python}
#| label: tbl-sc-crop-mgmt-inputs
#| tbl-cap: Required user inputs for the Crop Management section.
import_table(
  "../resources/table_data/soil_and_crop/tbl-sc-crop-mgmt-inputs.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
``` 

```{python}
#| label: tbl-sc-fert-mgmt-inputs
#| tbl-cap: Required user inputs for the Fertilizer Management section.
import_table(
  "../resources/table_data/soil_and_crop/tbl-sc-fert-mgmt-inputs.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```

```{python}
#| label: tbl-sc-fert-schd-inputs
#| tbl-cap: Required user inputs for the Fertilizer Schedule section.
import_table(
  "../resources/table_data/soil_and_crop/tbl-sc-fert-schd-inputs.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```

```{python}
#| label: tbl-sc-man-app-inputs
#| tbl-cap: Required user inputs for the Manure Application section.
import_table(
  "../resources/table_data/soil_and_crop/tbl-sc-man-app-inputs.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```
...
* `applied_mass` (kg) = mass of the given fertilizer required to satisfy all nutrient requests.
* fert_mass<sub>nitrogen</sub> (kg) = mass nitrogen requested.
* fert_mass<sub>phosphorus</sub> (kg) = mass of phosphorus requested.
* fert_mass<sub>potassium</sub> (kg) = mass of potassium requested.
* `fert_nitrogen_frac` (proportion) = fraction of nitrogen in the fertilizer mix.
* `fert_phosphorus_frac` (proportion) = fraction of phosphorus in the fertilizer mix.
* `fert_potassium_frac` (proportion) = fraction of potassium in the fertilizer mix.
...
* `applied_nitrogen` (kg) = mass of nitrogen applied.
* `applied_phosphorus` (kg) = mass of phosphorus applied.
* `applied_potassium` (kg) = mass of potassium applied.
...
* `surface_applied_nutrient` (kg/ha) = mass per ha of each nutrient applied to the surface soil layer.
* `surface_remainder_fraction` (proportion) = user-provided input defining the fraction of the fertilizer that remains on the soil surface.
* `field_size` (ha) = size of the field.
...
* `depth_factor` (proportion) = fraction of subsurface fertilizer added to each soil layer.
* `layer_depth` (mm) = bottom depth of each sub-surface soil layer.
* `depth_factor_sum` (proportion) = sum of the depth factors for all layers above the deepest layer receiving fertilizer.
...
* `manure_applied_water` = amount of water applied to the field with a manure application (L)
...
* `surface_dry_matter_mass` (kg) = amount of manure solids or dry matter that is applied to the soil surface at that application.
* `manure_dry_matter_mass` (kg) = amount of manure solids or dry matter that is applied at that application.
* `surface_remainder_fraction` (kg) = user-provided value for the amount of the manure applied that remains on the surface.
...
* `mass_to_add_to_labile_phos` (kg) = phosphorus that will be added to the surface labile phosphorus pool during that application.
* `mass_to_add_to_active_phos` (kg) = phosphorus that will be added to the surface active phosphorus pool during that application.
* `total_phosphorus_mass` (kg) = total phosphorus applied during that application.
* `soil_infiltration` = 0.4 if the manure applied is less than 15% dry matter, and 0 otherwise.
...
* `mass_nitrates_added` (kg N/ha) = mass of nitrate nitrogen applied to relevant soil layer.
* `mass_ammonium_added` (kg N/ha) = mass of ammonium nitrogen applied to relevant soil layer.
* `mass_fresh_organic_nitrogen_added` (kg N/ha) = mass of fresh organic nitrogen applied to relevant soil layer.
* `mass_stable_organic_nitrogen_added` (kg N/ha) = mass of stable organic nitrogen applied to relevant soil layer.
* `manure_dry_matter_mass` (kg) = mass of manure dry matter added to the soil layer.
* `ammonium_fraction` (proportion) = fraction of inorganic nitrogen that is in the form of ammoniacal nitrogen and is provided by the Manure Module.
* `fresh_fraction_of_organic_nitrogen` (proportion) = fraction of organic nitrogen that is more quickly mineralized and is set to a constant of 0.9286.
...
```{python}
#| label: tbl-sc-soil-inputs
#| tbl-cap: Required user inputs for the Soil section.
import_table(
  "../resources/table_data/soil_and_crop/tbl-sc-soil-inputs.csv",
  colalign = ["left", "left", "center", "center", "center"]
)
```
...
$$
\text{S} = \text{S}_\text{max} \times \left(1 - \frac{\text{SW}_\text{available}}{\text{SW}_\text{available} + e^{\text{w}_1 - (\text{w}_2 \times \text{SW}_\text{available})}}\right)
$$
:::  
...
$$
\text{w}_1 = ln \left(\frac{\text{FC}}{1 - (\text{S}_3 \times \text{S}_{\text{max}}^{-1})} - \text{FC}\right) + (\text{w}_2 \times \text{FC})
$$
:::  
...
$$
\text{S}_3 = 25.4 \times \left(\frac{1000}{\text{CN}_3} - 10 \right)
$$
:::  
...
$$
\text{ET}_\text{max} = max \left({\frac{0.0023 \times \text{H}_0 \times (\text{T}_\text{max} - \text{T}_\text{min})^{0.5} \times (\text{T}_\text{avg} + 17.8)}{\text{LHV}}, 0.001}\right)
$$
:::  
...
Potential evaporation is adjusted based on trans{max} as: 

::: {#eq-sc-evp-6}
[[**SC.EVP.6**]]{.aside .content-visible when-format="html"}
$$
\text{evap}_\text{max} = min \left(\frac{\text{evap}_\text{max} \times \text{ET}_\text{max}}{\text{evap}_\text{max} + \text{trans}_\text{max}}, \text{evap}_\text{max}\right)
$$
:::    
...
::: {#eq-sc-nnv-4}
[[**SC.NNV.4**]]{.aside .content-visible when-format="html"}
$$
...
$$
\text{NO3\_effect} = 1.0 - \left(0.5 + \text{arctan}\left(\pi \times 0.01 \times \frac{\text{NO3\_content} - 190}{\pi}\right)\right) \times 25 
$$
:::    
...
& \text{MOP}_\text{leach} = max \left(0.0, \frac{\text{manure}_\text{extr} \times \text{WOP}}{0.6}\right)
\end{aligned}
$$
:::          
...
$$
\text{PD}_\text{fact} = \left(\frac{\text{runoff}}{\text{rainfall}}\right)^{0.225}
$$
::: 
...
* wet{rate} is the wet rate of manure decomposition (unitless)
* dry{rate}is the dry rate of manure decomposition (unitless)
* manure{mass}is the total mass of manure on the field (kg/ha)
...
$$
\text{cov}_\text{dcom} = min \left(\text{manure}_\text{cov}, max \left(0.0, \frac{\text{dcom}}{\text{manure}_\text{mass}}\right)\right)
$$
:::         
...
* P{pools} represents each of the tracked P pools: stable inorganic P (SIP), stable organic P (SOP), water-extractable organic P (WOP)
...
$$
\text{cov}_\text{ASIM} = min \left(\text{manure}_\text{cov}, max \left(0.0, \frac{\text{manure}_\text{ASIM}}{\text{manure}_\text{mass} \times \text{manure}_\text{cov}}\right)\right)
$$
:::     
...
$$
\text{P}_\text{ASIM} = min \left(\text{P}, max \left(0.0, \frac{\text{manure}_\text{ASIM}}{\text{manure}_\text{mass} \times \text{P}}\right)\right)
$$
:::         
...
* MDRP{runoff} is the manure dissolved reactive P transported in runoff (mg/L)
...
* TIP{runoff} is the total inorganic P runoff
* MIP{runoff} is the manure inorganic P runoff
* MDRP{runoff} is the manure dissolved reactive P runoff [SC.PSL.55](eq-sc-psl-55)
* fertP{runoff} is the fertilizer P runoff
...
* PSP{actual} is the actual P sorption parameter (unitless)
...
* PSP{act} is the The actual P sorption parameter of the layer adjusted from the maximum (unitless)
...
* PD{fac} is the P desorption factor
* day{count} is the number of days that the active inorganic P pool has been greater than it would be when in equilibrium with the labile inorganic P pool. 
* labile{pflow} is the amount of P moved from the active to labile inorganic P pool to maintain equilibrium (kg/ha).
...
* PSP{act}	is the actual P sorption parameter of the layer adjusted from the maximum (unitless)
...
* PS{fac} is the P sorption factor
...
* Stable{pflow} is the P transferred from the stable to the active inorganic P pool (kg ha$^{-1}$)
...
* C{active}% = active C composition of the soil (%)
* C{slow}% = slow C composition of the soil (%)
* C{passive}% = passive C composition of the soil (%)
...
* T{d} = unitless temperature effect on decomposition factor (empirically determined based on average soil temperature)
* teff{1} = x-value at inflection point (empirical factor set at 15.4)
* teff{2} = y location of inflection point (before normalization) (empirical factor set to 11.75)
* teff{3} = distance from maximum point to the minimum point (empirical factor set at 29.7)
* teff{4} = slope of line at inflection point (empirical factor set at 0.031)
...
* Md = unitless moisture effect on decomposition factor (empirically determined based on soil water levels)
* water{fac} = relative water saturation (%)
* `a` = dimensionless empirical factor (0.55)
* `b` = dimensionless empirical factor (1.7)
* `c` = dimensionless empirical factor (-0.007)
* e1 = dimensionless empirical factor (6.648115)
* e2 = dimensionless empirical factor (3.22)
...
* C{active decomp rate} = rate at which active C is decomposed into slow C, passive C and CO<sub>2</sub> (%) (kg year$^{-1}$)
* K5 = maximum rate of C decomposition (0.14, [@parton1987analysis]) 
* `silt:clay_%` = ratio of percent silt to clay content in the soil
...
* C{active decomp} = active C decomposed into slow or passive C and CO<sub>2</sub> (kg ha$^{-1}$)
* C{active} = active C stored in the soil (kg ha$^{-1}$)
...
* C{slow decomp} = slow C decomposed into active or passive carbon and CO<sub>2</sub> (kg ha$^{-1}$)
* `K6` = slow C decomposition factor (set at 0.0038, [@parton1987analysis])
* C{slow} = slow C stored in the soil (kg ha$^{-1}$)
...
* C{passive decomp} = passive C decomposed into active or passive carbon and CO<sub>2</sub> (kg ha$^{-1}$)
* `K7` = passive C decomposition factor (set at 0.0038, [@parton1987analysis])
* C{passive} = passive C stored in the soil (kg ha$^{-1}$)
...
* C{active_to_slow} = active C decomposed into slow C (kg ha$^{-1}$)
* C{active loss} = active C lost as CO<sub>2</sub> during decomposition into slow C (kg ha$^{-1}$)
...
* C{active}:C{passive} = active C decomposed into passive carbon (kg ha$^{-1}$)
...
* C{slow_to_active} = slow C decomposed into active C (kg ha$^{-1}$)
* CO<sub>2</sub>_to_C{slow loss} = slow C lost as CO<sub>2</sub> during decomposition (fraction)
* frac_C{slow_to_passive} = slow C decomposed into passive C (fraction)
* C{slow_to_passive} = slow C decomposed into passive C (kg ha$^{-1}$)
* C{slow_to_CO$_2$} = slow C lost as CO<sub>2</sub> during decomposition (kg ha$^{-1}$)
...
* C{passive_to_active} = passive C decomposed into active carbon (kg ha$^{-1}$)
* frac_CO<sub>2</sub>{passive loss} = passive C lost as CO<sub>2</sub> during decomposition (fraction)
* C{passive loss} =passive C lost as CO<sub>2</sub> during decomposition (kg ha$^{-1}$)
...
* C{CO<sub>2</sub> loss decomp} =C lost as CO<sub>2</sub> during decomposition (kg ha$^{-1}$)
...
* AG{lignin res frac} = fraction of lignin composition of above ground plant residue (unitless)
...
* AG{L:N} = above ground plant lignin to nitrogen (N) ratio when nitrogen in plant residue at harvest is greater than 0 (unitless)
* fr{N} = fraction of N in plant residue at harvest (unitless)
...
* AG{met frac} = above ground residue that is metabolic (unitless)
...
* AG{met} = above ground metabolic carbon (kg ha$^{-1}$)
* AG{met frac} = fraction of above ground residue that is metabolic (unitless)
* AG{DM res} = dry matter residue at harvest (kg ha$^{-1}$)
...
* AG{met}:C{active}	= above ground metabolic C decomposed to active carbon (kg ha$^{-1}$)
* AG{met active decomp} = rate of decomposition from metabolic to active C (set at 0.28, \cite{parton1987analysis})
...
* AG{met}:BG{met} = above ground metabolic C decomposed to below ground metabolic C (kg ha$^{-1}$)
...
* AG{struct} = above ground structural C (kg ha$^{-1}$)
...
* AG{struct decomp} = rate at which above ground structural C decomposes into slow or active C
...
* AG{struct}:C{active} = above ground structural C decomposed into active C (kg ha$^{-1}$)
* AG{struct}:C{slow} = above ground structural C decomposed into slow C (kg ha$^{-1}$)
...
* AG{struct}:BG{struct} = transfer of structural carbon during tillage (kg ha$^{-1}$)
...
* BG{lignin res} = below ground residue comprised of lignin (fraction)
* BG{DM res} = below ground residue mass (kg ha$^{-1}$)
* BG{bio} = below ground biomass (kg ha$^{-1}$)
...
$$
\text{BG}_\text{L:N} = \text{AG}_\text{L:N} \times \frac{\text{BG}_\text{DM res}}{\text{BG}_\text{DM res} + \text{BG}_\text{bio}} + \frac{\text{BG}_\text{lignin res frac}}{\text{fr}_\text{N} \times 100} \times \left(1 - \frac{\text{BG}_\text{DM res}}{\text{BG}_\text{DM res} + \text{BG}_\text{bio}}\right)  
$$
::: 
    
*Where*:
            
* BG{L:N} = below ground lignin to N ratio
...
* BG{met frac} = below ground residue that is metabolic (fraction)
...
* BG{met}=below ground metabolic C (kg ha$^{-1}$)
...
* BG{met active decomp} =	rate at which below ground metabolic C decomposes into	active C (set at 0.35, @parton1987analysis)
...
* BG{struct}\_to\_C{active} = below ground structural C decomposed into active C (kg ha$^{-1}$)
* BG{struct}\_to\_C{slow} = below ground structural C decomposed into slow C (kg ha$^{-1}$)
* BG{struct decomp} = rate at which below ground structural C decomposes into slow or active C (set at 0.094, @parton1987analysis)
* BG{struct} = below ground structural C (kg ha$^{-1}$)
...
* frac_CO2{met\_to\_active} = rate of C dioxide loss during transformation of metabolic to active C (0.55, @parton1987analysis)
* AG{met}\_to\_C{active loss} = above ground metabolic C being lost as carbon dioxide during decomposition into active C (kg ha$^{-1}$)
* AG{met}\_to\_C{active act} = above ground metabolic C decomposed to active carbon after accounting for C dioxide loss (kg ha$^{-1}$)
...
* frac\_CO2{struct active} = rate of CO<sub>2</sub> loss during transformation of structural to	active C (0.45, @parton1987analysis)
* AG{struct}\_to\_C{active act} = above ground structural C decomposed to active C after accounting for CO<sub>2</sub> loss C (kg ha$^{-1}$)
* AG{struct}\_to\_C{active loss} = above ground structural C being lost as CO2 during decomposition into active C (kg ha$^{-1}$)
...
* AG{struct}\_to\_C{slow loss} = decomposing above ground structural C being lost as CO<sub>2</sub> during decomposition into slow C (kg ha$^{-1}$)
* AG{struct}\_to\_C{slow act}  = above ground structural C decomposed to slow C after accounting for CO<sub>2</sub> loss (kg ha$^{-1}$)
* frac\_CO2{struct\_to\_slow} = rate of CO2 loss during transformation of structural to slow C (0.3,  Partonet al. 1987)
...
* G{met}\_to\_C{active loss} = below ground metabolic C being lost as CO<sub>2</sub> during decomposition into active C (kg ha$^{-1}$)
* BG{met}\_to\_C{active act} = below ground metabolic C decomposed to active C after accounting for CO<sub>2</sub> loss (kg ha$^{-1}$)
...
* BG{struct}\_to\_C{active loss} = below ground structural C being lost as CO<sub>2</sub> during  decomposition into active C (kg ha$^{-1}$)
* BG{struct}\_to\_C{active act} = below ground structural C decomposed to active C after accounting for CO<sub>2</sub> loss (kg ha$^{-1}$)
...
* BG{struct}\_to\_C{slow loss} = below ground structural C being lost as CO<sub>2</sub> during  decomposition into active C (kg ha$^{-1}$)
* BG{struct}\_to\_C{slow act} = below ground structural C decomposed to active C after accounting for CO<sub>2</sub> loss (kg ha$^{-1}$)
...
* AG\_to\_C{active} = above ground C decomposed into the active carbon pool (kg ha$^{-1}$)
* BG\_to\_C{active} = below ground C decomposed into the active carbon pool (kg ha$^{-1}$)
...
* C = soil carbon (kg ha$^{-1}$)
* C{mg} = soil carbon (mg ha$^{-1}$)
* C{g} = soil carbon (g ha$^{-1}$)
...
* AG{CO<sub>2</sub> loss} = above ground C lost as CO<sub>2</sub> (kg ha$^{-1}$)
* BG{CO<sub>2</sub> loss} = below ground C lost as CO<sub>2</sub> (kg ha$^{-1}$)
...
* C{CO<sub>2</sub> loss} = total C lost as CO<sub>2</sub> (kg ha$^{-1}$)
...
::: {#eq-sc-crp-6 style="font-size:90%"}
...
$$
\text{max\_water\_uptake}_\text{layer} = \text{max\_water\_uptake}_\text{bottom\_depth} - \text{max\_water\_uptake}_\text{top\_depth}
$$
:::

*Where*: 

* max_water_uptake_{bottom_depth} = potential water uptake for the lower boundary of the soil layer (mm H<sub>2</sub>O) 
* max_water_uptake_{top_depth} = potential water uptake for the upper boundary of the soil layer (mm H<sub>2</sub>O)
...
$$
\begin{aligned}
\text{potential\_water\_updake}_\text{layer} &= \text{max\_water\_uptake}_\text{layer} + \text{unmet\_demand}_\text{layer} \\[8pt]
&\qquad \times \text{ uptake\_compensation\_layer}
\end{aligned}
$$
:::
...
* potential_water_uptake_{layer} = adjusted potential water uptake for layer (mm H<sub>2</sub>O)
...
* available_water_capcity_{layer} = point at which plant available water in the soil layer begins to limit efficiency of plant uptake
* soil_water_{layer} = soil water in a given layer (mm H<sub>2</sub>O)
...
$$
\begin{aligned}
\text{actual\_water\_uptake\_layer} &= min(\text{adjusted\_potential\_water\_uptake}_\text{layer}, \\[8pt]
&\qquad \text{ soil\_water}_\text{layer} - \text{wilting\_point}_\text{layer})
\end{aligned}
$$
:::
...
* `actual_water_uptake_layer` = actual water for the layer (mm H<sub>2</sub>O)
* wilting_point_{layer} = water content of the layer at wilting point (mm H<sub>2</sub>O) 
...
* actual_water_uptake_later_i = actual water for layer (mm H<sub>2</sub>O) and n is the number of layers in the soil profile
...
* `daylength_min` = minimum day length for the location during the year (hrs)
...
* fr{PHU} = fraction of potential heat units
* fr{PHU,harvest,min} = minimum fraction of potential heat units acquired to warrant  harvest (user input)
      
The parameters that drive growth (fr{PHU}, HU{sum}, fr{LAI,max}) are reset to 0 and crop biomass, nitrogen, and phosphorus are reduced by 10% of their value at the time of harvest. Biomass lost when entering dormancy is added to residue.
...
* root_depth_{max} = maximum depth of root development in the soil (mm)
...
* nitrogen_uptake_{layer} = potential N uptake from the surface soil layer (kg N ha$^{-1}$)
...
* potential_nitrogen_uptake{depth,bottom} = potential N uptake from the soil surface to the lower boundary of the soil layer (kg N ha$^{-1}$)
* nitrogen_uptake{depth,upper} = potential N uptake from the soil surface to the upper boundary of the soil layer (kg N ha$^{-1}$)
...
* nitrogen_uptake{layer} = potential N uptake for the soil layer (kg N ha$^{-1}$)
...
* N{up},over = sum of potential N uptake for each layer overlying the given layer (kg N ha$^{-1}$) 
* NO$_3${over} = sum of nitrate content of each soil layer overlying the given layer (kg N ha$^{-1}$)
...
* `S` = surface layer
* `potential_nitrogen_uptake{layer}` = potential uptake for the given soil layer (kg N ha$^{-1}$)
* soil_nitrate{layer} is the nitrate content of the given soil layer (kg NO<sub>3</sub>-N ha$^{-1}$)

actual_nitrogen_uptake{total} for the entire profile on a given day is the sum of N{actualup} for each layer.
...
* soil_nitrates{layer},i = nitrate content in the given soil layer (kg N ha$^{-1}$)
...
* E{t} = actual amount of transpiration on a given day (mm H<sub>2</sub>O)
...
::: {#eq-sc-crp-77 style="font-size:90%"}
