...
* `surface_area` (m^2/animal) = surface area per stall, provided in @tbl-mn-surface-area

```{python}
#| label: tbl-mn-surface-area
...
Note that both slurry storage methods inherit some behavior from the base class, Storage. Because the two methods are highly similar, the content below is representative of both types of slurry storage, with specific differences between the two noted explicitly (@tbl-mn-ss-classes).
