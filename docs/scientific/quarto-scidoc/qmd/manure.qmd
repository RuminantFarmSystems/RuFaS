...
* When the end of the storage’s user defined storage time is reached, the accumulated manure in storage is removed completely and entirely, and is passed to the next processor in the chain (e.g., a subsequent storage, field application, etc.) unless otherwise stated in the specific processor's documentation section.
...
Manure handlers in RuFaS have two primary functions: calculate and add cleaning water if used, and calculat indoor housing emissions of CH<sub>4</sub>, CO<sub>2</sub>, and NH<sub>3</sub> if applicable.

The following are important assumptions made in the manure handler processors, as well as context on how manure handlers are used in the larger manure management chain:
...
```{python}
#| label: tbl-mn-hdl-classes
#| tbl-cap: The classes of handlers that are used in this module.
import_table(
  "../resources/table_data/manure/tbl-mn-hdl-classes.csv",
  colalign = ["left", "left"]
)
```
...
```{python}
#| label: tbl-mn-hdl-inputs
#| tbl-cap: "Required inputs for manure handlers."
import_table(
  "../resources/table_data/manure/tbl-mn-hdl-inputs.csv",
...
* `exposed_manure_surface_area` (m$^2$): surface area soiled with manure. See section below on determining soiled surface area
...
* Alley scraper, manual scraping, flush system:
  * `housing_methane` (kg): daily CH<sub>4</sub> (kg) emitted from manure on barn floors
  * `housing_ammonia_N_emissions` (kg): daily NH<sub>3</sub>-N (kg) emitted from manure on barn floors
  * `housing_carbon_dioxide` (kg): daily CO<sub>2</sub> (kg) emitted from manure on barn floors

### Methodology

**Calculate barn temperature** `_determine_barn_temperature`

Air temperature inside barns is, in general, slightly cooler than the outdoors during the day, and slightly-to-considerably warmer than the outdoor temperature overnight, depending on the season. With this, basic bounds were implemented in the determination of barn temperature. These bounds were suggested by manure SMEs and are supported by barn temperature ranges reported in @Bucklin2009 (Florida, upper limit). The lower bound (5°C) suggested by SMEs was based on general industry standards/conditions.
...
**Calculate cleaning/flushing fresh water addition** `determine_handler_cleaning_water_volume`
...
$$
\begin{aligned}
\text{cleaning\_water (L)} &= \text{num\_animals} \times \text{cleaning\_water\_use\_rate} \\[8pt]
&\qquad \times (1 - \text{cleaning\_water\_recycle\_fraction})
\end{aligned}
$$
:::
...
$$
\begin{aligned}
\text{total\_cleaning\_water (m}^3\text{)} &= (\text{cleaning\_water (L)} + \text{fresh\_water\_volume\_used\_for\_milking (L)}) \\[8pt] 
& \qquad \times \text{LITERS\_TO\_CUBIC\_METERS}
\end{aligned}
$$
:::
...
**Calculate housing CH<sub>4</sub> emissions**

Manure deposited on barn floors or other housing areas is a small source of CH<sub>4</sub> emissions. The following method, from @Chianese2009a, is an empirically-derived equation consisting of a simple relationship between manure CH<sub>4</sub> emission rate, ambient temperature in the barn, and surface area covered by manure.

::: {.callout-note}
The soiled surface area (m$^2$) value is calculated in the Animal module, using pen-level information (number of stalls, number of cows, etc.) and the soiled surface area value is passed to the manure module directly. As no housing emissions are considered to be emitted in the milking parlor, the total soiled surface area for each pen is apportioned to the pen's general manure streams(s) based on the user-provided stream proportion value(s).
:::
...
* `surface_area` (m^2/animal) = surface area per stall, provided in @tbl-mn-hdl-surface-area.

```{python}
#| label: tbl-mn-hdl-surface-area
#| tbl-cap: The surface area per stall that is covered with manure is a function of the pen type and type of animal housed.
import_table(
  "../resources/table_data/manure/tbl-mn-hdl-surface-area.csv",
  colalign = ["left", "center", "center"]
)
```

We use this surface area value to determine CH<sub>4</sub> emissions (kg) each day:

:::{#eq-mn-met-1}
[[**MN.MET.1**]]{.aside .content-visible when-format="html"}
$$
\text{housing\_CH}_4\text{ (kg)} = max(0.0, 0.13 \times \text{barn\_temperature} (^{\circ}\text{C})) \times \frac{\text{soiled\_surface\_area (m}^2\text{)}}{1000}
$$
:::
...
**Calculate housing CO<sub>2</sub> emissions**

Housing CO<sub>2</sub> emissions are estimated similarly to CH<sub>4</sub> emissions, using an empirical equation based on soiled surface area and barn air temperature.

:::{#eq-mn-hdl-5}
[[**MN.HDL.5**]]{.aside .content-visible when-format="html"}
$$
\text{housing\_CO}_2\text{ (kg)} = max(0.0, 0.0065 + \text{barn\_temperature} (^{\circ}\text{C})) \times \text{soiled\_surface\_area (m}^2\text{)}
$$
:::
...
The total quantity of VS loss through microbial degradation is estimated based on CH<sub>4</sub> emissions.
...
* `housing_CH4` (kg) = daily emissions of CH<sub>4</sub> from manure on barn floors, calculated in [MN.MET.1](#eq-mn-met-1).
* `VS_TO_METHANE_LOSS_RATIO` (kg) = a constant set to 6.665 representing the kg of total VS degraded per kg of CH<sub>4</sub> emitted from manure. See the Slurry Storage section for more information on derivation of this constant.
...
**Calculate housing NH<sub>3</sub> emissions**

Ammonia N loss from manure is predicted using methods based on @Rotz2006. Here, estimated NH<sub>3</sub>-N volatilized from the surface of manure is based on a function where the NH<sub>3</sub>-N is transported to the free atmosphere through a pathway with a finite resistance. Daily NH<sub>3</sub>-N loss depends primarily on pen type (which determines soiled surface area), manure TAN content, and barn air temperature. The same base equation for NH<sub>3</sub>-N volatilization is utilized for estimation of both housing and storage NH<sub>3</sub>-N emissions.

In the original work of @Rotz2006, housing NH<sub>3</sub>-N emissions are based only on urine (not manure) TAN concentration, rather than total manure (urine + feces) TAN. Within RuFaS, urine nutrients are not tracked separately from feces/manure. Therefore, the model assumes, for simplicity, that all TAN originates from urine and thus animal-excreted urine TAN = manure TAN. However, to maintain consistency with documentation, what would ordinarily in RuFaS be referred to as manure TAN within RuFaS is referred to here as urine TAN, though urine mass here (kg/d) is reflective of actual estimated manure mass, not manure (urine + feces) mass.

First, we need to derive the value of the equilibrium coefficient Q for the NH<sub>3</sub> gas in the air for a given concentration of TAN in stored slurry using Henry's law of distribution.

::: {.callout-note}
The concentration of NH<sub>3</sub> in the free atmosphere is assumed to be zero. Since Q is a function of the Henry's law coefficient K<sub>h</sub> and a dissociation of ammonium coefficient K<sub>a</sub>, we will calculate those first.
:::
...
$$
\text{K}_\text{h} = 10^{\frac{1478}{\text{barn\_temperature} - 1.69}}
$$
:::
...
$$
\text{K}_\text{a} = 1 + 10^{\left(0.09018 + \left(\frac{2729.9}{\text{barn\_temperature}} - \text{pH}\right)\right)}
$$
:::
...
* K_<sub>a</sub>: Dissociation coefficient of ammonium, calculated in [MN.AMM.2](#eq-mn-amm-2).

Second, we determine the effective total resistance value of NH<sub>3</sub> transport from manure solution to the surface and from the manure surface to the atmosphere.

:::{#eq-mn-amm-4}
[[**MN.AMM.4**]]{.aside .content-visible when-format="html"}
$$
\begin{aligned}
\text{ammonia\_resistance} &= \text{HOUSING\_SPECIFIC\_CONSTANT} \\[8pt]
&\qquad \times (1 - 0.027 \times (20.0 - \text{barn\_temperature}))
\end{aligned}
$$
:::
...
Next, the rate of NH<sub>3</sub>-N loss in kg N/m$^2$ from manure on the barn floor is calculated:

:::{#eq-mn-amm-5}
[[**MN.AMM.5**]]{.aside .content-visible when-format="html"}
$$
\text{NH}_3\text{N emission rate (kg N/m}^2\text{)} = \frac{\text{TAN} \times \text{c} \times \text{y}}{\text{r} \times \text{M} \times \text{Q}}
$$
:::
...
* r = the resistance of NH<sub>3</sub> transfer from solution to manure surface, and from manure surface to atmosphere, determined in [MN.AMM.4](#eq-mn-amm-4)
* M = urine mass (received from animal module)
* Q = equilibrium coefficient, calculated in [MN.AMM.3](#eq-mn-amm-3)

Lastly, we calculate total NH<sub>3</sub>-N emissions (kg), based on the emission rate we just calculated and the soiled surface area.
...
* NH<sub>3</sub>N_rate (kg N/m$^2$) = Rate of NH<sub>3</sub>-N loss (kg/m^2) from manure, calculated in [MN.AMM.5](#eq-mn-amm-5)
* `soiled_surface_area` (m$^2$) = the total surface area soiled with manure
...
::: {.callout-note}
Some formulas below are a summarization of multiple steps detailed above, and are intended to provide an overview of what mass losses/gains are reflected in the value of each variable.
:::
...
```{python}
#| label: tbl-mn-hdl-manure-update
#| tbl-cap: Calculated manure storage variables and their units.
import_table(
  "../resources/table_data/manure/tbl-mn-hdl-manure-update.csv",
  colalign = ["left", "center", "left"]
)
```
...
* Separated solids (`SeparatedSolids`) representing the separated manure solids
* Liquid fraction (`SeparatedLiquid`) representing the separated liquid
...
* `storage_surface_area` = the user-defined or model-estimated storage surface area (m$^2$). 
...
* Ln(A) = The natural log of the Arrhenius parameter (`NATURAL_LOG_ARRHENIUS_CONSTANT` constant), set at 30.6 based on @Petersen2024. This is an empirically-derived value determined based on observed manure CH<sub>4</sub> emission values.
* `ACTIVATION_ENERGY` = the apparent activation energy of methanogenesis in cattle slurry (J/mol), set at 81,000 J/mol, based on @Elsgaard2016. 
* Gas constant = ideal gas constant, set at 8.314 J K/mol.
* Manure temperature (K) = temperature of manure in storage. 
...
$$
\text{retained\_manure}_\text{i} = \text{accumulated\_manure}_\text{i} \times \text{ANAEROBIC\_LAGOON\_MANURE\_RETENTION}
$$
:::
