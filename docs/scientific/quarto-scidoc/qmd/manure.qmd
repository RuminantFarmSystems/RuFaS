...
$$
\text{updated\_`ammoniacal_nitrogen` (kg)} = min(\text{`ammoniacal_nitrogen`(kg)} \times \text{TAN\_INCREASE\_FACTOR},\text{nitrogen (kg)})
$$
:::
...
#| tbl-cap: Methane conversion factor values for bedded pack pens, based on presence or absenc of mixing and average annual air temperature.
import_table(
...
&\qquad \times \frac{\text{oxygen\_half\_saturation\_constant} + \text{oxygen\_ambient\_air_mole\_fraction}}{\text{oxygen\_ambient\_air\_mole\_fraction}} \\[8pt]
...
$$
\text{carbon\_from\_VSd (kg)} = \text{degradable\_volatile\_solids} \times \text{DEFAULT\_CARBON\_FRACTION\_AVAILABLE\_IN\_VSD}
$$
:::
...
$$
\text{carbon\_from\_VSnd (kg)} = \text{non\_degradable\_volatile\_solids} \times \text{DEFAULT\_CARBON\_FRACTION\_AVAILABLE\_IN\_VSND}
$$
:::
...
:::{#eq-mn-amm-8}
[[**MN.AMM.***]]{.aside .content-visible when-format="html"}
$$
\text{storage\_ammonia\_N (kg)} = \text{daily\_manure\_N (kg)} \times \text{ammonia\_coefficient}
$$
:::
...
In addition to NH<sub>3</sub>-N emissions, manure nitrogen deposition in the bedded pack also results in N2O emissions. Similar to NH<sub>3</sub>, we estimate daily N<sub>2</sub>O-N loss using an emissions factor from Hanson et al. (2024) based on the type of management used. 

:::{}
[See [MN.NIT.1](#eq-mn-nit-1)]{.aside .content-visible when-format="html"}
$$
\text{storage\_nitrous\_oxide\_N (kg)} = \text{daily\_manure\_N (kg)} \times \text{nitrous\_oxide\_coefficient}
$$
:::
...
$$
\text{storage\_leached\_N (kg)} = \text{daily\_manure\_N (kg)} \times \text{LEACHING\_COEFFICIENT}
$$
:::
...
Calculates the daily mass of methane emitted from the open lot based on daily manure volatile solids (VS) added to lot manure (through animal excretion and bedding addition) and the emission factor calculated in [MN.MET.5]{@eq-mn-met-5}. 

\textbf{Note that the quantity of volatile solids utilized in determination of CH<sub>4</sub> includes only manure-excreted volatile solids; bedding volatile solids are excluded.} Here and in other equations, ‘daily’ denotes the value associated with received manure added to the open lot on a specified simulation day.
...
In addition to NH<sub>3</sub>-N emissions, nitrogen added to and accumulating in the compost also results in  \(N_2O\) emissions. Similar to NH<sub>3</sub>, here we utilize daily manure N addition to compost and an emission factor based on composting method \parencite{Hanson2024} to estimate total kg of  \(N_2O-N\) loss.
...
Stored manure outputs represent the accumulated quantity of manure and nutrients present in storage on a single day. These values are the net quantity of mass/nutrients remaining each day after adding received manure values and subtracting any losses to gas emissions or other losses. In composting processors, daily losses include CH<sub>4</sub>, NH<sub>3</sub>, N leaching, and N2O emissions. The order of operations in updating accumulated manure values is: 
