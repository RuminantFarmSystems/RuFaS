...
def test_calc_optimal_nitrogen_fraction(heatfrac, emerge, mature, shape1, shape2):
...
def test_calc_fixed_nitrogen(demand, stage, water, nitrate, expect):
    """check that nitrogen values are calculated as expected with calc_fixed_nitrogen()"""
    assert calc_fixed_nitrogen(demand, stage, water, nitrate) == expect
...
def test_error_calc_fixed_nitrogen(demand, stage, water, nitrate):
    with pytest.raises(ValueError):
        calc_fixed_nitrogen(demand, stage, water, nitrate)
...
def test_determine_nitrogen_shape_parameters(half_heat, mat_heat, emerge, half, near, mature):
    ...
    assert incorp.shapes_nitrogen_uptake == calc_shape_parameters(half_heat, mat_heat, emerge, half, near, mature)
...
def test_determine_actual_nitrogen_uptakes(requests, nitrates):
    ...
    incorp.determine_actual_nitrogen_uptakes(nitrates)
    expect = calc_layer_extracted_resource(requests, nitrates)
