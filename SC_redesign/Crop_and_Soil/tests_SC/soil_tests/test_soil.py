...
@pytest.mark.parametrize("solar_radiation,avg_temp,min_temp,max_temp,plant_cover,snow_cover,avg_annual_air_temp", [
    (20, 18, 16, 25, 36, 77, 24),
    (20.1, 18.36, 16.48, 25.31, 36.66, 77.34, 24.99)
])
def test_daily_soil_routine(solar_radiation: float, avg_temp: float, min_temp: float, max_temp: float,
    ...
    soil.daily_soil_routine(solar_radiation, avg_temp, min_temp, max_temp, plant_cover, snow_cover, avg_annual_air_temp)
    assert soil.soil_temp.daily_soil_temperature_update.call_count == 1
    soil.soil_temp.daily_soil_temperature_update.assert_called_once_with(solar_radiation, avg_temp, min_temp, max_temp,
                                                                         plant_cover, snow_cover, avg_annual_air_temp)


@pytest.mark.parametrize("rainfall,weighting_coefficient,potential_evapotranspiration,has_seasonal_high_water_table,"
...
def test_daily_soil_water_routine(rainfall: float, weighting_coefficient: float, potential_evapotranspiration: float,
    ...
    soil.daily_soil_water_routine(rainfall, weighting_coefficient, potential_evapotranspiration,
                                  has_seasonal_high_water_table, maximum_soil_evaporation, avg_air_temp, residue,
                                  minimum_cover_management_factor, field_size)
    ...
    soil.phosphorus_cycling.cycle_phosphorus.assert_called_once_with(rainfall, soil.data.accumulated_runoff, field_size,
                                                                     avg_air_temp)
