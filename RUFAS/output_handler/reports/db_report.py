...
class DBReport(BaseReport):
    def __init__(self, data, fPath):
        ...
        self.annual_variables = {
            ...
            'avg_num_calf': ['sum(animal_management.num_calf_lst) / len('
                             'animal_management.num_calf_lst) if len(animal_management.num_calf_lst) > 0 else 0', '', 0],
            'avg_num_heiferI': ['sum(animal_management.num_heiferI_lst) / '
                                'len(animal_management.num_heiferI_lst) if len(animal_management.num_heiferI_lst) > 0 else 0', '', 0],
            'avg_num_heiferII': ['sum(animal_management.num_heiferII_lst) / '
                                 'len(animal_management.num_heiferII_lst) if len(animal_management.num_heiferII_lst) > 0 else 0', '', 0],
            'avg_num_heiferIII': ['sum(animal_management.num_heiferIII_lst) / '
                                  'len(animal_management.num_heiferIII_lst) if len(animal_management.num_heiferIII_lst) > 0 else 0', '', 0],
            'avg_num_cow': ['sum(animal_management.num_cow_lst) / len('
                            'animal_management.num_cow_lst) if len(animal_management.num_cow_lst) > 0 else 0', '', 0],
            'avg_milk_prod_by_cow_per_year': ['sum('
                                              'animal_management.avg_milk_lst) / len(animal_management.avg_milk_lst) if len(animal_management.avg_milk_lst) > 0 else 0', '', 0],
        ...
        self.purchased_feed_info_variables = {
            ...
            'total_purchased_feed': ['animal_management.purchased_feed_amounts[str(feed_id)] if '
                                     'str(feed_id) in '
                                     'animal_management.purchased_feed_amounts.keys() else 0', '',
                                     []],
            'total_cost': ['feed.feed_costs[str(feed_id)] * '
        ...
        self.cow_print_variables = {
            ...
            'milk_protein_prod': ['animal_management.annual_milk_protein_prod', '', 0],
            'milk_fat_prod': ['animal_management.annual_milk_fat_prod', '', 0],
            'avg_prod_per_lactation': ['animal_management.total_milk_prod_per_lactation / '
            ...
            'heiferI_animal_days': ['animal_management.heiferI_animal_days', '', 0],
            'heiferII_animal_days': ['animal_management.heiferII_animal_days', '', 0],
            'heiferIII_animal_days': ['animal_management.heiferIII_animal_days', '', 0],
            'lactating_cow_animal_days': ['animal_management.lactating_cow_animal_days', '', 0],
            'dry_cow_animal_days': ['animal_management.dry_cow_animal_days', '', 0]
        }
        ...
        self.feed_print_variables = {
            ...
            'lactating_cow_DMI': ['animal_management.annual_lactating_cow_DMI', '', 0],
        ...
        self.daily_manure_variables = {
            ...
            'j_day': ['time.day', '', []],
            'pen_id': ['pen_id', '', []],
            'number_animals': ['animal_management.get_num_animals_from_pen_id(pen_id)', '', []],
            'treatment': ['0', '', []],
            'housing_methane': ['0', '', []],
        ...
        self.annual_manure_variables = {
            ...
            'num_days_in_year': ['sum([(0 if day == None else 1) for day in '
                                 'time.years[time.year - 1]])', '', []],
            'pen_id': ['pen_id', '', []],
            'average_number_animals': ['animal_management.get_avg_num_animals_from_pen_id(pen_id)',
                                       '', []],
            'treatment': ['0', '', []],
            'housing_methane': ['0', '', []],
            'housing_direct_nitrous_oxide': ['0', '', []],
    ...
    def write_annual_report(self):
        ...
        try:
            ...
            # The number of rows produced is the same as the number of entries in the list for
            ...
            insert_statement = \
                ...
                "({})".format(','.join(['?'] * len(self.daily_manure_variables)))
            ...
            insert_statement = \
                ...
                "({})".format(','.join(['?'] * len(self.purchased_feed_info_variables)))

            row_list = []
            first_key = list(self.purchased_feed_info_variables.keys())[0]
            # The number of purchased feeds is the same as the number of entries in the list for
            # each of the variables.
            # TODO add better documentation about this and pen ids
            num_purchased_feeds = len(self.purchased_feed_info_variables[first_key][2])
            for i in range(num_purchased_feeds):
                curr_row = []
                for variable in self.purchased_feed_info_variables:
                    curr_row.append(self.purchased_feed_info_variables[variable][2][i])
                row_list.append(tuple(curr_row))
            ...
            insert_statement = \
                ...
                "({})".format(','.join(['?'] * len(self.grown_feed_info_variables)))

            row_list = []
            first_key = list(self.grown_feed_info_variables.keys())[0]
            # The number of growing feeds is the same as the number of entries in the list for
            # each of the variables.
            num_growing_feeds = len(self.grown_feed_info_variables[first_key][2])
            for i in range(num_growing_feeds):
                curr_row = []
                for variable in self.grown_feed_info_variables:
                    curr_row.append(self.grown_feed_info_variables[variable][2][i])
                row_list.append(tuple(curr_row))
            ...
            insert_statement = \
                ...
                "({})".format(','.join(['?'] * len(self.energy_print_variables)))
            ...
            insert_statement = \
                ...
                "({})".format(','.join(['?'] * len(self.annual_manure_variables)))

            row_list = []
            first_key = list(self.annual_manure_variables.keys())[0]
            # The number of pens is the same as the number of entries in the list for
            # each of the variables.
            num_pens = len(self.annual_manure_variables[first_key][2])
            for i in range(num_pens):
                curr_row = []
                for variable in self.annual_manure_variables:
                    curr_row.append(self.annual_manure_variables[variable][2][i])
                row_list.append(tuple(curr_row))
        ...
        for pen in animal_management.all_pens:
            ...
            for variable in self.daily_manure_variables:
                self.daily_manure_variables[variable][2].append(
                    eval(self.daily_manure_variables[variable][0], globals(), locals()))
    ...
    def annual_update(self, state, weather, time):
        ...
        for feed_id in feed.purchased_feeds:
            for variable in self.purchased_feed_info_variables:
                self.purchased_feed_info_variables[variable][2].append(
                    eval(self.purchased_feed_info_variables[variable][0], globals(), locals()))
        ...
        for feed_id in total_yields.keys():
            for variable in self.grown_feed_info_variables:
                self.grown_feed_info_variables[variable][2].append(
                    eval(self.grown_feed_info_variables[variable][0], globals(), locals()))
        ...
        for variable in self.feed_print_variables:
            self.feed_print_variables[variable][2] = \
                eval(self.feed_print_variables[variable][0], globals(), locals())

        for variable in self.energy_print_variables:
            self.energy_print_variables[variable][2] = \
                eval(self.energy_print_variables[variable][0], globals(), locals())

        for pen in animal_management.all_pens:
            pen_id = pen.id
            for variable in self.annual_manure_variables:
                self.annual_manure_variables[variable][2].append(
                    eval(self.annual_manure_variables[variable][0], globals(), locals()))
...
- test run to see if results are written correctly/values arent missed
- add variables to eval() part that already exist
- add variables in animal module for variables that we can calculate
- test run to see if results are written correctly/values arent missed 

- maybe make some of the above into functions
'''