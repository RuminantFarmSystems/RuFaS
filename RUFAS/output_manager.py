...
class OutputManager(object):
    ...
    def _dict_to_file_csv(self, data_dict: Dict[str, Any], path: str, field: str) -> None:
        ...
        field : str
            The field from the data_dict that should be saved
        ...
        if all(field not in v for v in data_dict.values()):
            self.add_error("save_dict_file_try", f"Unable to save {path} due to missing values.", info_map)
            return

        df = pd.DataFrame({k: pd.Series(v[field], dtype=object) for k, v in data_dict.items()})
        try:
    ...
    def save_variables_to_csv_files(self, base_csv_dir: str, exclude_info_maps: bool = False) -> None:
        """
        ...
        variables_path = Path(base_csv_dir) / "variables"
        info_map_path = Path(base_csv_dir) / "info_maps"
        try:
            variables_path.mkdir(parents=True, exist_ok=True)
            if not exclude_info_maps:
                info_map_path.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            raise e
    ...
    def dump_all_pools(self, path: str, exclude_info_maps: bool = False) -> None:
        ...
        self.save_variables_to_csv_files(str(Path(path) / "CSVs" / "om"), exclude_info_maps)
