...
class EconomicFramework:
        ...
        if is_dataframe:
            ...
            if has_cost_column:
                ...
                if total_declared_costs == 0.0:
                    self.om.add_warning("ZeroCapitalCost", "Capital cost breakdown sums to zero.", info_map)
                    return False
                return True
    ...
    def _goal_seek_config(self) -> tuple[dict[str, str], set[str]]:
        goal_seek_name_map = {
        ...
        return goal_seek_name_map, allowed_goal_seek_variables

    def _solve_goal_seek(
        self,
        calculator: DCFRORCalculator,
        calculator_inputs: dict,
        goal_seek_name_map: dict[str, str],
        allowed_goal_seek_variables: set[str],
    ) -> tuple[float, str]:
        fixed_variables = calculator_inputs.get("goal_seek_fixed_variables", [])
        if not isinstance(fixed_variables, list):
            fixed_variables = []

        info_map = {"class": __name__, "function": self.run_economic_analysis.__name__}
        ...
        return solved_value, solved_variable

    def _add_goal_seek_outputs(self, solved_value: float, solved_variable: str) -> None:
        info_map = {
            "class": __name__,
            "function": self.run_economic_analysis.__name__,
            "units": MeasurementUnits.UNITLESS,
        }
        self.om.add_variable("econ_dcfror_goal_seek_solution", solved_value, info_map=info_map)
        self.om.add_variable("econ_dcfror_goal_seek_variable", solved_variable, info_map=info_map)
    ...
    def run_economic_analysis(self) -> None:
        ...
        if capital_present:
            calculator = DCFRORCalculator()
            goal_seek_name_map, allowed_goal_seek_variables = self._goal_seek_config()
            ...
            if calculator_inputs.get("enable_goal_seek", False):
                solved_value, solved_variable = self._solve_goal_seek(
                    calculator,
                    calculator_inputs,
                    goal_seek_name_map,
                    allowed_goal_seek_variables,
                )
                self._add_goal_seek_outputs(solved_value, solved_variable)
