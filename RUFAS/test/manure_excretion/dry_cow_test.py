...
File name: dry_cow_test.py
Description: This file contains the manure excretion subroutine unit tests for dry cows.
...
from RUFAS.routines.animal.manure import dry_cow_manure_excretion as manure_excretion
from RUFAS.test.manure_excretion import inputs_outputs


class DryCowTest(unittest.TestCase):
    ...
    def test_manure_1(self):
        inputs = self.inputs_outputs.dry_input_1
        input_ration = self.inputs_outputs.ration_formulation0
        expected_manure = self.inputs_outputs.expected_dry_manure_1
        feed = self.feeds[0]

        excrt, manure = \
            manure_excretion.manure_calculations(input_ration, feed,
                                                 inputs['BW'], inputs['milk_prod'],
                                                 inputs['p_feces_excrt'], inputs['p_urine'],
                                                 inputs['ME_intake'])
