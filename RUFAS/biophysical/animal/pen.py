...
class Pen:
    ...
    def handle_calf_ration(
        ...
        """
        self.set_animal_nutritional_requirements(temperature=temperature, available_feeds=pen_available_feeds)
        target_dry_matter_intake = RationManager.resolve_target_dmi(self.animal_combination, self)

        if is_ration_defined_by_user:
            ration_per_calf = RationManager.get_user_defined_ration(
                animal_combination=self.animal_combination, target_dry_matter_intake=target_dry_matter_intake
            )
        else:
            # TODO make the below its own method: get avg calf INTAKE
            ...
            for calf in list(self.animals_in_pen.values()):
                ...
                # TODO consider reporting the above in AnimalReporter?
            ...
            scaling = target_dry_matter_intake / sum(average_calf_intake.values())
            scaled_average_calf_intake = {intake: value * scaling for intake, value in average_calf_intake.items()}

            ration_per_calf = CalfRationManager.formulate_ration(
                calf_feed_ids = [x.rufas_id for x in pen_available_feeds],
                animal_intake=scaled_average_calf_intake
            )
