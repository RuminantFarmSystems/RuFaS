    ...
    def daily_update(self, animal_management: AnimalManagement) -> None:
        ...
        for pen in animal_management.all_pens:
            ...
            if ManureTreatmentType.get_type(mm_pen.manure_treatment) is \
                    ManureTreatmentType.ANAEROBIC_DIGESTION_AND_LAGOON or \
                    ManureTreatmentType.get_type(mm_pen.manure_treatment) is \
                    ManureTreatmentType.ANAEROBIC_DIGESTION_AND_LAGOON_WITH_SPLIT:

                # Currently, calling the daily_update on the treatment only returns one output
                ...
                # Notes on the second parameter - manure_treatment_daily_input:
                # If there exists a manure separator, then the manure treatment will receive the output
                # of the separator. Otherwise, it will receive the output of the reception pit.
                # Hence, the use of the more general name 'daily_input' instead of
                # 'manure_separator_daily_output' or 'reception_pit_daily_output'.
                manure_treatment_daily_output = self.manure_treatments[mm_pen.id].daily_update(
                        manure_handler_daily_output=manure_handler_daily_output,
