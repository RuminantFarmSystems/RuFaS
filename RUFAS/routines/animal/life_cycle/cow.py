...
class Cow(HeiferIII):
    ...
    def _milking_update(self):
        ...
        if AnimalBase.config['lactation_curve'] == 'wood':
            l = self._determine_param_value(AnimalBase.config['l'][breed_index][parity_index],
                                            AnimalBase.config['l_std'][breed_index][parity_index])
            m = self._determine_param_value(AnimalBase.config['m'][breed_index][parity_index],
                                            AnimalBase.config['m_std'][breed_index][parity_index])
            n = self._determine_param_value(AnimalBase.config['n'][breed_index][parity_index],
            ...
            estimated_daily_milk_produced = AnimalBase.config['a'] * \
                                            (1 - math.exp(
                                                (AnimalBase.config['c'] - self._days_in_milk) / AnimalBase.config[
                                                    'b']) / 2) * \
        ...
        fat_percent = 12.86 * self._days_in_milk ** (-1.081) * math.exp(
            (0.0926) * (math.log(self._days_in_milk)) ** 2) * (math.log(self._days_in_milk) ** 1.107)
        ...
        if self._calves == 1:
            self._body_weight = self._mature_body_weight * (
                        1 - (1 - (self._birth_weight / self._mature_body_weight) ** (1 / 3)) * math.exp(
                    -0.0039 * self._days_born)) ** 3 - (20 / 65) * self._days_in_milk * math.exp(
                1 - self._days_in_milk / 65) + 0.0187 ** 3 * (self._days_in_preg - 50) ** 3
        else:
            self._body_weight = self._mature_body_weight * (
                        1 - (1 - (self._birth_weight / self._mature_body_weight) ** (1 / 3)) * math.exp(
                    -0.006 * self._days_born)) ** 3 - (40 / 75) * self._days_in_milk * math.exp(
                1 - self._days_in_milk / 75) + 0.0187 ** 3 * (self._days_in_preg - 50) ** 3
    ...
    def calc_nutrient_rqmts(self, housing, pasture_concentrate, nutrient_rqmts):
        if self._milking:
            result = lactating_calculate_rqmts(self._body_weight, self._BCS, self._CBW, self._CI, pasture_concentrate,
    ...
    def calc_init_nutrient_rqmts(self, vertical_distance, horizontal_distance, housing, pasture_concentrate,
                                 nutrient_rqmts):
    ...
    def calc_manure_excretion(self, feed):
        if self._milking:
            self._manure_excretion = lactating_manure_calculations(self._ration_formulation, feed, self._body_weight,
    ...
    def _restart_estrus(self):
        self._estrus_day = self._determine_estrus_day(self._days_born, '1st estrus after calving',
    ...
    def _later_estrus(self):
        self._estrus_day = self._determine_estrus_day(self._estrus_day, 'estrus occur before vwp',
    ...
    def _resynch_ed_tai(self, record_econ_stats):
        if self._resynch_method == 'TAIafterPD':
            ...
            self._tai_program_start_day_c = self._abortion_day + 8
            self._conception_rate -= AnimalBase.config['conception_rate_decrease']
            self._estrus_day = self._determine_estrus_day(self._abortion_day, 'estrus after PGF',
    ...
    def _cull_update(self, record_econ_stats):
        ...
        # 	return True
        if self._do_not_breed and self._days_in_milk > 80 and self._estimated_daily_milk_produced < AnimalBase.config[
            'cull_milk_production']:  # estimated_daily_milk_produced < AnimalBase.config['cull_milk_production']:
            self._culled = True
    ...
    def get_economy_stats(self):
        ...
        if self._repro_program == 'TAI':
            self._repro_cost = self._GnRH_injections * 2.4 + self._PGF_injections * 2.65 + (
                        self._GnRH_injections + self._PGF_injections) * 0.25
        if self._repro_program == 'ED-TAI':
            self._repro_cost = self._ED_days * 0.15 + self._GnRH_injections * 2.4 + self._PGF_injections * 2.65 + (
                        self._GnRH_injections + self._PGF_injections) * 0.25
