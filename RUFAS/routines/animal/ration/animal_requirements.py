...
class AnimalRequirements:
    ...
    def calculate_NRC_protein_requirements(
        ...
        TDN_estimate = 0.7
        MP_bactria_estimate = dry_matter_intake_estimate * GeneralConstants.KG_TO_GRAMS * TDN_estimate * 0.13
    ...
    def calculate_NRC_DMI(
        ...
        if animal_type in [AnimalType.LAC_COW, AnimalType.DRY_COW]:
            ...
            dry_matter_intake_estimate = (
                body_weight**0.75
                * (0.2435 * net_energy_maintenance_diet - 0.0466 * net_energy_maintenance_diet**2 - 0.1128)
                / net_energy_maintenance_diet
            )
        ...
        return max(
            dry_matter_intake_estimate,
            dry_matter_intake_estimate_minimum_percentage,
    ...
    def calculate_NASEM_DMI(
        ...
        if lactating:
            ...
            dry_matter_intake_estimate = (
                0.022 * mature_body_weight * (1 - math.exp(-1.54 * (body_weight / mature_body_weight)))
            )
        dry_matter_intake_estimate_minimum_flat = AnimalModuleConstants.MINIMUM_DMI
        dry_matter_intake_estimate_minimum_percentage = AnimalModuleConstants.MINIMUM_DMI_PERCENTAGE * body_weight
        return max(
