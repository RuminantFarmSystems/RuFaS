...
class RationOptimizer:
    ...
    def total_energy(self, x: np.ndarray, ration_config: RationConfig) -> float:
        ...
        for i in range(len(ration_config.MEact_list)):
            if ration_config.is_fat_list[i] == 1:
                ...
                ration_config.NEm_act_list.append(1.37 * ration_config.MEact_list[i] - 0.138 * ration_config.MEact_list[i] ** 2 + 0.0105 *
        ...
        for i in range(len(ration_config.MEact_list)):
            if ration_config.feed_type_list[i] == 'Mineral':
                ...
                ration_config.NElact_list.append(0.703 * ration_config.MEact_list[i] - 0.19 + ((0.097 * ration_config.MEact_list[i] +
        ...
        for i in range(len(ration_config.MEact_list)):
            if ration_config.feed_type_list[i] == 'Mineral':
                ...
                ration_config.NEgact_list.append(1.42 * ration_config.MEact_list[i] - 0.174 * ration_config.MEact_list[i] ** 2 + 
        ...
        if not ration_config.NEm_act_list:
            ...
            for i in range(len(ration_config.MEact_list)):
                if ration_config.is_fat_list[i] == 1:
                    ...
                    ration_config.NEm_act_list.append(1.37 * ration_config.MEact_list[i] - 0.138 * ration_config.MEact_list[i] ** 2 + 
                                   0.0105 * ration_config.MEact_list[i] ** 3 - 1.12)
        # returning the NEm_act constraint in the NLP
        return (sum(np.multiply(x, ration_config.NEm_act_list)) - (ration_config.NEmaint_requirement + ration_config.NEa_requirement))
            ...
            for i in range(len(ration_config.MEact_list)):
                if ration_config.feed_type_list[i] == 'Mineral':
                    ...
                    ration_config.NElact_list.append(0.703 * ration_config.MEact_list[i] - 0.19 + ((0.097 * 
                                ration_config.MEact_list[i] + 0.19) / 97) * (ration_config.EE_list[i] - 3))
                else:
                    ration_config.NElact_list.append(0.703 * ration_config.MEact_list[i] - 0.19)
            # returning the NElact constraint in the NLP
        return sum(np.multiply(x, ration_config.NElact_list)) - (ration_config.NEpreg_requirement + ration_config.NEl_requirement)
            ...
            for i in range(len(ration_config.MEact_list)):
                if ration_config.feed_type_list[i] == 'Mineral':
                    ...
                    ration_config.NEgact_list.append(1.42 * ration_config.MEact_list[i] - 0.174 * 
                                ration_config.MEact_list[i] ** 2 + 0.0122 * ration_config.MEact_list[i] ** 3 - 1.65)
    ...
    def phosphorus_constraint(self, x: np.ndarray, ration_config: RationConfig) -> float:
        ...
        return sum(np.multiply(x, np.multiply(np.multiply(ration_config.phosphorus_list, 0.01), ration_config.dP_list))) - ((ration_config.P_requirement) / 1000)
    ...
    def protein_constraint(self, x: np.ndarray, ration_config: RationConfig) -> float:
        ...
        for i in range(len(ration_config.feed_type_list)):
            if ration_config.feed_type_list[i] == 'Conc':
                ...
                Kp.append(3.362 + 0.479 * (DMI / ration_config.BW) * 100 - 0.017 * ration_config.NDF_list[i] - 0.007 * PercentConc)
        ...
        for i in range(len(ration_config.Kd_list)):
            if Kp[i] > -ration_config.Kd_list[i]:
                ration_config.RDP_list.append((ration_config.Kd_list[i] / (ration_config.Kd_list[i] + Kp[i])) * (ration_config.N_B_list[i] / 100) * ration_config.CP_list[i] + (ration_config.N_A_list[i] / 100) * ration_config.CP_list[i])
            else:
    ...
    def optimize(self, animal_combination, available_feeds: Dict, ration_config: RationConfig) -> npt.NDArray:
        ...
        if udrm.udr_or_not:
            bnds = self.make_user_bounds(UserDefinedRationManager.ration_to_use(animal_combination, available_feeds), ration_config.DMIest_requirement)
    ...
    def attempt_optimization(self, requirements: AnimalRequirements, available_feeds: Dict, animal_combination):
        ...
        if str(animal_combination) in ['AnimalCombination.LAC_COW']:
            ...
            lactating = False
        ration_config = RationConfig(price_list, requirements.NEmaint_requirement, requirements.NEa_requirement, requirements.NEpreg_requirement,
                        ...
                        TDN_list, DE_list, EE_list, is_fat_list, requirements.avg_BW, calcium_list, phosphorus_list, NDF_list,
                        feed_type_list, is_wetforage_list, Kd_list, N_A_list, N_B_list, CP_list, dRUP_list, feed_limit_list, lactating,
