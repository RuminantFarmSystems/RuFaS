...
class FieldDataReporter:
    ...
    def send_daily_variables(self) -> None:
        ...
        for field in self.fields:
            ...
            for index, layer in enumerate(field.soil.data.soil_layers):
                ...
                om.add_variable("temperature", layer.temperature,
                                dict(info_map, **{"data_origin": [("SoilTemp", "daily_soil_temperature_update")]}))
                om.add_variable("percolated_water", layer.percolated_water,
                                dict(info_map, **{"data_origin": [("Percolation", "percolate"),
                                                                  ("Percolation", "_percolate_excess_water")]}))
                om.add_variable("water_content", layer.water_content,
                                dict(info_map, **{"data_origin": [("Evaporation", "evaporate"),
                                                                  ("LayerData", "__post_init__"),
                                                                  ("Percolation", "percolate"),
                                                                  ("Percolation", "_percolate_excess_water")
                                                                  ]}))
                om.add_variable("water_factor", layer.water_factor,
                                dict(info_map, **{"data_origin": [("LayerData", "water_factor")
                                                                  ]}))
                ...
                om.add_variable("plant_metabolic_active_carbon_usage", layer.plant_metabolic_active_carbon_usage,
                                dict(info_map, **{"data_origin": [("ResiduePartition", "partition_residue")
                                                                  ]}))
                om.add_variable("plant_metabolic_active_carbon_loss", layer.plant_metabolic_active_carbon_loss,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                ...
                om.add_variable("active_carbon_to_slow_amount", layer.active_carbon_to_slow_amount,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                om.add_variable("active_carbon_to_slow_loss", layer.active_carbon_to_slow_loss,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                om.add_variable("active_carbon_to_passive_amount", layer.active_carbon_to_passive_amount,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                om.add_variable("slow_to_active_carbon_amount", layer.slow_to_active_carbon_amount,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                om.add_variable("slow_carbon_co2_lost_amount", layer.slow_carbon_co2_lost_amount,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                om.add_variable("passive_to_active_carbon_amount", layer.passive_to_active_carbon_amount,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                om.add_variable("passive_carbon_co2_lost_amount", layer.passive_carbon_co2_lost_amount,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                om.add_variable("plant_active_decompose_carbon", layer.plant_active_decompose_carbon,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                om.add_variable("soil_active_decompose_carbon", layer.soil_active_decompose_carbon,
                                dict(info_map, **{"data_origin": [("PoolGasPartition", "partition_pool_gas")
                                                                  ]}))
                om.add_variable("soil_overall_carbon_fraction", layer.soil_overall_carbon_fraction,
                                dict(info_map, **{"data_origin": [("CarbonCycling", "_soil_carbon_aggregation"),
                                                                  ]}))
                ...
                om.add_variable("fresh_organic_phosphorus_content", layer.fresh_organic_phosphorus_content,
                                dict(info_map, **{"data_origin": [("LayerData", ""),
                                                                  ]}))
                ...
                om.add_variable("ammonium_content", layer.ammonium_content,
                                dict(info_map, **{"data_origin": [("FertilizerApplication", "apply_fertilizer"),
                                                                  ("ManureApplication", "_apply_subsurface_fertilizer"),
                                                                  ("ManureApplication", "_add_nitrogen_to_soil_layer"),
                                                                  ("LayerData", "_initialize_nitrogen_pools"),
                                                                  ("HumusMineralization",
                                                                   "mineralize_organic_nitrogen"),
                                                                  ("LeachingRunoffErosion", "_erode_nitrogen"),
                                                                  ("LeachingRunoffErosion", "_leach_nitrogen"),
                                                                  ("NitrificationVolatilization",
                                                                   "do_daily_nitrification_and_volatilization")
                                                                  ]}))
                om.add_variable("active_organic_nitrogen_content", layer.active_organic_nitrogen_content,
                                dict(info_map, **{"data_origin": [("CropManagement", "_distribute_residue_nutrients"),
                                                                  ("FertilizerApplication", "apply_fertilizer"),
                                                                  ("FertilizerApplication",
                                                                   "_apply_subsurface_fertilizer"),
                                                                  ("ManureApplication", "_add_nitrogen_to_soil_layer"),
                                                                  ("LayerData", "_initialize_nitrogen_pools"),
                                                                  ("HumusMineralization",
                                                                   "mineralize_organic_nitrogen"),
                ...
                om.add_variable("stable_organic_nitrogen_content", layer.stable_organic_nitrogen_content,
                                dict(info_map, **{"data_origin": [("ManureApplication", "_add_nitrogen_to_soil_layer"),
                                                                  ("LayerData", "_initialize_nitrogen_pools"),
                                                                  ("HumusMineralization",
                                                                   "mineralize_organic_nitrogen"),
                                                                  ("LeachingRunoffErosion", "_erode_nitrogen"),
                ...
                om.add_variable("fresh_organic_nitrogen_content", layer.fresh_organic_nitrogen_content,
                                dict(info_map, **{"data_origin": [("CropManagement", "_transfer_residue"),
                                                                  ("CropManagement", "_distribute_residue_nutrients"),
                                                                  ("FertilizerApplication", "apply_fertilizer"),
                                                                  ("FertilizerApplication",
                ...
                om.add_variable("nitrous_oxide_emissions", layer.nitrous_oxide_emissions,
                                dict(info_map, **{"data_origin": [("Denitrification", "denitrify")
                                                                  ]}))
                om.add_variable("ammonia_emissions", layer.ammonia_emissions,
                                dict(info_map, **{"data_origin": [("NitrificationVolatilization",
                                                                   "do_daily_nitrification_and_volatilization")
                                                                  ]}))
            ...
            for crop in field.crops:
                ...
                om.add_variable("root_depth", crop.data.root_depth,
                                dict(info_map, **{"data_origin": [("RootDevelopment", "develop_roots")
                                                                  ]}))
                om.add_variable("biomass", crop.data.biomass,
                                dict(info_map, **{"data_origin": [("BiomassAllocation", "photosynthesize"),
                                                                  ("CropManagement", "cut_crop"),
                                                                  ("Dormancy", "enter_dormancy")
                                                                  ]}))
                om.add_variable("usable_light", crop.data.usable_light,
                                dict(info_map, **{"data_origin": [("BiomassAllocation", "photosynthesize")
                                                                  ]}))
                om.add_variable("biomass_growth_max", crop.data.biomass_growth_max,
                                dict(info_map, **{"data_origin": [("BiomassAllocation", "photosynthesize")
                                                                  ]}))
                om.add_variable("biomass_growth", crop.data.biomass_growth,
                                dict(info_map, **{"data_origin": [("BiomassAllocation", "photosynthesize")
                                                                  ]}))
                om.add_variable("growth_factor", crop.data.growth_factor,
                ...
                om.add_variable("root_biomass", crop.data.root_biomass,
                                dict(info_map, **{"data_origin": [
                                    ("BiomassAllocation", "partition_biomass"),
                                    ("CropManagement", "_recalculate_biomass_distribution")
                                ]}))
                om.add_variable("water_uptake", crop.data.water_uptake,
                                dict(info_map, **{"data_origin": [("WaterUptake", "uptake_water")
                                                                  ]}))
                om.add_variable("water_stress", crop.data.water_stress,
                                dict(info_map, **{"data_origin": [("GrowthConstraints", "constrain_growth")
                                                                  ]}))
                om.add_variable("temp_stress", crop.data.temp_stress,
                                dict(info_map, **{"data_origin": [("GrowthConstraints", "constrain_growth")
                                                                  ]}))
                om.add_variable("nitrogen_stress", crop.data.nitrogen_stress,
                                dict(info_map, **{"data_origin": [("GrowthConstraints", "constrain_growth")
                                                                  ]}))
                ...
                om.add_variable("accumulated_heat_units", crop.data.accumulated_heat_units,
                                dict(info_map, **{"data_origin": [("CropManagement", "cut_crop"),
                                                                  ("HeatUnits", "add_heat_units")
                                                                  ]}))
                om.add_variable("heat_fraction", crop.data.heat_fraction,
                                dict(info_map, **{"data_origin": [("CropData", "heat_fraction")
                                                                  ]}))
                om.add_variable("is_growing", crop.data.is_growing,
                                dict(info_map, **{"data_origin": [("HeatUnits", "absorb_heat_units")
                                                                  ]}))
                om.add_variable("is_dormant", crop.data.is_dormant,
                                dict(info_map, **{"data_origin": [("Dormancy", "enter_dormancy"),
                                                                  ("Field", "_assess_dormancy")
                                                                  ]}))
                om.add_variable("leaf_area_index", crop.data.leaf_area_index,
                                dict(info_map, **{"data_origin": [("CropManagement", "cut_crop"),
                                                                  ("Dormancy", "enter_dormancy"),
                                                                  ("LeafAreaIndex", "grow_canopy"),
                                                                  ("LeafAreaIndex", "add_leaf_area")
                                                                  ]}))
                om.add_variable("canopy_height", crop.data.canopy_height,
                                dict(info_map, **{"data_origin": [("LeafAreaIndex", "grow_canopy")
                                                                  ]}))
                om.add_variable("leaf_area_added", crop.data.leaf_area_added,
                                dict(info_map, **{"data_origin": [("LeafAreaIndex", "determine_leaf_area_added")
                                                                  ]}))
                om.add_variable("optimal_leaf_area_change", crop.data.optimal_leaf_area_change,
                                dict(info_map, **{"data_origin": [("LeafAreaIndex", "grow_canopy")
                                                                  ]}))
                om.add_variable("potential_nitrogen_uptake", crop.data.potential_nitrogen_uptake,
                                dict(info_map, **{"data_origin": [("NitrogenIncorporation", "incorporate_nitrogen")
                                                                  ]}))
                om.add_variable("total_phosphorus_uptake", crop.data.total_phosphorus_uptake,
                                dict(info_map, **{"data_origin": [
                                    ("PhosphorusIncorporation", "tally_total_phosphorus_uptake")
                                ]}))
                om.add_variable("total_nitrogen_uptake", crop.data.total_nitrogen_uptake,
                                dict(info_map, **{"data_origin": [("NitrogenIncorporation",
                                                                   "tally_total_nitrogen_uptake")
                                                                  ]}))
                om.add_variable("optimal_nitrogen_fraction", crop.data.optimal_nitrogen_fraction,
                                dict(info_map, **{"data_origin": [("NitrogenIncorporation", "incorporate_nitrogen")
                                                                  ]}))
                om.add_variable("potential_phosphorus_uptake", crop.data.potential_phosphorus_uptake,
                                dict(info_map, **{"data_origin": [("PhosphorusIncorporation", "incorporate_phosphorus")
                                                                  ]}))
                om.add_variable("actual_phosphorus_uptakes", crop.data.actual_phosphorus_uptakes,
                                dict(info_map, **{"data_origin": [("PhosphorusIncorporation", "uptake_phosphorus"),
                                                                  ("PhosphorusIncorporation",
                                                                   "extend_phosphate_uptakes_to_full_profile")
                                                                  ]}))
                om.add_variable("actual_nitrogen_uptakes", crop.data.actual_nitrogen_uptakes,
                                dict(info_map, **{"data_origin": [("NitrogenIncorporation", "uptake_nitrogen"),
                                                                  ("NitrogenIncorporation",
                                                                   "extend_nitrate_uptakes_to_full_profile")
                                                                  ]}))
                ...
                om.add_variable("cumulative_transpiration", crop.data.cumulative_transpiration,
                                dict(info_map, **{"data_origin": [("WaterDynamics", "cycle_water"),
                                                                  ("Field", "_cycle_water")
                                                                  ]}))
                om.add_variable("cumulative_evapotranspiration", crop.data.cumulative_evapotranspiration,
                                dict(info_map, **{"data_origin": [("WaterDynamics", "cycle_water")
                                                                  ]}))
                om.add_variable("water_deficiency", crop.data.water_deficiency,
                                dict(info_map, **{"data_origin": [("WaterDynamics", "cycle_water")
                                                                  ]}))
                om.add_variable("max_transpiration", crop.data.max_transpiration,
                                dict(info_map, **{"data_origin": [("WaterDynamics", "set_maximum_transpiration")
                                                                  ]}))
                om.add_variable("canopy_water", crop.data.canopy_water,
                                dict(info_map, **{"data_origin": [("WaterDynamics", "evaporate_from_canopy"),
                                                                  ("Field", "_handle_water_in_crop_canopies"),
                                                                  ]}))
                om.add_variable("cut_biomass", crop.data.cut_biomass,
                                dict(info_map, **{"data_origin": [("CropManagement", "cut_crop")
                                                                  ]}))
    ...
    def send_annual_variables(self) -> None:
        ...
        for field in self.fields:
            ...
            om.add_variable("annual_water_content_change", water_content_change,
            ...
            om.add_variable("annual_runoff_machine_manure_inorganic_phosphorus",
                            ...
                            dict(info_map, **{"data_origin": [("Manure", "_leach_and_update_phosphorus_pools"),
                                                              ("SoilData", "do_annual_reset")
                                                              ]}))
