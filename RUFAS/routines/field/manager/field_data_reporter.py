...
class FieldDataReporter:
    ...
    def send_daily_variables(self) -> None:
        ...
        for field in self.fields:
            ...
            om.add_variable("current_residue", field.field_data.current_residue,
                            dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE}))
            ...
            om.add_variable("water_evaporated", field.soil.data.water_evaporated,
                            dict(info_map, **{"units": AcceptableUnits.MILLIMETERS}))
            om.add_variable(
                "eroded_sediment", field.soil.data.eroded_sediment,
                dict(info_map, **{"units": AcceptableUnits.METRIC_TONS})
            )
            ...
            om.add_variable("water_sublimated", field.soil.data.water_sublimated,
                            dict(info_map, **{"units": AcceptableUnits.MILLIMETERS}))
            om.add_variable("cover_type", field.soil.data.cover_type,
            ...
            om.add_variable(
                "profile_carbon_total", field.soil.data.profile_carbon_total,
                dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
            )
            om.add_variable(
            ...
            for index, layer in enumerate(field.soil.data.soil_layers):
                ...
                om.add_variable("water_factor", layer.water_factor,
                                dict(info_map, **{"units": AcceptableUnits.UNITLESS}))
                om.add_variable(
                    "evaporated_water_content", layer.evaporated_water_content,
                    dict(info_map, **{"units": AcceptableUnits.MILLIMETERS})
                )
                om.add_variable(
                ...
                om.add_variable(
                    "metabolic_litter_amount", layer.metabolic_litter_amount,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                om.add_variable(
                    "structural_litter_amount", layer.structural_litter_amount,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                om.add_variable(
                ...
                om.add_variable(
                    "active_carbon_amount", layer.active_carbon_amount,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                om.add_variable("slow_carbon_amount", layer.slow_carbon_amount,
                                dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE}))
                om.add_variable(
                    "passive_carbon_amount", layer.passive_carbon_amount,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                om.add_variable(
                ...
                om.add_variable(
                    "total_soil_carbon_amount", layer.total_soil_carbon_amount,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                om.add_variable(
                    "carbon_emissions", layer.total_soil_carbon_amount,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                om.add_variable(
                ...
                om.add_variable(
                    "percolated_phosphorus", layer.percolated_phosphorus,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                ...
                om.add_variable(
                    "nitrous_oxide_emissions", layer.nitrous_oxide_emissions,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
            ...
            for crop in field.crops:
                ...
                om.add_variable(
                    "biomass_growth_max", crop.data.biomass_growth_max,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                ...
                om.add_variable("growth_factor", crop.data.growth_factor,
                                dict(info_map, **{"units": AcceptableUnits.UNITLESS}))
                om.add_variable(
                    "above_ground_biomass", crop.data.above_ground_biomass,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                ...
                om.add_variable("nitrogen_stress", crop.data.nitrogen_stress,
                                dict(info_map, **{"units": AcceptableUnits.UNITLESS}))
                om.add_variable(
                    "phosphorus_stress", crop.data.phosphorus_stress,
                    dict(info_map, **{"units": AcceptableUnits.UNITLESS})
                )
                om.add_variable(
                    "accumulated_heat_units", crop.data.accumulated_heat_units,
                    dict(info_map, **{"units": AcceptableUnits.UNITLESS})
                )
                ...
                om.add_variable("canopy_height", crop.data.canopy_height,
                                dict(info_map, **{"units": AcceptableUnits.METERS}))
                ...
                om.add_variable(
                    "total_nitrogen_uptake", crop.data.total_nitrogen_uptake,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                om.add_variable(
                ...
                om.add_variable(
                    "cumulative_evaporation", crop.data.cumulative_evaporation,
                    dict(info_map, **{"units": AcceptableUnits.MILLIMETERS})
                )
                om.add_variable(
                ...
                om.add_variable("cut_biomass", crop.data.cut_biomass,
                                dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE}))
                om.add_variable(
                    "wet_yield_collected", crop.data.wet_yield_collected,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                om.add_variable(
                    "dry_matter_yield_residue", crop.data.yield_residue,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
                ...
                om.add_variable("residue_nitrogen", crop.data.residue_nitrogen,
                                dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE}))
                om.add_variable(
                    "residue_phosphorus", crop.data.residue_phosphorus,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
    ...
    def send_annual_variables(self) -> None:
        ...
        for field in self.fields:
            ...
            om.add_variable(
                "annual_nitrates_content_change", nitrates_content_change,
                dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
            )
            ...
            om.add_variable(
                ...
                dict(info_map, **{"units": AcceptableUnits.MILLIMETERS_PER_HECTARE}),
            )
            om.add_variable(
            ...
            for index, layer in enumerate(field.soil.data.soil_layers):
                ...
                om.add_variable(
                    "annual_carbon_CO2_lost", layer.annual_carbon_CO2_lost,
                    dict(info_map, **{"units": AcceptableUnits.KILOGRAMS_PER_HECTARE})
                )
