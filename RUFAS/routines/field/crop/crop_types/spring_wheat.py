...
class SpringWheat(BaseCrop):
    """``SpringWheat`` is a child of the ``BaseCrop`` class, and one of the main crop types.
       Attributes of ``Corn`` are derived from data collected for the crop.
       The crop-specific attributes for spring wheat, are located in the crop config variable ``SpringWheat`` in crop_config.py.
    ...
    def __init__(self, crop_name, data):
        ...
        # get spring wheat variables from input data:
        ...
        for key, val in crop_config.SPRING_WHEAT.items():
