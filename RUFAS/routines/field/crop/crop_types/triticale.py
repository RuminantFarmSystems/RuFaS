...
class Triticale(BaseCrop):
    """``Corn`` is a child of the ``Triticale`` class, and one of the main crop types.
       Attributes of ``Triticale`` are derived from data collected for the crop.
       The crop-specific attributes for triticale, are located in the crop config variable ``Triticale`` in crop_config.py.
    ...
    def __init__(self, crop_name, data):
        ...
        # get triticale variables from input data:
        ...
        for key, val in crop_config.TRITICALE.items():
