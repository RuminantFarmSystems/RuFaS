...
class E2ETestResultsHandler:
    ...
    def compare_actual_and_expected_test_results(
        ...
        for path_set in test_result_path_sets:
            ...
            expected_results = E2ETestResultsHandler._load_expected_results(path_set, convert_variable_table_path)
    ...
    @staticmethod
    def validate_comparison_configuration(output_prefix: str, convert_variable_table_path: str | None) -> None:
        """
        ...
        ValueError
            If a must-change variables file or an expected results file is not valid, or if a must-change variable
            is not found in the expected results of any domain.
        KeyError
        ...
        the input set's must-change variables files, expected results files, and conversion table must load, and
        every must-change variable must be a key of at least one domain's expected results. Running these checks
        before the simulation makes a configuration mistake, such as a mistyped variable name or an expected results
        file left invalid by its autogenerated warning line, fail the task in seconds instead of after the full
        simulation. The comparison repeats the checks afterwards as a backstop.
        """
        om = OutputManager()
        info_map: dict[str, Any] = {
            "class": E2ETestResultsHandler.__class__.__name__,
    ...
    @staticmethod
    def _load_expected_results(path_set: ResultPathType, convert_variable_table_path: str | None) -> dict[str, Any]:
        """
        Loads the expected results of a domain from its expected results file.
        ...
        convert_variable_table_path : str | None
            String path to the csv lookup table to convert the variable names in the expected results to match the
            variable names in the actual results. No conversion is applied when ``None``.

        Returns
        -------
        dict[str, Any]
            The domain's expected results, keyed by variable name.
        ...
        ValueError
            ...
            by ``update_expected_test_results``, or does not contain a dictionary under the ``expected_results`` key.
        """
        om = OutputManager()
        info_map: dict[str, Any] = {
            "class": E2ETestResultsHandler.__class__.__name__,
            "function": E2ETestResultsHandler._load_expected_results.__name__,
            "domain": path_set.domain,
        }
        ...
        try:
            with open(expected_results_path, "r", encoding="utf-8") as expected_results_file:
                filter_and_results = json.load(expected_results_file)
        ...
        expected_results = filter_and_results.get("expected_results") if isinstance(filter_and_results, dict) else None
        if not isinstance(expected_results, dict):
            om.add_error(
                f"End-to-end testing configuration error for {path_set.domain}",
                f"Expected results file {expected_results_path} must contain a dictionary of expected results under "
                "the 'expected_results' key.",
                info_map,
            )
            raise ValueError(
                f"E2E testing error: Expected results file {expected_results_path} must contain a dictionary of "
                "expected results under the 'expected_results' key."
            )
