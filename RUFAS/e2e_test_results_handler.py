...
class E2ETestResultsHandler:
    ...
    def compare_actual_and_expected_test_results(
        json_output_path: Path, convert_variable_table_path: str | None, output_prefix: str, use_accepted_ranges: bool
    ) -> None:
        """
        ...
        use_accepted_ranges : bool
            Whether the variables listed in the input set's accepted ranges file are compared against their accepted
            range instead of the recorded expected results.
        ...
        When ``use_accepted_ranges`` is ``True``, the variables listed in the input set's accepted ranges file are
        likewise left out of the regular comparison and are graded against their accepted range instead: every
        numerical value of a listed variable must fall within the range, whether or not it differs from the recorded
        expected value. The comparison results report each range together with the observed extremes of its variable.
        """
        om = OutputManager()
        ...
        accepted_ranges: dict[str, dict[str, Any]] = (
            E2ETestResultsHandler._load_accepted_ranges(test_result_path_sets) if use_accepted_ranges else {}
        )
        ...
        for path_set in test_result_path_sets:
            ...
            matched_ranged_variables.update(domain_accepted_ranges)
        ...
        if unknown_ranged_variables and all_domains_compared:
            info_map.pop("domain", None)
            info_map.pop("prefix", None)
            om.add_error(
