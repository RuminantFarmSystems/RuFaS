...
- [TBD](TBD) - [minor change] [Tooling] [NoInputChange] [NoOutputChange] Harden the layered `CLAUDE.md` files with conventions learned from the PR #47 review: beef-module extension rules in `RUFAS/biophysical/animal/CLAUDE.md` (dual dispatch-dict registration, extend `animal_groups` instead of parallel loops, `num_*` input-schema naming, merge-then-validate config parsing, calculator `_validate_inputs` on every float field, reporter layering, documented feedlot dead-end limit); sourced-constant + named-clamp rule in `RUFAS/biophysical/CLAUDE.md`; stricter test conventions in `tests/CLAUDE.md` (`mocker.patch.object` only — string-path patches disallowed, `pytest.approx` on all float comparisons, one home per benchmark scenario, shared per-package fixture builders); and a no-speculative-code (YAGNI) rule in the root `CLAUDE.md`.
