...
- [2938](https://github.com/RuminantFarmSystems/RuFaS/pull/2938) - [minor change] [TaskManager] [EEE] [InputChange] [NoOutputChange] Moves EEE execution from the main simulation loop to `TaskManager.handle_post_processing`, enabling EEE to run as a standalone post-processing step on previously saved simulation output. Changes the default value of `run_eee` to `false` in task configuration files. Renames `output_pool_path` to `save_output_pool_to_file_path` and `name` to `prefix` in saved output pool configuration.
