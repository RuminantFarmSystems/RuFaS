...
def get_latlong_from_zip(zip_to_latlong_sheet, zipcodes: list[int]
...
for idx_county_code, county_code in enumerate(county_codes):
    ...
    gps_points_in_state = [x for x,
                           y in zip(tempmap,
                                    county_codes_without_self) if get_statecode(y) == statecode]
    for idx_feed, feed_id in enumerate(feed_columns):
        ...
        if emissions_doc[feed_id][idx_county_code] == 0.0 and float(feed_id) in feed_ids_to_check:
            ...
            neighbors_gps_list, neighbors_distance_list = points_within_radius(gps_county,
                                                                               gps_points_in_state,
                                                                               km_radius)
