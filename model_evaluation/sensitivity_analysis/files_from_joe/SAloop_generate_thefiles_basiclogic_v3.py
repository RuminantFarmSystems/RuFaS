"""
SA loop basic logic
for generalized SA methods
implemented for RuFaS
"""
# from setuptools import setup
# TEMP TO REPLACE from RUFAS.output_handler.reports import life_cycle_report
# import everything
import os
import glob
import introcs
...
# OUTDATED KEEP FOR NOTES sys.path.insert(0, 'C:\\Users\\jw2574\\Documents\\data\\MASM-yg_sensitivity-updated\\SAmethods')
import SAmethods.SAmethods_helpers as SAH
...
setupfile.keys()
setupfile['analysis_type']
setupfile['total_num_analyses'] # THIS NEEDS TO BE ADDED

# THIS needs to be the number: total_number_of_analyses
numberofsimruns = 100
    

animal_management_list = []
for i in range(0,numberofsimruns):
    animal_management_list.append(str('animal_management_' + str(i) + '.json'))
print(animal_management_list)

animal_management_animal_list = []
for i in range(0,numberofsimruns):
    animal_management_animal_list.append(str('animal_management_animal_' + str(i) + '.json'))
print(animal_management_animal_list)

life_cycle_report_list = []
for i in range(0,numberofsimruns):
    life_cycle_report_list.append(str('life_cycle_report_' + str(i) + '.json'))
print(life_cycle_report_list)

# override the values inside each
for i in range(0,numberofsimruns):
    animal_management_list[i]
    # populate the above with the name of -> animal_management_animal_list[i]
    animal_management_animal_list[i]
    # populate the above with the JSON for the corresponding params
    life_cycle_report_list[i]
    # populate the above with the name of -> 'herd_report_' + str(i)



def simulation_run_number(x:float):
    """
    Temporary function that doesn't do anything yet
    """
    x=x+1
    return x

for simrun in range(0,numberofsimruns):
    print(simrun)
    main.main(simulation_run_number(simrun)) # this requires main to be modified to use the term in the SimulationEngine command