"""
generalized SA methods
as temporarily implemented for RuFaS
"""
# import everything
import os
import glob
...
import sys
import shutil
# sys.path.insert(1, 'C:\\Users\\jw2574\\Documents\\data\\MASM-yg_sensitivity-updated\\')
import main
...
sys.path.insert(1, 'C:\\Users\\jw2574\\Documents\\data\\MASM-yg_sensitivity-updated\\SAmethods')
import SAmethods_helpers as SAH
...
prefix_variable = 'outputtesting_11_fast'
...
# can swimply switch order here so whichever is last is the option selected
analysis_type = 'ff'
...
# true if not run yet, false if already run
checkornot = False
if checkornot == True:
    # now we'll list the output variables
    # we first run the sim to generate the output
    main.main()
    # we then read it in and list the output variables and their currently loaded values
    output_df = pd.read_csv(current_directory+output_to_read)
else:
    output_df = pd.read_csv(current_directory + '\\' + prefix_variable + '\\problem0\\herdreports\\herd_report_0.csv')
    odc = output_df.columns
    odcout = []
    for odc in odc:
        odcout.append(odc)
    print("This is the full list of output variables in the output_to_read file")
    print(*odcout, sep='\n')


###################################
# Running the RuFaS model X times #
###################################
...
for i, p in enumerate(problem_list):
    ...
    # len(param_values)
    minutestorun = 5
    print("this problem's analysis will take approximately " + str(len(param_values)*minutestorun) + " minutes to run")
    print("this problem's analysis will take approximately " + str(len(param_values)*minutestorun/60) + " hours to run")
    print("this problem's analysis will take approximately " + str(round(len(param_values)*minutestorun/60/24,3)) + " days to run")
    ...
    for s in range(0,len(param_values)):
        settings = param_values[s]
        namelist = p['names']
        plot_dir_path = prefix_variable + '/problem' + str(i) + '/graphics/'
        plotfilename = plot_dir_path + 'outputs_plotting.json'

        print('staring sample', s+1, 'out of', len(param_values))

        # populate input json with settings from samples
        SAH.json_populater(settings, problem_list[i], json_to_modify)

        out_file_base = 'p' + str(i+1) + '_s' + str(s)
        print('starting ' + out_file_base)

        # execute simulation
        main.main()

        # here we're moving the outputs to a separate folder, to later run the actual analyses on all the main runs
        shutil.move(current_directory+output_to_read, dir_path)
        new_herd_report_name = 'herd_report_' + str(s) + '.csv'
        os.rename(dir_path + 'herd_report.csv', dir_path + new_herd_report_name)

        save_outputs = dir_path + 'outputs_' + out_file_base + '.json'
        save_output_file = open(save_outputs, 'w')
        ...
        # read herd_report for days only after the steady
        #   state day & find averages for outputs needed
        settings_analysis_row = []
        averages_temp = []

        for str_index in herd_structure_output:
            avg = SAH.find_avg_in_last_year(dir_path + new_herd_report_name, odcout.index(str_index), steady_state_day)
            save_dict[str_index] = avg
            settings_analysis_row.append(avg)
            averages_temp.append(avg)
        for str_index in output_indices[i]:
            #avg = find_avg_in_last_year(dir_path + new_herd_report_name, eval(str_index))
            avg = SAH.find_avg_in_last_year(dir_path + new_herd_report_name, odcout.index(str_index), steady_state_day)
            save_dict[str_index] = avg
            settings_analysis_row.append(avg)
            averages_temp.append(avg)

        analysis_lists.append(settings_analysis_row)
        plotting_row.append([list(settings), averages_temp])
        json.dump(save_dict, save_output_file)
        save_output_file.close()

    total_num_cols = len(analysis_lists[0])
    print('\t', total_num_cols, len(herd_structure_output) + len(output_indices[i]))
    assert total_num_cols == len(herd_structure_output) + len(output_indices[i])
    curr_col = 0

    if os.path.exists(dir_path + '/herd_structure/'):
        pass#        shutil.rmtree(dir_path + '/herd_structure/')
    else:
        os.mkdir(dir_path + '/herd_structure/')
    analysis = 'NA_JCW_needstoimplementthis_in_case_of_impossible_analysis'

    for str_index in herd_structure_output:
        #print(p)
        #print(param_values)
        #print([r[curr_col] for r in analysis_lists])
