...
- [3150](https://github.com/RuminantFarmSystems/RuFaS/pull/3150) - [minor change] [Crop and Soil] [NoInputChange] [OutputChange] Re-implements `MineralizationDecomposition._calculate_nutrient_cycling_residue_composition_factor` to return the SWAT 3:1.2.8 value `min(nitrogen_term, phosphorus_term, 1)` clamped to a `1e-5` minimum instead of always returning `1`, so residue C:N and C:P ratios now affect the decay rate constant. Regenerates the end-to-end expected results.
