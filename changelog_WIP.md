...
- [PR_NUMBER](https://github.com/RuminantFarmSystems/RuFaS/pull/PR_NUMBER) - [minor change] [Crop and Soil] [Properties] [InputChange] [NoOutputChange] Adds a `pasture` crop configuration (modeled after tall fescue) for grazing and a `deposit_all_residue_at_harvest` crop-configuration flag; when the flag is set, harvest operations collect no yield and deposit all cut biomass into the field as residue rather than removing it as harvested yield. Implements #3131.
