...
- [3248](https://github.com/RuminantFarmSystems/RuFaS/pull/3248) - [minor change] [Animal] [InputChange] [OutputChange] Adds `intake_option` and `intake_value` ration inputs so dry matter intake of user-defined rations can be set directly (`set_DMI`) or per kg of milk yield / average daily gain (`set_DMI_per_X`), and pins feeds at their inclusion limits instead of crashing when a ration target exceeds them.
