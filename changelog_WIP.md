...
- [XXXX](https://github.com/RuminantFarmSystems/RuFaS/pull/XXXX) - [minor change] [Manure] [Crop and Soil] [NoInputChange] [NoOutputChange] Makes daily spread manure applications honor the user-input `manure_type` from the manure schedule's `daily_spread` block instead of hard-coding solid, so liquid daily-spread requests draw from DailySpread storages instead of silently applying nothing.
