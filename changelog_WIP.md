...
- [XXXX](https://github.com/RuminantFarmSystems/RuFaS/pull/XXXX) - [Major change] [Animal] [Manure] [Crop and Soil] [InputChange] [OutputChange] Adds manure carbon to the soil carbon pools (issue #3103): manure organic carbon applied to fields is estimated from the volatile solids in storage (45% carbon, C:N clamped to 4.24–19.6), accumulated in a per-layer manure carbon residue pool analogous to `plant_residue`, and partitioned daily into the metabolic and structural litter pools by the lignin:N ratio of the applied manure. Lignin is tracked through the manure train (excreted lignin equals ration lignin intake, plus bedding lignin via the new optional `bedding_lignin_fraction` input; separators split it by total solids efficiency). Adds manure carbon reporting variables to the field, soil, and manure outputs.
