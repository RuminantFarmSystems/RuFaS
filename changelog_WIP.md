...
- [XXXX](https://github.com/RuminantFarmSystems/RuFaS/pull/XXXX) - [minor change] [Simulation Engine] [Manure] [InputChange] [NoOutputChange] Adds a `manure_only` simulation type that runs the manure module without any other biophysical modules, fed by a new optional daily manure supply input that states the manure streams entering the manure system each day.
