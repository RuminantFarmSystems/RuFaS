...
- [2966](https://github.com/RuminantFarmSystems/RuFaS/pull/2966) - [minor change] [Animal] [Reproduction] [Genetics] [InputChange] [OutputChange] Adds a selective reproduction strategy that allocates sexed-dairy/conventional-dairy/beef semen by each animal's genetic ranking-index percentile and assigns calf sex from semen-type-specific male-calf rates; replaces the single `semen_type`/`male_calf_rate_*` config with per-group `semen_allocation_proportions`, `selective_repro_strategy`, `ranking_method`, and `genetic_selection_index_weights`.
