...
- [3273](https://github.com/RuminantFarmSystems/RuFaS/pull/3273) - [minor change] [E2E Testing] [NoInputChange] [NoOutputChange] Validates the end-to-end testing configuration (expected results, must-change, and conversion files, and the actual results file prefixes) before the simulation runs, for both the comparison and the expected results update tasks, so configuration mistakes fail in seconds instead of after the full run.
