...
- [XXXX](https://github.com/RuminantFarmSystems/RuFaS/pull/XXXX) - [minor change] [Testing] [EEE] [InputChange] [NoOutputChange] Adds `animals_only` end-to-end testing (issue 3053) and fixes `EmissionsEstimator` crash when feed storage inputs are excluded, which broke `animals_only` and `field_only` simulation types.
